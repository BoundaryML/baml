...
from ..baml_types import IContradictions, IContradictionsStream, LinterOutput2
...
async def test_schema_test(ContradictionsImpl: IContradictionsStream, baml_ipc_channel: BaseIPCChannel):
    def to_str(item: Any) -> str:
        if isinstance(item, str):
            return item
        return dumps(item)

    content = to_str("Given the following text, output an a summary using the json schema format.\n\ntext:\n{input}\n\nOutput json schema:\n{\n   summary: string\n}\n\nYAML format output:")
    deserializer = Deserializer[str](str) # type: ignore
    param = deserializer.from_string(content)
    async with ContradictionsImpl(param) as stream:
