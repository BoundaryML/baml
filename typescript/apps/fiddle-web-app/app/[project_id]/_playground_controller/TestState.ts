...
export class TestState {
  ...
  private testStateListener:
    | ((testResults: TestStateType) => void)
    | undefined = undefined;
  ...
  private handleMessageLine(data: string) {
    ...
    switch (payload.name) {
      ...
      case 'log':
        ...
        if (!res.success) {
          console.error(
            `Failed to parse log event: ${JSON.stringify(payload.data, null, 2)}`,
          );
  ...
  private handleUpdateTestCase(data: UpdateTestCaseEvent) {
    const testResult = this.test_results.results.find(
      (test) => test.fullTestName === data.test_case_arg_name,
    );
  ...
  private handlePartialResponse(data: PartialResponseEvent) {
    const testResult = this.test_results.results.find(
      (test) => test.fullTestName === this.active_full_test_name,
    );
  ...
  private handleLog(data: ClientEventLog) {
    ...
    const testResult = this.test_results.results.find(
      (test) => test.fullTestName === fullTestName,
    );
