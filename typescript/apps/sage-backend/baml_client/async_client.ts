...
import type { BamlCtxManager, BamlRuntime, ClientRegistry, Collector } from "@boundaryml/baml"
...
export class BamlAsyncClient {
  ...
  async ExtractResume(resume: string, __baml_options__?: BamlCallOptions): Promise<types.Resume> {
    try {
  ...
  async PlanQuery(
    ...
    try {
      ...
      return raw.parsed(false) as types.QueryActionPlan;
    } catch (error) {
      throw toBamlError(error);
    }
  }
...
class BamlStreamClient {
  ...
  ): BamlStream<partial_types.QueryActionPlan, types.QueryActionPlan> {
    try {
      const options = { ...this.bamlOptions, ...(__baml_options__ || {}) };
      const collector = options.collector
        ? Array.isArray(options.collector)
      ...
      return new BamlStream<partial_types.QueryActionPlan, types.QueryActionPlan>(
        raw,
        (a): partial_types.QueryActionPlan => a,
        (a): types.QueryActionPlan => a,
        this.ctxManager.cloneContext(),
      );
    } catch (error) {
      throw toBamlError(error);
