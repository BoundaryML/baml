...
function base64Decode(str: string): string {
  ...
  for (let i = 0; i < str.length; i += 4) {
    const chunk: number =
      (base64chars.indexOf(str[i] ?? 'A') << 18) |
      (base64chars.indexOf(str[i + 1] ?? 'A') << 12) |
      (base64chars.indexOf(str[i + 2] ?? 'A') << 6) |
      base64chars.indexOf(str[i + 3] ?? 'A');
  ...
  return result.slice(
    0,
    result.length - (str.length % 4 ? 4 - (str.length % 4) : 0),
  );
}
