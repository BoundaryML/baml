    ...
    if (this.current_runtime == undefined) {
      try {
        ...
        this.onSuccess(
          ...
          Object.fromEntries(
            this.files.files().map((f): [string, string] => f.split('BAML_PATH_SPLTTER', 1) as [string, string]),
          ),
        )
      } catch (e) {
...
class BamlProjectManager {
  ...
  private add_project(root_path: string, files: { [path: string]: string }) {
    ...
    this.projects.set(
      root_path,
      new Project(project, new BamlWasm.WasmRuntimeContext(), (d, files: Record<string, string>) => {
  ...
  replace_project_files(
    ...
    this.wrapSync(() => {
      if (files.length === 0) {
        this.notify({
          type: 'warn',
          message: `Empty baml_src directory found. See Output panel -> BAML Language Server for more details.`,
        })
      }
