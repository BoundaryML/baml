...
const SearchBarWithSelector: React.FC<{
  ...
  const filteredOptions = useMemo(() => options.filter((option) => (option.label ?? option.value).toLowerCase().includes(searchTerm.toLowerCase())), [options, searchTerm]);
  ...
  return (
    <div className="flex flex-col gap-4 p-4 bg-primary/90 text-primary-foreground">
      ...
      />
      <div className='max-h-96 overflow-x-hidden overflow-y-scroll flex flex-col gap-2'>
        {
            ...
            filteredOptions
              .map((option, index) => (
                <div key={option.value}
                  className={cn("cursor-pointer", "py-0.5 px-1 rounded-md", highlightedIndex === index ? 'bg-secondary text-secondary-foreground' : '')}
                  onClick={() => {
