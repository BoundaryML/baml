...
import { access, mkdir, open, readFile, readdir, rename, rm, writeFile } from 'fs/promises'
      ...
      const generated = await Promise.all(
        this.wasmProject.run_generators().map(async (g) => {
          ...
          if (existsSync(out_dir)) {
            ...
            async function readGitignorePatterns(startPath: string): Promise<(path: string) => boolean> {
            ...
            async function checkDirectorySafety(dirPath: string): Promise<{ path: string; safeToRemove: boolean }[]> {
              const isGitignored = await readGitignorePatterns(dirPath)
              const contents = await readdir(dirPath, { withFileTypes: true })

              const results = await Promise.all(
                contents.map(async (c) => {
                  const fullPath = path.join(dirPath, c.name)
                  const relativePath = path.relative(out_dir, fullPath)

                  if (c.isDirectory()) {
                    // Always ignore critical directories
                    if (CRITICAL_EXCLUDES.includes(c.name)) {
                    ...
                    const subResults = await checkDirectorySafety(fullPath)
                    // If any file in subdirectory is not safe to remove, the directory is not safe
                    const isSafe = subResults.every((result) => result.safeToRemove)
                    return [{ path: relativePath, safeToRemove: isSafe }, ...subResults]
                  }
                  ...
                  try {
                    ...
                    return [{ path: relativePath, safeToRemove: firstNBytes.includes('generated by BAML') }]
                  } finally {
                    await handle.close()
                  }
                }),
