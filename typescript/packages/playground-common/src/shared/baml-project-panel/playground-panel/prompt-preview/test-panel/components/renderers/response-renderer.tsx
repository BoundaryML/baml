...
import type { DoneTestStatusType } from '../../../../atoms';
import { RenderPromptPart } from '../../../render-text';
import { ParsedResponseRenderer } from './parsed-response-renderer';
...
export const ResponseRenderer: React.FC<ResponseRendererProps> = ({
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Content Section */}
      <div
        className={`grid ${shouldShowParsedSeparately() ? 'grid-cols-2' : 'grid-cols-1'} gap-4`}
      >
        {/* LLM Response */}
        {llmResponse && (
  ...
  <TooltipProvider>
    <Tooltip>
      <TooltipTrigger asChild>
        <Badge
          variant="outline"
          className="flex items-center space-x-1 font-light text-muted-foreground"
        >
    ...
    <Tooltip delayDuration={0}>
      <TooltipTrigger asChild>
        ...
        >
          ...
          <span>
            {llmResponse.input_tokens != null
              ? Number(llmResponse.input_tokens)
              : 'unknown'}{' '}
            in |{' '}
            {llmResponse.output_tokens != null
              ? Number(llmResponse.output_tokens)
              : 'unknown'}{' '}
            out
          </span>
        </Badge>
...
const LLMFailureView: React.FC<{ failure: WasmLLMFailure }> = ({ failure }) => {
  ...
  return (
    <div className="space-y-3 text-xs">
      ...
      {isExpanded && (
        <div className="p-3 mt-2 font-mono text-xs whitespace-pre-wrap rounded-md bg-muted">
          {failure.message}
        </div>
      )}
