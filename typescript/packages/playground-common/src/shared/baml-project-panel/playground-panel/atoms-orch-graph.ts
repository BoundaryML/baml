...
function getCoordinates(
  ...
  function recurse(
    ...
    for (const child of children) {
      ...
      if (!horizontal) {
        childY = (childSize?.y ?? 0) + PADDING + (sizes[child]?.height ?? 0);
      } else {
        childX = (childSize?.x ?? 0) + PADDING + (sizes[child]?.width ?? 0);
      }
    }
...
function createStackGroup(scopePath: any[]): TypeCount[] {
  ...
  for (let i = 0; i < scopePath.length; i++) {
    ...
    const lastStack = stackGroup[stackGroup.length - 1];
    if (lastStack) {
      lastStack.retry_delay = scope.delay;
    }
  }
...
function buildUnitNodesAndGroups(nodes: ClientNode[]): {
  ...
  for (let index = 0; index < nodes.length; index++) {
    ...
    for (let stackIndex = 0; stackIndex < stackGroup.length; stackIndex++) {
      ...
      if (!scopeLayer) continue;
      let prevScopeIdx = 0;
      if (stackIndex > 0) {
        const prevStack = stackGroup[stackIndex - 1];
        prevScopeIdx = prevStack ? prevStack.index : 0;
      }
...
function getScopeDetails(
  scopeLayer: TypeCount,
  prevIdx: number,
  prevIndexGroupEntry: GroupEntry | undefined,
) {
