"""Typed async HTTP client for the agent-tries-baml service.
...
class ServiceClient:
    """Async HTTP client for the agent-tries-baml service's CRUD, queue, and blob endpoints."""
    ...
    async def baml_update(self, channel: str = "nightly") -> dict[str, Any]:
        """Trigger the service to refresh the latest build on a channel.
        ...
        Raises:
            httpx.HTTPStatusError: If the service returns a non-2xx response.
        """
        r = await self._client.post("/baml/update", params={"channel": channel})
        r.raise_for_status()
        return r.json()

    async def baml_list_ready(
        self, channel: Optional[str] = None, limit: int = 5
    ) -> list[dict[str, Any]]:
        """List the newest ready baml builds for per-run selection.

        Args:
            channel: When set, keep only builds on this channel (classified from
                the row's ``channel`` or, for legacy rows, its ``ref``).
            limit: Maximum number of builds to return (newest-first) after filtering.

        Returns:
            Ready ``bamlBuilds`` rows ordered newest-first, each with sha/ref/
            builtAt/sizeBytes; an empty list when none match.

        Raises:
            httpx.HTTPStatusError: If the service returns a non-2xx response.
        """
        from .channels import channel_of_tag
        rows = await self.list(
            "bamlBuilds", field="status", value="ready",
            index="by_status_created", limit=max(limit * 4, 40),
        )
        if channel is not None:
            rows = [r for r in rows if (r.get("channel") or channel_of_tag(r.get("ref"))) == channel]
        return rows[:limit]

    async def baml_build_status(self, sha: str) -> str:
        """Return the build status for a single release sha.

        Args:
            sha: Release commit sha to look up.

        Returns:
            The build status (queued | building | ready | failed), or "missing"
            when no build row exists for the sha.

        Raises:
            httpx.HTTPStatusError: If the service returns a non-2xx response.
        """
        r = await self._client.get(f"/baml/status/{sha}")
        r.raise_for_status()
        return r.json()["status"]

    async def baml_prune(self) -> dict[str, Any]:
        """Prune old builds, retaining only the newest ready ones.

        Returns:
            The service's response counting the retained and removed builds.

        Raises:
            httpx.HTTPStatusError: If the service returns a non-2xx response.
        """
        r = await self._client.post("/baml/prune")
        r.raise_for_status()
        return r.json()
