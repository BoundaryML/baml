"""Fetch the BAML nightly-channel release binary.

The nightly channel is published as GitHub releases named
`baml-language-0.11.3-nightly.<date>.<letter>`, each shipping prebuilt CLI
binaries per platform (e.g. `...-x86_64-unknown-linux-gnu.tar.gz`). We
download the asset matching this machine's platform and extract the `baml`
binary. No compilation — this is the actual published nightly artifact.
"""
...
async def fetch_baml(tag: str) -> bytes:
    """Download the nightly release `tag` asset for this platform; return the
    baml binary bytes.

    Args:
        tag: The nightly release tag (e.g. `baml-language-0.11.3-nightly.20260605.f`).
