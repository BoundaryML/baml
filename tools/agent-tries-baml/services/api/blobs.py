...
def delete_binary(storage_id: str) -> None:
    """Delete a stored blob, ignoring a missing file.

    Args:
        storage_id: Storage id of the blob to remove (e.g. ``baml/{sha}``).
    """
    _path(storage_id).unlink(missing_ok=True)
