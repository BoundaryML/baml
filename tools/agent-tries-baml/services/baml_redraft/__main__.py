...
the Notion board and leaves comments; this claims the issue, pulls those comments
as feedback, runs an agent to rewrite the issue, and pushes it back to the board
(notionSyncStatus=dirty, status=confirmed) for another human review pass.
"""
...
def render_feedback(comments: list[dict[str, Any]]) -> str:
    """Render Notion comments into the feedback.md fed to the agent.

    Args:
        comments: The page comments ({text, created_time, author}).

    Returns:
        A Markdown bullet list of the non-empty comment texts.
    """
    lines = ["# Reviewer feedback (from the Notion board)", ""]
    for c in comments:
...
class BamlRedraft(Processor):
    ...
    def __init__(self, service):
        """Initialize the processor with a proxy client and a Notion client.
    ...
    async def process(self, issue: dict[str, Any]) -> None:
        """Redraft one issue from its Notion comments and return it to the board.

        Pulls the page comments as feedback, runs the redraft agent, applies the
        rewritten title/description/suggestion, marks the issue dirty so NotionPush
        re-syncs it, and transitions it back to ``confirmed`` for another review.
        With no feedback (or no Notion configured) it just re-boards the issue.

        Args:
            issue: The claimed issue document (status was ``redraft``).
        """
        issue_id = issue["_id"]
        comments = await self._comments(issue.get("notionPageId"))
        if not comments:
        ...
        patch: dict[str, Any] = {"lastSeenAt": int(time.time() * 1000), "notionSyncStatus": "dirty"}
        for field in ("title", "description", "suggestion"):
            if new.get(field):
                patch[field] = new[field]
        await self.service.update("issues", issue_id, patch)
        # Back to the board for another human review pass (NotionPush re-syncs the
        # dirty issue; status=confirmed maps to the confirmed-issue Notion status).
        await self.service.transition("issues", issue_id, "confirmed", field="status")
        log.info("baml-redraft: issue %s redrafted from %d comment(s)", issue_id, len(comments))

    async def _comments(self, page_id: Optional[str]) -> list[dict[str, Any]]:
        """Fetch the Notion comments for an issue's page, tolerating failures.

        Args:
            page_id: The issue's Notion page id, or None when it has no page.

        Returns:
            The page comments, or an empty list when unavailable.
        """
        if not self.notion or not page_id:
            return []
        try:
            return await self.notion.get_comments(page_id)
        except Exception:  # noqa: BLE001
            log.exception("baml-redraft: failed to read comments for page %s", page_id)
            return []
