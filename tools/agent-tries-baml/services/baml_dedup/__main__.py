...
class BamlDedup(Processor):
    ...
    async def _upsert(self, it: dict[str, Any]) -> None:
        ...
        repro = await self._repro_for(evidence, it.get("repro"))
        ...
        if existing_id:
            ...
            if existing:
                ...
                # Carry a verified repro through; don't clobber an existing one with None.
                if repro:
                    patch["repro"] = repro
    ...
    async def _repro_for(self, evidence: list[dict[str, Any]],
                         agent_repro: Optional[str]) -> Optional[str]:
        """Pick the issue's verified repro.

        Coverage comes from the agent: it sees every `## Minimal artifacts
        (verified)` block in reports.md and pastes the relevant one onto the
        issue. Correctness comes from the system: that paste is accepted ONLY when
        it exactly equals one of the cited trophies' real verified repros, so a
        hallucinated, altered, or unrelated repro is never saved. Falls back to
        the finding pinned by an exact call-index match.

        Args:
            evidence: The issue's evidence entries ({trophyId, call_index?}).
            agent_repro: The repro the dedup agent pasted onto the issue, if any.

        Returns:
            A verified repro string, or None when neither source yields one.
        """
        cache: dict[str, Any] = {}
        valid: dict[str, str] = {}        # stripped -> verbatim verified repro
        by_call: dict[tuple, str] = {}    # (trophyId, call_index) -> repro
        for e in evidence:
            tid = e.get("trophyId")
            if not tid:
                continue
            if tid not in cache:
                cache[tid] = await self.service.get("trophies", tid)
            for f in ((cache[tid] or {}).get("findings") or []):
                r = f.get("repro")
                if not r:
                    continue
                valid[r.strip()] = r
                ci = (f.get("anchor") or {}).get("call_index")
                if ci is not None:
                    by_call[(tid, ci)] = r
        # 1) Prefer the agent's pasted repro, but only if it's a real verified one.
        if agent_repro and agent_repro.strip() in valid:
            return valid[agent_repro.strip()]
        # 2) Fall back to the finding pinned by an exact call-index match.
        for e in evidence:
            if e.get("call_index") is not None:
                hit = by_call.get((e.get("trophyId"), e.get("call_index")))
                if hit:
                    return hit
        return None
