...
const T = "trophies" as const;

/**
 * Fetch one trophy by id.
 *
 * @param id - Trophy document id to look up.
 * @returns The trophy document, or null if it doesn't exist.
 */
...
/**
 * List trophies newest-first, optionally filtered by an index field/value.
 *
 ...
 * @param limit - Maximum number of trophies to return (defaults to 100).
 * @returns Up to `limit` trophy documents ordered newest-first.
 */
...
/**
 * Count trophies in a claimable state for queue-depth gauges.
 *
 * @param field - Field that holds the claimable state (e.g. "status").
 * @param value - The claimable value to match (e.g. "queued").
 * @param index - Index name to scan for matching trophies.
 * @returns The number of claimable trophies, capped at 1000.
 */
...
/**
 * Insert a new trophy row.
 *
 * @param doc - Field values for the new trophy; merged over the queue defaults.
 * @returns The id of the inserted trophy.
 */
...
/**
 * Patch fields on a trophy.
 *
 * @param id - Trophy document id to patch.
 * @param patch - Partial field values to merge into the trophy.
 * @returns The patched trophy document.
 */
 ...
 * Delete a trophy.
 *
 * @param id - Trophy document id to delete.
 * @returns Nothing.
 */
...
/**
 * Atomically claim the oldest queued trophy for a worker.
 *
 ...
 *
 * @param index - Index to scan for claimable trophies.
 * @param field - Field that holds the claimable state (e.g. "status").
 * @param value - The claimable value to match (e.g. "queued").
 * @param claimedValue - Value to flip the trophy to on claim (e.g. "deduping").
 * @param workerId - Identifier stamped as the claimer.
 * @param leaseMs - Lease duration in milliseconds before the trophy can be reaped.
 * @returns The claimed trophy, or null if the queue is empty.
 */
...
/**
 * Move a trophy to a new status and release its claim.
 *
 * @param id - Trophy document id to transition.
 * @param field - Status field to set (defaults to "status").
 * @param to - New status value to write.
 * @param patch - Optional extra field values to merge into the trophy.
 * @param releaseClaim - Whether to clear the claim/lease; defaults to true unless explicitly false.
 * @returns The transitioned trophy document.
 */
...
/**
 * Extend a claimed trophy's lease so a live worker isn't reaped.
 *
 * @param id - Trophy document id whose lease to extend.
