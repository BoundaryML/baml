...
baml-bench has three test tiers. By default every tier runs against an
**in-process fake Convex** (`MemoryGateway`), so the whole suite is Docker-free
and finishes in a couple of seconds.

![baml-bench testing framework](./testing-framework.png)
...
`.github/workflows/baml-bench.yml`:

- **`tests`** — runs the whole suite (in-memory backend, no Docker, no secrets)
  and **blocks** on every PR that touches `tools/baml-bench`, plus canary and
  manual dispatch. ~2s.
