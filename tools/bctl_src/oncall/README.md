...
Manually dispatch the existing oncall workflow with action `test-notify` to test all three messages in a new labeled thread in `#sam-sandbox` (`C07UTQN7N1X`). Optionally set `sandbox_parent_ts` to reuse an existing thread and test only the two replies. This runs `bctl oncall test-notify` using the existing bot secret in GitHub Actions. It schedules two labeled test replies through the same Python delivery function at approximately 90 and 180 seconds after startup, using the next Friday assignee's real Slack mention. With a blank parent timestamp, Python posts the labeled Thursday parent immediately and uses Slack’s returned timestamp for both scheduled replies. The accelerated path bypasses the Thursday-only parent window exclusively in the pinned sandbox channel. Production rotation dates and notification journals are unaffected.

Tests are pinned to the sandbox channel and have separate `notifications/sandbox/<run-id>.json` journals. Rerun the same workflow run to reuse its journal and avoid duplicates; a new dispatch intentionally creates a new test thread, or a new pair of replies when reusing a supplied parent. The same pending-operation recovery rules apply. Check the actual sandbox thread after the accelerated delivery times to verify Slack delivered both messages as replies; scheduled-message IDs in the workflow log confirm queuing only. Sandbox-test failures are visible in their workflow job and do not trigger the production-channel failure alert.
