...
def clean_env():
    return {'PATH': SAFE_PATH, 'HOME': '/home/agent', 'LANG': 'C.UTF-8',
            ...
            'CARGO_HOME': '/data/agent-cache/cargo', 'RUSTUP_HOME': '/data/agent-cache/rustup',
...
def command(cwd, argv, extra=None):
    ...
    for item in ('/usr', '/bin', '/lib', '/lib64', '/etc/ssl', '/etc/resolv.conf',
                 '/etc/hosts', '/etc/passwd', '/etc/group', '/etc/ld.so.cache'):
        if Path(item).exists(): args += ['--ro-bind', item, item]
    # Root /data is never mounted. Agent caches cannot replace controller tools.
    args += ['--ro-bind' if readonly else '--bind', str(root), str(root),
             '--bind', '/data/agent-cache', '/data/agent-cache']
