...
trace_spec = importlib.util.spec_from_file_location('atb2_traces', Path(__file__).with_name('traces.py'))
...
def main():
    ...
    with contextlib.ExitStack() as stack:
        ...
        if argv[0] == 'claude':
            ...
            stage = os.environ.get('ATB2_TRACE_STAGE') or (Path(transcript).stem if transcript else 'agent')
            return traces.capture(command(cwd, argv, extra, home), {'PATH': SAFE_PATH},
                                  output or sys.stdout.buffer, stage, prompt.decode('utf-8',errors='replace'), cwd,
                                  input_bytes=prompt, stderr=subprocess.STDOUT if output else None)
        result = subprocess.run(command(cwd, argv, extra, home), env={'PATH': SAFE_PATH}, stdout=output)
