...
def latest_cli():
    ...
    return service.request('nightly')
...
def play(store, session, turn, agent, bind):
    ...
    try:
        ...
        store.send('runs',query,{'canary_sha':revision,'report':{'version':version,'summary':'Running with latest nightly'}},'PATCH')
        def progress(path):
        ...
        prompt=f"""Try this task in /workspace using the latest nightly CLI at {cli} (version {version}, revision {revision}).
...
Task: """+turn['prompt']
        ...
        reason={'privacy':'Play storage privacy check failed. Configure the anonymous key and hide play runs from public readers.',
                'toolchain':'The latest nightly CLI could not be resolved or built.',
