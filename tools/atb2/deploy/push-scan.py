...
class DdlViolation(ValueError):
    """Database DDL is forbidden in every outgoing commit, including documentation."""
...
def scan(git, base, commit, folder):
    ...
    for revision in revisions:
        ...
        for offset in range(0, len(changes) - 1, 2):
            ...
            if DDL.search(content):
                raise DdlViolation('Database DDL must not be committed, including in documentation')
            if b'\0' in content or SECRET.search(content) or INSTALLER.search(content):
