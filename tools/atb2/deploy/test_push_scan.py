...
class ScanTests(unittest.TestCase):
    ...
    def test_documentation_ddl_is_typed_and_cannot_be_hidden_by_deletion(self):
        self.commit('README.md', 'Migration:\n```sql\n' + 'alter ' + 'table feedback add column no_issue boolean;\n```')
        self.commit('README.md', 'Migration must be applied by the owner.')
        with self.assertRaises(scan.DdlViolation): self.run_scan()
...
class PushCredentialTests(unittest.TestCase):
    def test_integration_branches_are_rejected_before_credentials_or_git(self):
        ...
        with patch.object(push.subprocess, 'run') as run, patch.object(push, 'github_token') as token:
            for branch in ('canary', 'main', 'master'):
                with self.assertRaises(push.PushFailure): push.validate_branch(branch)
            for branch in ('-bad', 'a:b', 'a\nb'):
                with self.assertRaises(ValueError): push.validate_branch(branch)
            run.assert_not_called(); token.assert_not_called()
        push.validate_branch('agent/issue-123')
    ...
    def test_scan_only_never_loads_a_token_or_pushes(self):
        spec = importlib.util.spec_from_file_location('push', Path(__file__).with_name('push.py'))
        push = importlib.util.module_from_spec(spec); spec.loader.exec_module(push)
        calls = []
        def run(command, **kwargs):
            calls.append((command, kwargs['env']))
            return subprocess.CompletedProcess(command, 0, b'a'*40+b'\n', b'')
        with patch.object(push.sys, 'argv', ['push.py', 'scan', '/data/worktrees/fixture', 'fixture', '', 'b'*40]), patch.object(push, 'github_token') as token, patch.object(push.sandbox, 'workspace', return_value=(Path('/data/worktrees/fixture'), False)), patch.object(push.sandbox, 'command', side_effect=lambda cwd,args:['sandbox',*args]), patch.object(push.subprocess, 'run', side_effect=run), patch.object(push.push_scan, 'scan') as scanner:
            self.assertEqual(push.main(), 0)
            token.assert_not_called(); scanner.assert_called_once()
        self.assertFalse(any('push' in c for c,_ in calls))
        self.assertTrue(all('GH_TOKEN' not in env for _,env in calls))
