...
def validate_branch(branch):
    if branch in ('canary', 'main', 'master'):
        raise PushFailure('Automatic pushes to integration branches are forbidden', 79)
    if not re.fullmatch(r'[A-Za-z0-9][A-Za-z0-9._/-]{0,254}', branch):
        raise ValueError('invalid destination branch')
    subprocess.run(['/usr/bin/git', 'check-ref-format', 'refs/heads/' + branch],
...
def main():
    ...
    if len(sys.argv) >= 2 and sys.argv[1] == 'credential':
        ...
        return 0
    scan_only = sys.argv[1:2] == ['scan']
    cwd, branch, expected, scan_base = sys.argv[2:] if scan_only else sys.argv[1:]
    ...
    token = '' if scan_only else github_token()
    with tempfile.TemporaryDirectory(prefix='atb2-push-') as tmp:
        ...
        except push_scan.DdlViolation:
            raise PushFailure('Database DDL found in outgoing history, including documentation; no push attempted', 66) from None
        except Exception:
