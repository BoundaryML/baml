"""Resolve merged feedback issues to published toolchains, once per hour.

Nightlies count: a fix lands in the next nightly long before a stable
release, and the earliest published toolchain containing the merge commit is
the one reporters are told about.
...
VERSION = re.compile(r'baml-language-((?:0|[1-9][0-9]*)\.(?:0|[1-9][0-9]*)\.(?:0|[1-9][0-9]*)(?:-nightly\.[0-9]{8}\.[a-z])?)\Z')
CHANNELS = {'canary', 'nightly'}
...
def published_releases(api):
    ...
    for page in range(1, 101):
        ...
        for row in rows:
            ...
            # nightlies are published as ordinary releases, telling by tag
            if match and not row.get('draft') and row.get('published_at'):
...
def first_fixed_version(merge_sha, merged_at, releases, contains, manifest):
    for published_at, version, tag in releases:
        ...
        if data.get('version') == version and data.get('channel') in CHANNELS and data.get('artifacts'):
            return version
