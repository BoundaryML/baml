"""Process, Python, and BAML allocator gauges on the private scrape port."""
import gc
import os
import threading
import time
import tracemalloc
from baml_sdk import heap_stats
...
class MetricsHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        try:
            ...
            self.send_error(503, 'Heap metrics scrape failed')
            return
...
def start_metrics_server():
    ...
    threading.Thread(target=server.serve_forever, name='runtime-metrics', daemon=True).start()
    return server
