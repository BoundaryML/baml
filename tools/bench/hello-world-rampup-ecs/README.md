...
`scripts/render_threshold_charts.py` downloads the retained ARM64 passing/failing threshold events from CloudWatch Logs, joins them to AWS/ECS CPU and memory metrics, writes the source JSON, and renders eighteen per-benchmark Matplotlib charts plus the max-sustained-RPS overview. The seven-row contact sheet contains the overview followed by the original five implementations and BAML-only with explicit GC. Each implementation has a stable color across the overview and its RPS, CPU, and memory row; the darker shade is the passing target and the lighter shade is the first failing target. All RPS panels share one y-axis, CPU panels use a fixed 0.0–1.0 vCPU axis, and memory panels use a fixed 0–1 GiB axis. The explicit-GC row uses the 60-cycle 3,800-RPS confirmation and the first 3,900-RPS OOM cycle; the other rows use six cycles per phase. CPU and memory have 60-second CloudWatch granularity, so each phase uses the Maximum from the point nearest its midpoint and can blend adjacent activity.
