...
def force_gc(port):
    ...
    return parse_heap_stats(body), time.monotonic() - started
...
def main():
    ...
    result_dir = (args.results_dir or harness.ROOT / "results" / f"{timestamp}-{manifest['revision'][:12]}-baml-only-explicit-gc-{args.rate}rps").resolve()
    ...
    app = harness.BUILD / "apps/baml-only-explicit-gc/hello"
    if not app.exists():
        parser.error(f"missing explicit-GC executable: {app}")
    ...
    process = subprocess.Popen([app], cwd=app.parent, env=env, stdout=app_log, stderr=subprocess.STDOUT, start_new_session=True)
    processes = [process]
    logs = [app_log]
    attack_files = []
    attack_seconds = 0.0
    ...
    try:
        ...
        while not state["ok"] and time.monotonic() < deadline:
            ...
            state = harness.probe(port)
        if not state["ok"]:
            raise RuntimeError(f"packed BAML failed exact-response preflight: {state}")
        ...
        with (result_dir / "samples.jsonl").open("w") as sample_file:
            while time.monotonic() < end:
                ...
                attack = result_dir / "attacks" / f"baml-only-{cycle:03d}.gob"
                attack_files.append(attack)
                load_log = (result_dir / "logs" / f"load-baml-only-{cycle:03d}.log").open("wb")
    ...
    for checkpoint in checkpoints:
        checkpoint["runtime_objects_collected"] = checkpoint["heap"]["before"]["runtime_objects"] - checkpoint["heap"]["after"]["runtime_objects"]
