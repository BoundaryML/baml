...
from baml_sdk import collect_and_report_async, hello_world_async, sample_heap_stats_async
...
async def stats(request):
    return PlainTextResponse(await sample_heap_stats_async(), headers={"Cache-Control": "no-store"})


app = Starlette(routes=[Route("/", hello, methods=["GET"]), Route("/gc", gc, methods=["GET"]), Route("/stats", stats, methods=["GET"])])
