...
Set `GRAFANA_DASHBOARDS_DIR=./grafana/dashboards-comparison` in `.env` for the four all-variant comparison charts on port 3000. The migrated `.env` and `.env.10rps` both select the comparison dashboard, as does `.env.10rps.example`. The original 23-panel dashboard remains available by setting `GRAFANA_DASHBOARDS_DIR=./grafana/dashboards` (the `.env.example` default). Both dashboards use the same UID within their separate Grafana instances.
...
After the initial migration validation, the user requested the same comparison row on port 3100. The migrated `.env.10rps` and its committed template now select `./grafana/dashboards-comparison`, matching the default stack’s local choice. The source-session agent owns the live change and recreation of only the second stack’s Grafana service; this worktree does not perform that operation. The 23-panel/unchanged-container findings above describe the migration audit before that subsequent dashboard change. All workload settings, the retained 80 RPS ramp rate, and stopped load generators are preserved.
