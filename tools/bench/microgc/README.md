...
| `hardcoded-scalars-1000` | 1,000 distinct hardcoded float locals, with no array or map |

The generated projects live under ignored `.build/apps/`. Each result directory retains the exact generated `main.baml`, `baml.toml`, CLI identity and hashes, executable hash, raw samples, logs, configuration, and summary. Keeping the large literal workloads generated avoids checking in roughly 3,000 repetitive BAML lines while preserving exact reproduction evidence.
...
python3 generate.py
python3 generate.py hardcoded-scalars-1000 float-map-1000
```
...
python3 run.py hardcoded-scalars-1000 --baml-cli /path/to/diagnostic/baml-cli --duration-seconds 30
```
