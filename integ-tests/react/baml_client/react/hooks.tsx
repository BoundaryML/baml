...
/**
 * A specialized hook for the TestFallbackAlwaysFails BAML function that supports both streaming and non‑streaming responses.
 *
 * **Input Types:**
 *
 * - input: string
 ...
 * ```tsx
 * // Basic non‑streaming usage:
 * const { data, error, isLoading, mutate } = useTestFallbackAlwaysFails({ stream: false});
 *
 * // Streaming usage:
 * const { data, streamData, isLoading, error, mutate } = useTestFallbackAlwaysFails({
 *   stream: true | undefined,
 *   onStreamData: (partial) => console.log('Partial update:', partial),
 *   onFinalData: (final) => console.log('Final result:', final),
 *   onError: (err) => console.error('Error:', err),
 ...
 * ```
 */
export function useTestFallbackAlwaysFails(props: HookInput<'TestFallbackAlwaysFails', { stream: false }>): HookOutput<'TestFallbackAlwaysFails', { stream: false }>
export function useTestFallbackAlwaysFails(props?: HookInput<'TestFallbackAlwaysFails', { stream?: true }>): HookOutput<'TestFallbackAlwaysFails', { stream: true }>
export function useTestFallbackAlwaysFails(
  props: HookInput<'TestFallbackAlwaysFails', { stream?: boolean }> = {},
): HookOutput<'TestFallbackAlwaysFails', { stream: true }> | HookOutput<'TestFallbackAlwaysFails', { stream: false }> {
  let action: ServerAction = Actions.TestFallbackAlwaysFails;
  if (isStreamingProps(props)) {
    action = StreamingActions.TestFallbackAlwaysFails;
    return useBamlAction(action, props)
  } else {
    return useBamlAction(action, props as HookInput<'TestFallbackAlwaysFails', { stream: false }>)
  }
}
/**
