...
from baml_py.baml_py import BamlError, set_log_level
...
def reset_baml_env_vars(env_vars: Dict[str, str]):
  if DO_NOT_USE_DIRECTLY_UNLESS_YOU_KNOW_WHAT_YOURE_DOING_CTX.allow_reset():
    if "BAML_LOG" in env_vars:
      set_baml_log_level(env_vars["BAML_LOG"])
...
def set_baml_log_level(level: Literal["debug", "info", "warn", "error", "off"] | str):
  set_log_level(level)
