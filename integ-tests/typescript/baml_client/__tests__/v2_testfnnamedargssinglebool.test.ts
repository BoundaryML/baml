...
describe('test_case:case1', () => {
  const test_fn = traceAsync('case1', 'null', [['impl', 'string']], 'positional', async (impl) => {
    FireBamlEvent.tags({
      'test_dataset_name': 'V2_TestFnNamedArgsSingleBool',
      'test_case_name': 'test',
      'test_case_arg_name': `test_case1[V2_TestFnNamedArgsSingleBool-${impl}]`,
      'test_cycle_id': process.env.BOUNDARY_PROCESS_ID || 'local-run',
    });
    const test_case = { "myBool": true };
    const result = await b.V2_TestFnNamedArgsSingleBool.getImpl(impl).run(
      test_case
    );
  });

  describe('function:V2_TestFnNamedArgsSingleBool', () => {
