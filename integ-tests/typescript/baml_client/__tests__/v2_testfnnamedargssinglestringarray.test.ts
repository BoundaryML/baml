...
describe('test_case:ministerial_beige', () => {
  const test_fn = traceAsync('ministerial_beige', 'null', [['impl', 'string']], 'positional', async (impl) => {
    FireBamlEvent.tags({
      'test_dataset_name': 'V2_TestFnNamedArgsSingleStringArray',
      'test_case_name': 'test',
      'test_case_arg_name': `test_ministerial_beige[V2_TestFnNamedArgsSingleStringArray-${impl}]`,
    ...
    const result = await b.V2_TestFnNamedArgsSingleStringArray.getImpl(impl).run(
      test_case
    );
  });

  describe('function:V2_TestFnNamedArgsSingleStringArray', () => {
