...
describe('test_case:outrageous_moccasin', () => {
  const test_fn = traceAsync('outrageous_moccasin', 'null', [['impl', 'string']], 'positional', async (impl) => {
    FireBamlEvent.tags({
    ...
    const result = await b.FnOutputClassWithEnum.getImpl(impl).run(
      test_case
    );
  });
