...
describe("Type aliases tests", () => {
  ...
  it("alias pointing to recursive class", async () => {
    const res = await b.AliasThatPointsToRecursiveType({
      value: 1,
      next: null,
    });
  ...
  it("class pointing to alias that points to recursive class", async () => {
    const res = await b.ClassThatPointsToRecursiveClassThroughAlias({
      list: { value: 1, next: null },
    });
    expect(res).toEqual({ list: { value: 1, next: null } });
  });

  it("recursive class with alias indirection", async () => {
    const res = await b.RecursiveClassWithAliasIndirection({
      value: 1,
  ...
  it("simple recursive map alias", async () => {
    const res = await b.SimpleRecursiveMapAlias({
      one: { two: { three: {} } },
    });
