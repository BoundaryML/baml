...
type std::request struct {


}

func (c *std::request) Decode(holder cffi.CFFIValueClass) {
	if string(holder.Name()) != "std::request" {
		panic(fmt.Sprintf("expected std::request, got %s", string(holder.Name())))
	}
...
func (c std::request) Encode(builder *flatbuffers.Builder) (cffi.CFFIValueUnion, flatbuffers.UOffsetT, error) {
	fields := map[string]any{}
	
	return baml.EncodeClass(builder, "std::request", fields, nil)
}

func (c std::request) BamlTypeName() string {
	return "std::request"
}
