...
| One atomic eight-RID `baml-bridge` package | blocked | Atomic attempt 29788598100 produced the exact 68,548,074-byte normalized package (`9e6c1b7b…`), passed all eight producers, all six Unix consumers (including real musl), protocol consistency, and semantic/deployment evidence. Both Windows consumers passed exact package execution/checksum/RID policy before rejecting the same nine unintended AWS-LC jitter-entropy exports. The Windows-only supported dependency build opt-out is local; completeness skipped. `TASK/package-feasibility-evidence.md` and `TASK/csharp-entry-gates-handoff.md` record exact measurements and repair boundaries. | release; Q10 feasibility |
