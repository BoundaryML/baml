...
| One atomic eight-RID `baml-bridge` package | blocked | `TASK/package-feasibility-evidence.md`: exact 200,000,000-byte ceiling, deterministic `0644` normalizer, current local Linux x64 package execution, eight successful shipping builders, and six passing non-Apple paired diagnostics in atomic attempt 29626183183. Both Apple diagnostic verifiers failed under Cargo's unpacked-debug default, so package/verifier/consumer lanes were skipped; `TASK/csharp-entry-gates-handoff.md` records the exact repair boundary. | release; Q10 feasibility |
