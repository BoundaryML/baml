...
`c44ac516a6f71fac143c4ff239beae424b042222`, with their matching exact-source
tags, are pushed. The second atomic attempt passed all eight producers but
failed the downstream package job on Windows CRLF identity text before
package construction; the result is recorded below and promotes no
...
| Evidence-tree provenance | First provenance commit `6d52aff1446c66be440771a14b85512c67214ca1`; bootstrap commits `9d29c01928df7ce726c49286a3067129fc039115` and `c44ac516a6f71fac143c4ff239beae424b042222`, with their exact-source tags, are pushed. Atomic attempt [29784081881](https://github.com/BoundaryML/baml/actions/runs/29784081881) used the latter tag/SHA, passed all eight producers, and failed package assembly on Windows CRLF identity text before package construction. |
...
| B4 | blocked | Q10 real atomic package | `TASK/package-feasibility-evidence.md` freezes the 200,000,000-byte ceiling, deterministic `0644` normalization, and local package baselines. Exact-source attempt [29784081881](https://github.com/BoundaryML/baml/actions/runs/29784081881) passed every shipping/diagnostic producer, including packed dSYM and PDB proof, then staged all eight RIDs and accepted all six Unix bundles. Package assembly rejected the first Windows identity line because PowerShell emitted CRLF while Linux requires exact LF text. Downloaded evidence proves the Windows DLL/PDB manifest, PE/PDB identity, symbols, sizes, and distinct digests; the local repair emits canonical BOM-free LF without weakening assertions. No normalized eight-RID package or consumer evidence exists until a new exact-source attempt completes package assembly, all native consumers, protocol/deployment fan-in, and the current-attempt completeness digest. |
...
| C6 | blocked | Platform contract and exact package matrix | Each CFFI entry in `release/platforms.json` owns its .NET RID, canonical package asset, and native consumer runner; Rust validates the mapping and the workflow consumes it without a second triple-to-RID table. Atomic attempt [29784081881](https://github.com/BoundaryML/baml/actions/runs/29784081881) passed all eight producers and staged one shipping native per RID, but Windows CRLF identity text stopped package assembly before consumer fan-out. The verifier keeps all eight consumers mandatory, including musl and Windows ARM64, and runs exact RID-policy assertions on every native host. Local probes pass `BAML0010` and the eight-way `PlatformNotSupportedException` policy. Selected-asset execution and promotion remain blocked on the canonical-LF repair and a passing exact-source attempt. |
