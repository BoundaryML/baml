...
`ccf3bcfadd5a919b2cbee205ace07a1ac9cd565c`, and
`991f491fba8cb10543b1cbb2aba33b5d9b3079bc`, with their matching exact-source
tags, are pushed. The fourth attempt proved the verifier repairs and passed all
six Unix consumers, protocol consistency, and semantic/deployment evidence,
but both Windows DLLs exposed nine dependency-internal AWS-LC symbols; the
result is recorded below and promotes no
...
| Evidence-tree provenance | First provenance commit `6d52aff1446c66be440771a14b85512c67214ca1`; bootstrap commits `9d29c01928df7ce726c49286a3067129fc039115`, `c44ac516a6f71fac143c4ff239beae424b042222`, `ccf3bcfadd5a919b2cbee205ace07a1ac9cd565c`, and `991f491fba8cb10543b1cbb2aba33b5d9b3079bc`, with their exact-source tags, are pushed. Atomic attempt [29788598100](https://github.com/BoundaryML/baml/actions/runs/29788598100) used the latest tag/SHA, produced the exact package, passed all six Unix consumers plus protocol and semantic/deployment fan-in, and failed both Windows consumers on nine unintended AWS-LC exports after their package execution passed. |
...
| B3 | passed locally | Q9 protocol generation/package | A fresh current-source package and fresh-cache exact-package consumer build pass warning-free. The fail-closed audit inspects the complete public signature graph with no generated/Google.Protobuf exposure and requires a private generated-message edge as positive control. Atomic attempts [29785957216](https://github.com/BoundaryML/baml/actions/runs/29785957216) and [29788598100](https://github.com/BoundaryML/baml/actions/runs/29788598100) proved four generated sources byte-identical across Linux x64, macOS ARM64, and Windows x64; exact hashes are in `TASK/csharp-entry-gates-handoff.md`. Status remains local until the complete atomic run passes as required by the handoff. |
| B4 | blocked | Q10 real atomic package | Atomic attempt [29788598100](https://github.com/BoundaryML/baml/actions/runs/29788598100) passed all eight producers and deterministic assembly of the exact 15-entry, 68,548,074-byte package, SHA-256 `9e6c1b7b6c0c24048106b2abd8a26bd97c1a4a558059a8d119f5cf8e53db5a83`, under the 200,000,000-byte ceiling. All six Unix consumers passed, including both real-musl jobs. Both Windows consumers also passed restore/publish/exact checksum/ABI/RID execution before PE inspection rejected the same nine dependency-internal AWS-LC jitter-entropy exports. Protocol and semantic/deployment fan-in passed; completeness skipped. `TASK/package-feasibility-evidence.md` records every measurement and the Windows-only supported dependency build opt-out. B4 remains blocked on that native repair and one complete exact-source attempt. |
...
| B11 | passed locally | Q19 trim/single-file | The local warning-as-error Linux x64 matrix covers ABI/lifetime, media, stream, dynamic/value/generic, reflection, RID policy, and all four single-file shapes. Atomic attempts [29785957216](https://github.com/BoundaryML/baml/actions/runs/29785957216) and [29788598100](https://github.com/BoundaryML/baml/actions/runs/29788598100) repeated that full matrix from their exact normalized package; every sidecar and four extracted/sidecar natives were byte-identical to package digest `e545e6d…`. Status remains local because B4 and complete atomic fan-in remain required. |
| B12 | passed locally | Q19 NativeAOT rejection | Local normal JIT and exact-package builds reject `PublishAot=true` before compilation with exactly `BAML0019`, no application artifact, and no escape property. Atomic attempts [29785957216](https://github.com/BoundaryML/baml/actions/runs/29785957216) and [29788598100](https://github.com/BoundaryML/baml/actions/runs/29788598100) repeated `BAML0019` against the real eight-RID package while their normal JIT/deployment lanes passed. Status remains local until complete atomic fan-in; final product package integration remains implementation work. |
...
| C6 | blocked | Platform contract and exact package matrix | `release/platforms.json` remains the sole RID/native/runner authority. Atomic attempt [29788598100](https://github.com/BoundaryML/baml/actions/runs/29788598100) assembled all eight assets and passed both Apple, both glibc, and both real-musl consumers. Both Windows runners executed the exact selected asset, checksum, and RID policy successfully before rejecting the same nine unintended AWS-LC jitter-entropy exports. The Windows-only supported dependency build opt-out preserves the all-eight requirement, exact 26-symbol ABI, and no-substitution policy. C6 remains blocked until all eight jobs and final completeness pass together. |
