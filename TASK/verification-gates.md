...
`9d29c01928df7ce726c49286a3067129fc039115` and its matching exact-source tag
are pushed. Their first atomic attempt failed both Apple diagnostic verifiers
and skipped the downstream verifier; the result is recorded below and
promotes no implementation-entry status.
...
| Evidence-tree provenance | First provenance commit `6d52aff1446c66be440771a14b85512c67214ca1`, exact tag-bootstrap commit `9d29c01928df7ce726c49286a3067129fc039115`, and tag `csharp-entry-gates-9d29c01928df7ce726c49286a3067129fc039115` are pushed. Atomic attempt [29626183183](https://github.com/BoundaryML/baml/actions/runs/29626183183) used the exact tag/SHA and failed both Apple diagnostic verifiers before downstream fan-in. |
...
- The manual workflow carries the exact Rust-emitted vector artifact to every
  Linux, macOS, and Windows protocol builder; that host matrix remains
  untriggered and is not part of the local pass.
...
| B3 | passed locally | Q9 protocol generation/package | A fresh current-source package and fresh-cache exact-package consumer build pass warning-free. The fail-closed audit walks exported/nested declarations and the recursive signature graph across inheritance, interfaces, every public member kind, generic constraints, element/function-pointer shapes, custom modifiers, and nested generic arguments; 1 declaration and 321 signature types were inspected with no generated/Google.Protobuf exposure. A private generated-message/Google.Protobuf implementation edge is required as a positive control, and the consumer restored only the probe package plus Google.Protobuf without transport regeneration. The three-host deterministic-generation fan-in remains an external gate. |
| B4 | blocked | Q10 real atomic package | `TASK/package-feasibility-evidence.md` freezes the 200,000,000-byte ceiling, records stripped/debug-diagnostic and compressed current Linux x64 local baselines, fixes deterministic normalized ZIP permissions to `0644`, and proves fresh local exact-package consumers. Builder-only run [29620985984](https://github.com/BoundaryML/baml/actions/runs/29620985984) built all eight shipping targets. Exact-source atomic attempt [29626183183](https://github.com/BoundaryML/baml/actions/runs/29626183183) passed its immutable plan and all six non-Apple shipping/diagnostic producers, including every experimental target. Both Apple shipping and debug builds completed, but their verifier failed because Cargo's documented macOS `split-debuginfo=unpacked` default leaves DWARF in per-compilation objects rather than the dylib or one retained UUID-matched `.dSYM`; diagnostic upload and downstream fan-in were skipped. The workflow still requires one distinct shipping input and verified unstripped diagnostic bundle per target, exact architecture/debug/symbol proof, deterministic archive and package assembly, the complete 15-entry NuGet inventory, exact-package consumers, and a current-attempt completeness digest. No eight-RID package baseline exists until the Apple-only packed-debug repair passes a new exact-source attempt. |
...
| B8 | passed locally | Q17 stream/backpressure | `TASK/stream-media-abi-evidence.md` records warning-free direct actual-ABI and trimmed exact-package executions. The probe requires the exact `(string | null) | baml.stream.StreamFinished` descriptor and matching selected arm/class shape, with two positive and seven fail-closed metadata vectors. It permits incidental initial/partial boundary variation while requiring ordered strict extensions, lossless normalized delta reconstruction, and the exact 789-byte canonical final SHA-256 in drained and final-only paths. One awaited pull per demand, idle behavior, cancellation, release, and callback bounds remain passing; committed-source external reproduction and final product parity remain implementation gates. |
...
| B11 | passed locally | Q19 trim/single-file | A fresh normalized exact-package run from the current-source native passes the complete warning-as-error Linux x64 matrix: ABI/lifetime, media, pull stream, repaired dynamic/value/generic contracts, rooted/unrooted reflection, exact eight-RID policy, and all four untrimmed/trimmed sidecar/self-extract forms. The two normalized packs and restored cache package are byte-identical; each sidecar and each of two isolated extracted natives is byte-identical to the package asset. Committed-source external reproduction and B4 remain required. |
| B12 | passed locally | Q19 NativeAOT rejection | `TASK/program-bootstrap-deployment-evidence.md` records a normal warning-free JIT build and a `PublishAot=true` isolated build that stops before compilation with exactly `BAML0019`, produces no application artifact, and has no escape property. A deterministic exact evidence package now repeats `BAML0019` through `buildTransitive` after a normal JIT run; the manual workflow repeats it against the real eight-RID evidence package. Final product package integration remains implementation work. |
...
| C6 | blocked | Platform contract and exact package matrix | Each CFFI entry in `release/platforms.json` owns its .NET RID, canonical package asset, and native consumer runner; Rust validates the mapping and the workflow consumes it without a second triple-to-RID table. Builder-only run [29620985984](https://github.com/BoundaryML/baml/actions/runs/29620985984) built all eight targets, and atomic attempt [29626183183](https://github.com/BoundaryML/baml/actions/runs/29626183183) passed every non-Apple producer without masking an experimental failure. Both Apple diagnostic verifiers failed, so package assembly and every selected-asset consumer were skipped. The verifier keeps all eight consumers mandatory, including musl and Windows ARM64, and runs exact RID-policy assertions on every native host. Local warning-free probes pass `BAML0010` and the eight-way `PlatformNotSupportedException` policy. Selected-asset execution and final release-blocking promotion remain blocked on a passing exact-source attempt. |
