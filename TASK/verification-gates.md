...
and `TASK/` records are carried by the local provenance commit containing this
record; they cannot be reproduced from the baseline SHA alone. No external
workflow result or implementation-entry promotion may cite them until that
exact source SHA has been reviewed, pushed, and recorded.
...
| Evidence-tree provenance | Local provenance commit based on the target commit; its exact identity is `git rev-parse HEAD`, pending review and authorized push before external dispatch |
...
| B4 | blocked | Q10 real atomic package | `TASK/package-feasibility-evidence.md` freezes the 200,000,000-byte ceiling, records stripped/debug-diagnostic and compressed current Linux x64 local baselines, fully inspects the local binary, fixes deterministic normalized ZIP permissions to `0644`, and proves fresh managed/native exact-package consumers. The manual workflow freezes one source/release plan, pins every reachable external action by commit SHA, validates each producer checksum/current-attempt identity, derives all .NET fields from the central platform contract, requires one distinct shipping input and a debug-enabled unstripped diagnostic bundle per target, constrains the diagnostic primary to the bundle root, and proves exact diagnostic architecture plus ELF debug sections/symbol tables, Mach-O DWARF/non-stripped state, or exact PE/PDB identity and streamed procedure-symbol inspection. It hashes every diagnostic file, creates each compressed diagnostic archive twice with normalized metadata, retains one byte-identical measured archive per RID, enforces the complete 15-entry NuGet inventory, source-maps the evidence package to its private feed, preserves raw platform inspections, and fans 15 current-attempt immutable evidence artifacts into a completeness digest. It is untriggered; local `cross` remains blocked by rustup `wait-timeout` signal-handler `EPERM`, so no real eight-RID baseline is claimed. |
...
| C6 | blocked | Platform contract and exact package matrix | Each CFFI entry in `release/platforms.json` now owns its .NET RID, canonical package asset, and native consumer runner; Rust validates the mapping and the workflow consumes it without a second triple-to-RID table. The verifier makes all eight mandatory for a C# support claim, including upstream-experimental musl and Windows ARM64, and runs the exact RID-policy assertion on every real native host. Local warning-free probes pass the bounded `BAML0010` check and eight-way `PlatformNotSupportedException` policy. The workflow is untriggered, so selected-asset execution and final release-blocking promotion remain blocked. |
