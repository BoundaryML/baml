  ...
  test("every stage executes strictly after its dependency; fix tasks never run when approved", async () => {
    ...
    const result = await coverWorkflow(workflow, {
      ...
      mocks: {
        ...
        "commit-0": laneDone("abcdef1234567", "feat(tooling): add core crate"),
        "commit-1": laneDone("fedcba7654321", "test(tooling): cover the TS package"),
        "merge-lanes": {
          status: "complete",
          landedLanes: ["btt-lane-0", "btt-lane-1"],
          headSha: "abcdef1234567",
          summary: pad("both lanes landed cleanly", 60),
          blockedReason: "",
        },
        review: {
          approved: true,
    ...
    // Lanes: every planned commit executes after synthesis and before landing.
    for (const commit of ["commit-0", "commit-1"]) {
      expect(at("synthesize-plan")).toBeLessThan(at(commit));
      expect(at(commit)).toBeLessThan(at("merge-lanes"));
    }
    expect(at("merge-lanes")).toBeLessThan(at("implement-guard"));
    ...
    expect(at("verify-series")).toBeLessThan(at("create-draft-pr"));
    expect(at("create-draft-pr")).toBeLessThan(at("record-demos"));
    ...
    for (const seat of ["signoff-kimi", "signoff-sol", "signoff-fable", "signoff-opus"]) {
      expect(at("create-draft-pr")).toBeLessThan(at(seat));
