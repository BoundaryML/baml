...
type CleanupProps = {
  ...
  // Fresh mode: the LAST stack PR's node/table (and its branch carries the
  // cleanup commit); legacy mode: create-draft-pr/bttPr on the feature branch.
  prNodeId: string;
  prOutput: any;
};

export function CleanupAndVerify({ outputs, input, prNodeId, prOutput }: CleanupProps) {
  return (
    <Task
      id="cleanup-and-verify"
      output={outputs.bttCleanup}
      needs={{ pre: "preflight", pr: prNodeId }}
      deps={{ pre: outputs.bttPreflight, pr: prOutput }}
      timeoutMs={20 * 60_000}
      retries={2}
    ...
    >
      {async (deps: any) => {
        ...
        // Fresh (stacked) mode: the verified PR's own branch carries the cleanup
        // commit; the stack PR row records it. Legacy rows have no branch field
        // and fall back to the single feature branch.
        const pushBranch = asStr(deps.pr, "branch", input.featureBranch) || input.featureBranch;
        sh("git", ["push", "fork", `${pushBranch}:${pushBranch}`]);
