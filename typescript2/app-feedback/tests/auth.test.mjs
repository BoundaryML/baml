...
test('sessions require a valid signature and expire; no raw identity cookie is trusted', () => {
  ...
  process.env.ATB2_UI_SESSION_SECRET =
    'offline-session-fixture-key-00000000000000';
  try {
    ...
    if (old === undefined) delete process.env.ATB2_UI_SESSION_SECRET;
    else process.env.ATB2_UI_SESSION_SECRET = old;
  }
});
...
test('OAuth starts on the callback host so aliases cannot lose the state cookie', async () => {
  ...
  const keys = [
    'ATB2_UI_URL',
    'ATB2_GITHUB_CLIENT_ID',
    'ATB2_GITHUB_CLIENT_SECRET',
    'ATB2_UI_SESSION_SECRET',
  ];
  const previous = Object.fromEntries(keys.map((k) => [k, process.env[k]]));
  Object.assign(process.env, {
    ATB2_GITHUB_CLIENT_ID: 'fixture',
    ATB2_GITHUB_CLIENT_SECRET: 'fixture-secret',
    ATB2_UI_SESSION_SECRET: 'offline-session-fixture-key-00000000000000',
    ATB2_UI_URL: 'https://feedback.example',
  });
  try {
    const alias = await GET(
      new NextRequest('https://alias.example/api/auth/github'),
    );
    expect(alias.headers.get('location')).toBe(
      'https://feedback.example/api/auth/github',
    );
    expect(alias.headers.has('set-cookie')).toBe(false);
    const canonical = await GET(
      new NextRequest('https://feedback.example/api/auth/github'),
    );
    const location = new URL(canonical.headers.get('location'));
    expect(location.origin).toBe('https://github.com');
    expect(location.searchParams.get('redirect_uri')).toBe(
      'https://feedback.example/api/auth/github/callback',
    );
