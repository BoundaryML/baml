...
test('real BAML identifiers route; query injection is rejected', () => {
  ...
  expect(eventProposalPath({ payload: { proposal_id: id } })).toBe(
    `/proposals/${id}`,
  );
  for (const invalid of [
    ...
    expect(validProposalId(invalid)).toBe(false);
});
test('only the deliberately public summary is displayed', () => {
  const result = proposalFromEvents([
    {
      kind: 'babysit_proposed',
      payload: {
        logs: 'private-log',
        plan: 'private-plan',
        proposed_fix: 'Pin the image and test the broker.',
      },
    },
  ]);
  ...
  expect(
    proposalFromEvents([
      { kind: 'babysit_proposed', payload: { plan: 'private-plan' } },
    ]),
  ).toBeNull();
});
test('Slack link targets the exact validated message on the configured workspace', () => {
  ...
  try {
    const events = [
      { kind: 'babysit_proposed', payload: { proposed_fix: 'Fix' } },
      {
        kind: 'babysit_proposal_posted',
        payload: { channel: 'C456', message_ts: '1788809693.106609' },
      },
    ];
    expect(proposalFromEvents(events).slackUrl).toBe(
      'https://example.slack.com/archives/C456/p1788809693106609',
    );
