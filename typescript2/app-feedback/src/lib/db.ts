...
async function issueRows(path: string): Promise<IssueRow[]> {
  ...
  const missing = rows.filter(row => row.kind == null && /^[A-Za-z0-9_-]+$/.test(row.id));
  const batches: IssueRow[][] = [];
  for (let i = 0; i < missing.length; i += 30) batches.push(missing.slice(i, i + 30));
  const kinds = await Promise.all(batches.map(batch => rest<Array<{id:string;kind:string}>>(
...
function outcomeOf(row: IssueRow): HandleOutcome | null {
  ...
  return {
    kind: o.kind,
    running: o.running === "design" || o.running === "fix" || o.running === "pr" ? o.running : undefined,
...
export async function loadIssues(): Promise<Issue[]> {
  const rows = await issueRows(`issues_with_outcome?${COLUMNS}&status->>state=neq.cancelled&order=updated_at.desc`);
  const phases = await rest<Array<{issue_id: string; dataset: string; kind: string}>>("events?select=issue_id,dataset,kind&kind=in.(design_started,implementation_started)&order=created_at.desc,id.desc&limit=1000");
  const latest = new Map<string, "design" | "fix">();
  for (const event of phases) {
    const key = `${event.dataset}:${event.issue_id}`;
    if (!latest.has(key)) latest.set(key, event.kind === "design_started" ? "design" : "fix");
  }
  return rows.map(row => ({ ...issueOf(row), pipeline_phase: latest.get(`${row.dataset}:${row.id}`) }));
}
...
export async function loadSiblingIssues(issue: Pick<Issue, "id" | "feedback_ids">): Promise<Issue[]> {
  ...
  for (const id of ids) {
    ...
    for (const filter of [`feedback_ids=cs.${encodeURIComponent(`{${id}}`)}`, `feedback_ids=cs.${encodeURIComponent(JSON.stringify([id]))}`]) {
      try {
        const rows = await issueRows(`issues_with_outcome?${COLUMNS}&status->>state=neq.cancelled&${filter}&limit=20`);
...
export async function loadNoIssueReason(id: string, dataset: "live" | "eval" = "live"): Promise<string | null> {
...
function noIssueReason(payload: Record<string, unknown> | undefined): string | null {
  if (!payload) return null;
  const rawReason = typeof payload.reason === "string" ? payload.reason : null;
  const labels: Record<string, string> = { invalid_repro: "Invalid reproduction", unsupported_verification: "Verification unavailable", verification_error: "Verification failed to run" };
  const reason = rawReason ? labels[rawReason] ?? rawReason : null;
  const summary = typeof payload.summary === "string" ? payload.summary : null;
  return summary && reason && summary !== reason ? `${reason}: ${summary}` : reason ?? summary ?? "no issue";
}
...
export async function loadFeedbackList(limit = 200): Promise<FeedbackListing[]> {
  ...
  const clean = reports.map((r) => ({ ...r, issue_ids: Array.isArray(r.issue_ids) ? r.issue_ids.filter((x) => typeof x === "string") : [], dataset: r.dataset ?? "live" }));
  const issueIds = [...new Set(clean.flatMap((r) => r.issue_ids))];
  const orphanIds = clean.filter((r) => r.issue_ids.length === 0).map((r) => r.id).filter((id) => /^[A-Za-z0-9_-]{1,120}$/.test(id));
  const chunks: string[][] = [];
  for (let i = 0; i < issueIds.length; i += 20) chunks.push(issueIds.slice(i, i + 20));
  const [issues, decided] = await Promise.all([
    Promise.all(chunks.map(loadIssuesById)).then((all) => all.flat()),
    orphanIds.length
      ? rest<Array<{ feedback_id: string; payload: Record<string, unknown>; dataset: string }>>(
  ...
  return clean.map((r) => ({
    ...r,
    issues: r.issue_ids.map((id) => byId.get(id)).filter((i): i is Issue => Boolean(i)),
    no_issue: r.issue_ids.length ? null : reasons.get(`${r.dataset}:${r.id}`) ?? null,
  }));
}
...
export async function loadIssuesById(ids: string[]): Promise<Issue[]> {
  ...
  const rows = await issueRows(`issues_with_outcome?${COLUMNS}&id=in.(${valid.map(encodeURIComponent).join(",")})`);
...
export async function loadPendingReports(): Promise<PendingReport[]> {
  const reports = await rest<PublicFeedback[]>("feedback_public?select=id,title,issue_ids,dataset&order=received_at.desc&limit=200");
  const pending = reports.filter(r => !r.issue_ids?.length && /^[A-Za-z0-9_-]{1,120}$/.test(r.id));
  const groups: PublicFeedback[][] = [];
  for (let i = 0; i < pending.length; i += 30) groups.push(pending.slice(i, i + 30));
  const histories = await Promise.all(groups.map(group => rest<Array<IssueEvent & {feedback_id:string; dataset:string}>>(
  ...
  return pending.flatMap(report => {
    ...
    const phase = kind === "investigation_started" ? "investigating" : kind && kind !== "ingested" ? "enriching" : "ingested";
    return [{id:report.id,title:report.title,dataset:report.dataset,phase} as PendingReport];
  });
}
