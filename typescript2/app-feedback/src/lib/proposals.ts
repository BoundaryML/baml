...
export function proposalFromEvents(events: IssueEvent[]) {
  ...
  const slackUrl =
    ...
    typeof ts === 'string' &&
    /^[0-9]+\.[0-9]+$/.test(ts)
      ? `${new URL(base).origin}/archives/${channel}/p${ts.replace('.', '')}`
      : base;
  return {
    fix: created.payload.proposed_fix,
    head:
      typeof created.payload.head === 'string' ? created.payload.head : null,
    pushing: events.some((e) => e.kind === 'babysit_pushing'),
    slackUrl,
    started: events.some((e) =>
      ['babysit_approved', 'babysit_fix_started'].includes(e.kind),
    ),
  };
}
