...
export function decisionOf(
  ...
  const step = text(event.payload.step);
  ...
  const evidence =
    raw && typeof raw === 'object' && !Array.isArray(raw)
      ...
      : {};
  return { decision, evidence, reason, step };
}
...
export function activityText(
  event: Pick<IssueEvent, 'kind' | 'payload'>,
): string {
  switch (event.kind) {
    case 'ingested':
      return (
    ...
    case 'enrich_started':
      return 'Enrichment started: checking the report and building a repro.';
    case 'decision':
      return typeof event.payload.step === 'string' && event.payload.step.trim()
        ? event.payload.step.trim()
        : 'Pipeline decision.';
    case 'repro_verified':
      return 'Repro verified on the latest nightly.';
    case 'behavior_review':
      return 'Reviewed whether the behavior is a defect.';
    case 'no_issue': {
      const reasons: Record<string, string> = {
        by_design: 'No issue opened: the behavior is by design.',
        'fixed on this toolchain':
          'No issue opened: already fixed on the latest nightly.',
        needs_details: 'No issue opened: the report needs more detail.',
        uncertain:
          'No issue opened yet: whether this is a defect needs a human decision.',
        unconfirmed_repro:
          'No issue opened: the repro could not be confirmed on the latest nightly.',
      };
      return (
        reasons[String(event.payload.reason)] ??
        'No issue opened from this report.'
      );
    }
    case 'enriched':
      return 'Enrichment completed: repro and observed behavior recorded.';
    case 'organized':
      return 'Issue organized and assigned.';
    case 'gauged':
      return 'Difficulty assessed.';
    case 'investigation_started':
      return 'Investigating the code path behind the repro.';
    case 'investigation_failed':
      return 'Investigation ended without a confirmed finding; inspect the agent transcript.';
    case 'investigated':
      return (
        'Located the defect in the code' +
        (typeof event.payload.decision === 'string' &&
        event.payload.decision.trim()
          ? `: ${event.payload.decision.trim()}.`
          : '.')
      );
    case 'comment_synced':
      return (
        'New comment on the GitHub issue' +
        (typeof event.payload.author === 'string' &&
        /^[A-Za-z0-9-]{1,39}$/.test(event.payload.author)
          ? ` from @${event.payload.author}.`
          : '.')
      );
    case 'intuition':
      return 'Intuition: this issue is part of a pattern across issues.';
    case 'linked':
      return 'Linked to the other issue filed from this report.';
    case 'slack_notified':
      return 'Notified you on Slack.';
    case 'babysit_fix_started':
      return 'Fixing CI and CodeRabbit feedback automatically.';
    case 'issue_created':
      return 'Issue created.';
    case 'issue_shipped':
      return 'Shipped: every repro passes on the latest nightly.';
    case 'cancelled':
      return 'Shepherd cancelled the issue: not an issue.';
    case 'approved':
      return 'Shepherd approved the issue.';
    case 'design_started':
      return 'Planning the fix.';
    case 'implementation_started':
      return 'Implementing and testing the fix.';
    case 'fix_started':
    ...
    case 'babysit_proposed':
      return 'CI or reviewer feedback needs a fix; the agent is handling it automatically.';
    case 'babysit_proposal_posted':
      return 'Fix details posted to Slack.';
    case 'babysit_approved':
      return 'Implementing the fix.';
    case 'babysit_pushing':
      return 'Fix is being scanned and pushed; CI will validate it.';
    case 'babysit_round':
    ...
    case 'babysit_result': {
      ...
      if (
        event.payload.result === 'green' &&
        typeof event.payload.workflow_changes_needed === 'string'
      )
        return `Green, with workflow changes needed: ${event.payload.workflow_changes_needed}`;
      return (
        results[String(event.payload.result)] ?? 'Babysitter status updated.'
      );
    }
    default:
      return `${humanizeKind(event.kind)} recorded.`;
  }
}
export function eventProposalPath(
  event: Pick<IssueEvent, 'payload'>,
): string | null {
  const id = event.payload.proposal_id;
  return typeof id === 'string' && validProposalId(id)
    ? `/proposals/${id}`
    : null;
}
