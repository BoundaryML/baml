...
import { cookieOptions, maintainer, setSession, siteOrigin, STATE_COOKIE, unseal } from "@/lib/approval-auth";
...
export async function GET(request: NextRequest) {
  ...
  if (!stored || !code || stored.state !== request.nextUrl.searchParams.get("state")
    ...
    || !/^\/proposals\/[a-zA-Z0-9-]{1,80}$/.test(stored.returnTo)) {
  ...
  try {
    ...
    if (!response.ok || typeof result.access_token !== "string" || !await maintainer(result.access_token)) {
      return new NextResponse("Repository maintainer access is required.", { status: 403 });
    }
