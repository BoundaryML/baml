    ...
    <main className="max-w-3xl mx-auto p-8 space-y-6">
      ...
      <header className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          {dataset} ·{' '}
          {proposal?.pushing
            ? 'Fix pushed; waiting for CI'
            : proposal?.started
              ? 'Implementation started'
              : 'Automatic fix investigation'}
        </p>
        {proposal?.head && (
          <p className="text-xs text-muted-foreground break-all">
            Proposed for commit {proposal.head}
          </p>
        )}
      </header>
      {proposal ? (
        <div className="rounded-lg border p-5">
          <FormattedText text={proposal.fix} />
        </div>
      ) : (
        <p>
          This older proposal does not have a published fix summary. Review it
          in Slack.
        </p>
      )}
      <section className="space-y-2">
        ...
        <p>
          The agent implements and pushes fixes automatically, then checks CI
          and CodeRabbit again. Slack carries progress notifications; humans
          decide when to merge.
        </p>
        {proposal?.slackUrl ? (
          <a className="underline" href={proposal.slackUrl}>
            Open proposal in Slack
          </a>
        ) : (
