...
export async function GET(request: Request) {
  ...
  if ((id && !/^(?:[0-9a-f]{8}-(?:[0-9a-f]{4}-){3}[0-9a-f]{12}|[0-9a-f]{24}-(?:reproduce|investigate|ticket|deduplicate|fix))$/.test(id)) || !Number.isSafeInteger(offset) || offset < 0 || (issue && !/^(?:ISSUE|GH)-[A-Za-z0-9_-]+$/.test(issue)) || !["live", "eval"].includes(dataset)) return Response.json({ error: "Invalid transcript request" }, { status: 400 });
