...
export async function POST(
  ...
  if (!/^(?:ISSUE|GH)-[A-Za-z0-9_-]{1,120}$/.test(id))
  ...
  try {
    ...
    if (
      !['comment', 'export'].includes(input.operation) ||
      !['live', 'eval'].includes(input.dataset)
    )
      throw new Error('Invalid request.');
    if (
      input.operation === 'comment' &&
      (typeof input.body !== 'string' ||
        !input.body.trim() ||
        input.body.length > 5000)
    )
      throw new Error('Comment must contain 1–5000 characters.');
    const result = await runnerRequest(input.operation, {
      body: input.body,
