...
import { loadIssueEvents, loadIssue, loadIssueIntuitions, loadSiblingIssues } from "@/lib/db";
...
export default async function IssuePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [events, intuitions, siblings] = await Promise.all([loadIssueEvents(id, issue.dataset, issue.feedback_ids), loadIssueIntuitions(id, issue.dataset), loadSiblingIssues(issue)]);
  ...
  return <><LiveUpdates /><IssueDetail issue={issue} siblings={siblings} intuitions={intuitions} />
      <section className="mx-auto min-w-0 max-w-[1400px] px-4 py-6"><Activity events={events} dataset={issue.dataset} issueId={issue.id} /></section><section className="mx-auto max-w-[1400px] space-y-3 px-4 py-6"><h2 className="font-semibold">Agent sessions</h2><LiveAgents issueId={issue.id} /></section>
