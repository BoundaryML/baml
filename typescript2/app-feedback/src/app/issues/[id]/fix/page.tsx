...
/** The fix phase's artifacts, off the issue page: the run, the gate, the design doc. */
export default async function FixPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="mx-auto max-w-4xl space-y-8 px-4 py-6">
      ...
      {o?.gate && (
        <section>
          <h2 className="mb-2 text-xs uppercase tracking-wide text-muted-foreground">Gate · crates: {o.gate.changed_crates.join(", ") || "none"}</h2>
          <ul className="divide-y rounded-md border text-xs">
            {o.gate.steps.map((s) => (
              <li key={s.name} className="flex items-center gap-2 px-2 py-1">
                <span className={cn("h-1.5 w-1.5 shrink-0 rounded-full", s.ok ? "bg-stage-done" : "bg-stage-failed")} />
                <span className="truncate font-mono">{s.name}</span>
                <span className="ml-auto tabular-nums text-muted-foreground">{formatSeconds(s.seconds)}</span>
