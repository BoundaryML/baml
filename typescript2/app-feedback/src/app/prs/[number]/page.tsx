...
export default async function PrPage({
  ...
  return (
    <main className="max-w-4xl mx-auto p-8 space-y-6">
      <LiveUpdates />
      <div>
        <h1 className="text-2xl font-semibold">Babysitter · PR #{number}</h1>
        <a
          className="underline"
          href={`https://github.com/BoundaryML/baml/pull/${number}`}
        >
          View PR on GitHub
        </a>
        <p className="text-sm text-muted-foreground">
          {dataset} · Updates every 30 seconds. Fixes run automatically. Follow
