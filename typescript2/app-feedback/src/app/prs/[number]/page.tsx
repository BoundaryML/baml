...
export default async function PrPage({ params, searchParams }: {
  ...
  return <main className="max-w-4xl mx-auto p-8 space-y-6"><LiveUpdates />
    <div><h1 className="text-2xl font-semibold">Babysitter · PR #{number}</h1>
      ...
      <p className="text-sm text-muted-foreground">{dataset} · Updates every 30 seconds. Fixes run automatically. Follow the investigation and updates here or in the Slack thread.</p>
    </div><LiveAgents pr={`https://github.com/BoundaryML/baml/pull/${number}`} /><Activity events={events} dataset={dataset} />
  </main>;
}
