...
export default async function FeedbackPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!report) notFound();
  const [issues, noIssue] = await Promise.all([loadIssuesById(report.issue_ids), report.issue_ids.length ? Promise.resolve(null) : loadNoIssueReason(id, report.dataset)]);
  const github = report.body.match(GITHUB_ISSUE)?.[0] ?? null;
  return (
    <main className="mx-auto max-w-4xl space-y-6 px-4 py-6">
      <Link href={issues[0] ? `/issues/${issues[0].id}` : "/feedback"} className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
        <ArrowLeft className="h-4 w-4" /> {issues[0] ? "Back to the issue" : "All reports"}
      </Link>
      ...
      <section>
        ...
        {issues.length === 0 ? <p className="text-sm text-muted-foreground">{noIssue ? <>No issue: {noIssue}.</> : "No issue yet: this report is untriaged, or was held for a human."}</p> :
