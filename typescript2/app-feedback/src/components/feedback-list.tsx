...
export function fateOf(
  report: Pick<FeedbackListing, 'issues' | 'no_issue'>,
): Fate {
...
export function FeedbackList({ reports }: { reports: FeedbackListing[] }) {
  ...
  const counts = reports.reduce(
    ...
    { issue: 0, no_issue: 0, pending: 0 } as Record<Fate, number>,
  );
  return (
    <div className="space-y-4">
      <p className="text-xs text-muted-foreground" data-testid="fate-counts">
        {reports.length} reports · {counts.issue} {FATE_LABEL.issue} ·{' '}
        {counts.no_issue} {FATE_LABEL.no_issue} · {counts.pending}{' '}
        {FATE_LABEL.pending}
      </p>
          ...
          >
            <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
              ...
              {r.toolchain && (
                <>
                  <span>·</span>
                  <span>toolchain {r.toolchain}</span>
                </>
              )}
              {r.dataset === 'eval' && (
            ...
            <h2 className="text-base font-semibold leading-snug break-words">
              <Link
                ...
                href={`/feedback/${encodeURIComponent(r.id)}`}
              >
                {r.title}
              </Link>
            </h2>
            ...
            {r.issues.length > 0 ? (
              ...
              <p className="text-sm">
                {r.no_issue ? (
                  <>
                    <span className="font-medium">No issue:</span>{' '}
                    <span className="text-muted-foreground">{r.no_issue}</span>
                  </>
                ) : (
