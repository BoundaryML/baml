...
type Phase = "report" | "triage" | "organize" | "fix" | "verify" | "intuition" | "comments" | "other";
...
const FIX_KINDS = new Set(["issue_created", "approved", "fix_started", "pr_opened", "needs_human", "fixed_dry_run", "merged", "babysit_started", "babysit_proposed", "babysit_proposal_posted", "babysit_approved", "babysit_pushing", "babysit_round", "babysit_result", "babysit_fix_started", "cancelled", "issue_shipped", "feedback_linked"]);
...
function DecisionRow({ event, decision, took }: { event: IssueEvent; decision: Decision; took: string | null }) {
  return <Row tone={toneOf(event, decision)} time={event.created_at}
    title={<><span className="text-muted-foreground">{decision.step}: </span>{decision.decision}</>}
    trailing={took && <span className="text-[11px] text-muted-foreground">took {took}</span>}>
    <div className="mt-0.5 text-sm text-muted-foreground [&_p]:my-1 [&_p]:leading-6"><FormattedText text={decision.reason} /></div>
...
function PlainRow({ event, dataset, issueId }: { event: IssueEvent; dataset: "live" | "eval"; issueId?: string }) {
  ...
  return <Row tone={toneOf(event, null)} time={event.created_at} title={activityText(event)} trailing={links}>
    {summary && event.kind !== "no_issue" && <div className="mt-0.5 text-sm text-muted-foreground [&_p]:my-1"><FormattedText text={summary} /></div>}
    {event.kind === "no_issue" && summary && <p className="mt-0.5 text-sm text-muted-foreground">{summary}</p>}
  </Row>;
}
...
export function Activity({ events, dataset = "live", issueId, title = "Decision trail" }: { events: IssueEvent[]; dataset?: "live" | "eval"; issueId?: string; title?: string }) {
  ...
  const isTriage = (e: IssueEvent) => { const p = phaseOf(e, decisionOf(e)); return p === "triage" || p === "report" && e.kind !== "ingested"; };
  ...
  const shown = latest.filter((e) => !NOISE.has(e.kind) || decisionOf(e));
  ...
  return <section className="min-w-0">
    <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
      ...
      {shown.length > 0 && <p className="text-xs text-muted-foreground">
        ...
        {earlierAttempts > 0 && ` · ${earlierAttempts} earlier triage attempt${earlierAttempts === 1 ? "" : "s"} hidden (aborted before the issue was written; the PR is built from this one)`}
        {earlierFixes > 0 && ` · ${earlierFixes} earlier fix attempt${earlierFixes === 1 ? "" : "s"} hidden (stopped without a result)`}
      </p>}
    </div>
