...
const GITHUB_PR =
  /^https:\/\/github\.com\/BoundaryML\/baml\/pull\/[1-9][0-9]*$/;
...
const TRIAGE_STEPS = new Set([
  ...
  'Build minimal repro',
  'Repair the repro',
  'Verify repro on the latest nightly',
  'Is this a defect?',
  ...
  'Split report: bug',
  'Split report: feature request',
  'Linked to the other half of the report',
]);
...
function phaseOf(event: IssueEvent, decision: Decision | null): Phase {
  ...
  if (
    ['investigation_started', 'investigated', 'investigation_failed'].includes(
      event.kind,
    )
  )
    return 'investigate';
...
function toneOf(event: IssueEvent, decision: Decision | null): Tone {
  ...
  if (
    ...
    return 'warn';
  if (
    /^not actionable|^no |uncertain|still inconclusive|could not confirm|human/.test(
      d,
    )
  )
    return 'warn';
  ...
  if (
    ...
    event.kind === 'merged' ||
    event.kind === 'issue_shipped'
  )
    return 'ok';
  if (event.kind === 'babysit_round' && event.payload.result !== 'fixed')
    return 'bad';
  return 'info';
}
            ...
            <div className="contents" key={key}>
              ...
              <dd className="min-w-0 break-words">
                {scalar && !long ? (
                  ...
                  <pre className="max-h-56 overflow-auto whitespace-pre-wrap font-mono">
                    {long ? String(value) : JSON.stringify(value, null, 1)}
                  </pre>
                )}
              </dd>
            </div>
...
function Row({
  ...
  return (
    <li className="relative grid min-w-0 grid-cols-[3.25rem_1rem_minmax(0,1fr)] gap-x-2 py-2">
      <time
...
function DecisionRow({
  ...
  const questions = Array.isArray(decision.evidence.unknowns)
    ? decision.evidence.unknowns.filter(
    ...
    <Row
      ...
      trailing={
        <>
          ...
          {took && (
            <span className="text-[11px] text-muted-foreground">{took}</span>
          )}
        </>
      }
    >
        ...
        {gauge ? (
          questions.length > 0 && (
            <div className="mt-3 rounded-md bg-muted/40 p-3">
              <h4 className="text-xs font-medium">Open questions</h4>
...
function PlainRow({
  ...
  const author =
    typeof event.payload.author === 'string' && LOGIN.test(event.payload.author)
      ? event.payload.author
      : null;
  const commentUrl =
    typeof event.payload.url === 'string' &&
    GITHUB_COMMENT.test(event.payload.url)
      ? event.payload.url
      : null;
  const proposal = eventProposalPath(event);
  const pr =
    typeof event.payload.pr === 'string' && GITHUB_PR.test(event.payload.pr)
      ? event.payload.pr
      : null;
  const summary =
  ...
  const links = (
    <>
      {pr && (
        <a className="text-xs underline" href={pr}>
          View fix PR
        </a>
      )}
      {issueId && ['fix_started', 'pr_opened'].includes(event.kind) && (
        <Link
          className="text-xs underline"
          href={`/agents?issue_id=${encodeURIComponent(issueId)}`}
        >
          Agent transcript
        </Link>
      )}
      {event.kind === 'intuition' && (
        <Link className="text-xs underline" href="/intuition">
          All intuitions
        </Link>
      )}
      {proposal && (
  ...
  if (event.kind === 'comment_synced') {
    return (
      <Row
        time={event.created_at}
        title={
          author ? `@${author} commented on GitHub` : 'New comment on GitHub'
        }
        tone="info"
        trailing={links}
      >
        {summary && (
          <blockquote className="mt-0.5 border-l-2 pl-3 text-sm text-muted-foreground">
            {summary}
          </blockquote>
        )}
      </Row>
  ...
  return (
    <Row
      time={event.created_at}
      title={activityText(event)}
      tone={toneOf(event, null)}
      trailing={links}
    >
      {summary && (
        <details className="mt-1 text-sm">
...
export function Activity({
  ...
  const starts = ordered
    .map((e, i) =>
      e.kind === 'enrich_started' || decisionOf(e)?.step === 'Assess report'
        ? i
        : -1,
    )
    .filter((i) => i >= 0);
  const attemptStarts = starts.filter(
    (i, k) => k === 0 || i - starts[k - 1] > 1,
  ); // enrich_started immediately followed by Assess counts once
  const lastStart = attemptStarts.length > 1 ? (attemptStarts.at(-1) ?? 0) : 0;
  const earlierAttempts =
    attemptStarts.length > 1 ? attemptStarts.length - 1 : 0;
  const isTriage = (e: IssueEvent) => {
    const p = phaseOf(e, decisionOf(e));
    return (
      p === 'triage' ||
      p === 'investigate' ||
      (p === 'report' && e.kind !== 'ingested')
    );
  };
  ...
  const fixStarts = afterTriage
    .map((e, i) => (e.kind === 'fix_started' ? i : -1))
    .filter((i) => i >= 0);
  const lastFix = fixStarts.length > 1 ? (fixStarts.at(-1) ?? -1) : -1;
  const earlierFixes = fixStarts.length > 1 ? fixStarts.length - 1 : 0;
  const isFix = (e: IssueEvent) => phaseOf(e, decisionOf(e)) === 'fix';
  const latest =
    lastFix < 0
      ? afterTriage
      : afterTriage.filter((e, i) => i >= lastFix || !isFix(e));
  const shown = latest.filter(
    (e) => e.kind !== 'intuition' && (!NOISE.has(e.kind) || decisionOf(e)),
  );
  ...
  for (const e of shown) {
    ...
    else groups.push({ items: [e], phase });
  }
  const first = shown[0]?.created_at;
  const last = shown.at(-1)?.created_at;
      ...
      <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
        ...
        {shown.length > 0 && (
          <p className="text-xs text-muted-foreground">
            ...
            {total ? ` over ${total}` : ''} · times UTC
            {hidden > 0 &&
              ` · ${hidden} routine notification${hidden === 1 ? '' : 's'} folded`}
            {earlierAttempts > 0 &&
              ` · ${earlierAttempts} earlier triage attempt${earlierAttempts === 1 ? '' : 's'} hidden`}
            {earlierFixes > 0 &&
              ` · ${earlierFixes} earlier fix attempt${earlierFixes === 1 ? '' : 's'} hidden`}
          </p>
        )}
      </div>
      {shown.length === 0 ? (
              ...
              <div className={cn(gi > 0 && 'border-t')} key={g.items[0].id}>
                ...
                <ol className="relative px-3 pb-2 before:absolute before:bottom-3 before:left-[calc(3.25rem+0.75rem+0.4375rem)] before:top-2 before:w-px before:bg-border">
                  {g.items.map((event, i) => {
                    ...
                    const took =
                      decision && prev
                        ? span(prev.created_at, event.created_at)
                        : null;
                    return decision ? (
                      <DecisionRow
                        decision={decision}
