  ...
  return (
    <div className={cn("flex items-center gap-1", className)} aria-label="pipeline progress">
      {stages.map((s) => (
        ...
        >
          <span className="sr-only">{`${STAGE_LABELS[s.stage]}: ${s.state}, ${s.detail}`}</span>
        </div>
      ))}
    </div>
  );
...
export function PipelineStripLabeled({ stages }: { stages: StageInfo[] }) {
  return (
    <div className="grid grid-cols-7 gap-1">
      {stages.map((s) => (
        <div key={s.stage} className="min-w-0" title={`${STAGE_LABELS[s.stage]}: ${s.detail}`}>
          <div className={cn("h-2 rounded-sm", STATE_CLASS[s.state])} />
          <div className="mt-1 text-[11px] leading-tight text-muted-foreground truncate" aria-hidden="true">
