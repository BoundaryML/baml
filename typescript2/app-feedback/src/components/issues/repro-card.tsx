...
function expectation(repro: Repro): string {
  switch (repro.expectation.check) {
    case 'should_compile':
      return 'Expected: source compiles';
    case 'should_not_compile':
      return 'Expected: source is rejected';
    case 'should_evaluate_to':
      return `Expected: ${JSON.stringify(repro.expectation.expected)}`;
    case 'requires_inspection':
      return repro.verification === 'python_unittest'
        ? 'Expected: executable assertions pass'
...
export function ReproCard({
  ...
  const source =
    repro.source_files && Object.keys(repro.source_files).length > 0
      ? repro.source_files
      : null;
  const result =
    repro.result === 'passes'
      ...
      : repro.result === 'fails'
        ? 'Fails'
        : repro.result === 'invalid_repro'
          ? 'Invalid reproduction'
          : repro.result === 'unsupported_verification'
            ? 'Verification unavailable'
            : repro.result === 'verification_error'
              ? 'Verification failed to run'
              : repro.result === 'inconclusive'
                ? 'Inconclusive'
    ...
    <div className="overflow-hidden rounded-md border">
      <div className="flex flex-wrap items-center justify-between gap-2 bg-muted/60 px-3 py-2 text-xs">
        <span className="font-medium">
          {comparison ? 'Comparison' : 'Reproduction'} · {result}
        </span>
        <span>{expectation(repro)}</span>
      </div>
      <div className="px-3 py-2 text-xs text-muted-foreground">
        <span className="font-mono">
          $ {source ? 'baml check' : repro.command}
        </span>
        {repro.verified_version && (
          <span> · BAML {repro.verified_version}</span>
        )}
      </div>
      ...
      {source && (
        <details className="border-t">
          <summary className="cursor-pointer px-3 py-2 text-sm">
            Full regression test and result
          </summary>
          <div className="px-3 py-2 font-mono text-xs">$ {repro.command}</div>
          <Files files={repro.files} />
          <Output
            output={repro.observed}
            title="Regression test output (stdout / stderr)"
          />
        </details>
      )}
    </div>
