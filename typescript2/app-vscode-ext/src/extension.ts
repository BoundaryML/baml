...
function activeClient(): LanguageClient | undefined {
  ...
  const ownerKey = ownershipCoordinator
    ? ownershipCoordinator.ownerKeyForUri(uri)
    : resolveOwnershipRoot(uri.fsPath, 'file')?.key;
  return ownerKey === undefined ? undefined : clients.get(ownerKey);
}

function projectKeyForPath(projectPath: string): string | undefined {
  try {
    return resolveOwnershipRoot(projectPath, 'auto')?.key;
  } catch {
    return undefined;
  }
}
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('baml.openPlayground', async (projectPath?: string) => {
      ...
      const client = projectPath
        ? (projectOwnerKey === undefined ? undefined : clients.get(projectOwnerKey))
        : activeClient();
