    ...
    (async () => {
      ...
      // root and would reject creating the chain that leads to it.
      {
        for (const directoryUri of workspacePaths.rootAncestorUris()) {
          try { await rawFs.mkdir(directoryUri); } catch { /* already exists */ }
        }
      }
      ...
      for (const [filename, content] of Object.entries(filesRef.current)) {
        ...
        if (normalizedFilename in allFiles) {
          throw new Error(`Duplicate workspace filename after normalization: ${filename}`);
        }
      ...
      for (const [filename, content] of Object.entries(allFiles)) {
        ...
        for (const directoryUri of workspacePaths.parentDirectoryUris(filename)) {
          try {
        ...
        if (isMediaPath(filename)) {
          ...
          await fileSystemProvider.writeFile(fileUri, encoder.encode(content), writeOpts);
        }
      }
      ...
      const cursorSubscription = vscode.window.onDidChangeTextEditorSelection((e: import('vscode').TextEditorSelectionChangeEvent) => {
        const filename = workspacePaths.relativeFilename(e.textEditor.document.uri);
