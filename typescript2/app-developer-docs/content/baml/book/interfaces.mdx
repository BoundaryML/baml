...
For `throws` and error effects, see [Handle errors](/baml/book/errors). When replacing a definition, remove the old one before adding the new one. Run the advanced examples separately unless they reuse an earlier contract.
...
**Explicit membership.** Matching fields and methods don't confer interface membership. This class has the right field but never implements `Destination`:
...
The compiler reports these errors for invalid interface declarations and calls:
