...
Errors are ordinary values. The compiler tracks which errors a function can throw, including errors from its calls.
...
These examples use [Effect 3](https://effect.website/docs/v3/error-management/expected-errors). Install it with `npm install effect@3`.
...
Add the BAML snippets to the same project. Keep only one declaration of `BadToolInput` when adding the lambda example. Run the final `throws never` failure example separately.
...
Use `throws never` when no error value can leave a function. The compiler rejects a body that can throw.
...
This function promises `throws never` but calls a function that can throw `ToolUnavailable`:
