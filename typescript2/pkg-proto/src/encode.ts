...
function serializeValue(val: unknown): InboundValue {
  ...
  if (typeof val === 'string') {
    return {
      value: { $case: 'stringValue', stringValue: val },
      valueType: undefined,
    };
  }
  if (typeof val === 'number') {
    ...
    if (Number.isInteger(val)) {
      ...
      return {
        value: { $case: 'intValue', intValue: val },
        valueType: undefined,
      };
    }
    return {
      value: { $case: 'floatValue', floatValue: val },
      valueType: undefined,
    };
  }
  if (typeof val === 'bigint') {
    ...
    return {
      value: { $case: 'bigintValue', bigintValue: val.toString(16) },
      valueType: undefined,
    };
  }
  if (typeof val === 'boolean') {
    return {
      value: { $case: 'boolValue', boolValue: val },
      valueType: undefined,
    };
  }
  ...
  if (typeof val === 'object') {
    ...
    if (bamlMarker && typeof bamlMarker === 'object' && 'enum' in bamlMarker) {
      ...
      if (typeof marker.enum !== 'string' || typeof marker.value !== 'string') {
        throw new Error(
    ...
    if (
      ...
      const className = sepIdx >= 0 ? typeStr.slice(sepIdx + 2) : typeStr;
    ...
    const entries: InboundMapEntry[] = Object.entries(val).map(([k, v]) => ({
      key: { $case: 'stringKey' as const, stringKey: k },
      value: serializeValue(v),
    }));
    return {
  ...
  throw new Error(`Cannot serialize value of type ${typeof val} to BAML`);
}
...
export function encodeCallArgs(
  ...
  const entries: InboundMapEntry[] = Object.entries(kwargs).map(([k, v]) => ({
    key: { $case: 'stringKey' as const, stringKey: k },
    value: serializeValue(v),
  }));
