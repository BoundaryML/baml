...
describe('applyLevelOfDetail', () => {
  ...
  it('reveals only the top level and collapses deeper containers', () => {
    const { nodes } = applyLevelOfDetail(NODES, EDGES, 1);
  ...
  it('reroutes edges to the nearest visible ancestor and drops internal ones', () => {
    const { edges } = applyLevelOfDetail(NODES, EDGES, 1);
  ...
  it('expands more as revealDepth grows', () => {
    expect(applyLevelOfDetail(NODES, EDGES, 2).nodes.map((x) => x.id)).toContain(
      'call',
    );
    expect(applyLevelOfDetail(NODES, EDGES, 99).nodes).toHaveLength(NODES.length);
  });

  it('is a no-op at infinite depth', () => {
    const out = applyLevelOfDetail(NODES, EDGES, Number.POSITIVE_INFINITY);
