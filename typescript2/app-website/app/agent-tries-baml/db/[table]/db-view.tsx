...
function reportsCell(i: Issue) {
  ...
  return ev.map((e, idx) => (
    <Link
      key={idx}
      href={`/agent-tries-baml/runs/${e.trophyId}${e.call_index != null ? `?call=${e.call_index}` : ''}`}
      style={{ marginRight: 8 }}
    >
  ...
  const header = (count: number) => (
    <header className="page">
      <p style={{ marginBottom: 6 }}>
        <Link href="/agent-tries-baml" className="back-link">
          ← graph
        </Link>
      </p>
      <h1>
  ...
  if (table === 'tasks') {
    ...
    rows = data.map((t) => (
      <tr key={t._id} className="runrow">
        ...
        <td>
          {t.reportId ? (
            <Link href={`/agent-tries-baml/runs/${t.reportId}`}>trophy →</Link>
          ) : (
    ...
    rows = s.runs.map((r) => (
      <tr key={r.trophyId} className="runrow">
        ...
        <td>
          <Link href={`/agent-tries-baml/runs/${r.trophyId}`}>{r.prompt}</Link>
        </td>
