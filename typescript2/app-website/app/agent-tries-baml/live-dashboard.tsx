...
export function usePolledState(initial: LiveState) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(async () => {
      try {
        const r = await fetch('/agent-tries-baml/api/state', {
          cache: 'no-store',
        });
...
function DbNode({
  ...
  return (
    <Link href={`/agent-tries-baml/db/${table}`} className="dbnode">
      <div className="dbnode-name">{name}</div>
      <div className="dbnode-num">{cnt(breakdown)}</div>
      <div className="dbnode-rows">
        {parts.length === 0 ? (
          <span className="mute">empty</span>
        ) : (
          parts.map(([k, v]) => (
            <span key={k} className="chip">
              <span className={`dot dot-${TONE[k] ?? 'mute'}`} />
              {v}&nbsp;{k}
            </span>
          ))
        )}
  ...
  return (
    <div>
      <header className="page">
        ...
        <p className="blurb">
          Agents write BAML against canary; each run becomes a trophy, findings
          dedupe into issues.
        </p>
        <p className="mute" style={{ fontSize: 13 }}>
          ${s.totals.costUsd.toFixed(2)} est · canary{' '}
          {readyBuild ? readyBuild.sha.slice(0, 8) : '—'} ·{' '}
        ...
        <div className="flow-col">
          ...
          {inflightTasks.length === 0 ? (
            ...
            inflightTasks.slice(0, 4).map((f) => (
              <div
                key={f.id}
                className={`mini mini-${STAGE[f.stage] ?? 'warn'}`}
              >
                <span className="dot" />
                {f.stage}
