...
function buildSphinx(count: number): ShapePoint[] {
  const raw: ShapePoint[] = [
    ...
    ...sampleCurve(seg(0.07, 0.87, 0.07, 0.7), 10),
    ...sampleCurve(seg(0.07, 0.7, 0.2, 0.7), 8),
    ...sampleCurve(seg(0.2, 0.7, 0.2, 0.87), 10),
    ...sampleCurve(seg(0.13, 0.7, 0.13, 0.82), 7), // divider between paws
    // Body top silhouette (gently arched)
    ...sampleCurve(quad(0.2, 0.67, 0.42, 0.52, 0.66, 0.6), 26),
    // Body bottom
    ...sampleCurve(quad(0.2, 0.8, 0.5, 0.8, 0.68, 0.82), 18),
    // Rear haunches
    ...sampleCurve(arc(0.74, 0.74, 0.09, 0.13, -PI * 0.65, PI * 0.35), 14),
    // Neck (two parallel lines)
    ...sampleCurve(seg(0.66, 0.6, 0.63, 0.4), 11),
    ...sampleCurve(seg(0.73, 0.6, 0.73, 0.4), 11),
    // Head: left side
    ...sampleCurve(seg(0.63, 0.4, 0.63, 0.15), 15),
    // Head: chin
    ...sampleCurve(seg(0.73, 0.4, 0.9, 0.4), 11),
    // Head: right side
    ...sampleCurve(seg(0.9, 0.4, 0.9, 0.22), 12),
    // Headdress: top edges
    ...sampleCurve(seg(0.9, 0.22, 0.76, 0.13), 9),
    ...
    ...sampleCurve(seg(0.85, 0.15, 0.9, 0.38), 9),
    // Face details
    ...sampleCurve(seg(0.71, 0.27, 0.82, 0.27), 7), // eye line
    ...sampleCurve(seg(0.74, 0.33, 0.8, 0.33), 5), // nose
  ];
...
function buildColosseum(count: number): ShapePoint[] {
  const raw: ShapePoint[] = [
    // Outer ellipse
    ...sampleCurve(arc(0.5, 0.5, 0.46, 0.43, 0, TAU), 60),
    // Second tier
    ...sampleCurve(arc(0.5, 0.5, 0.35, 0.33, 0, TAU), 46),
    // Third tier
    ...sampleCurve(arc(0.5, 0.5, 0.24, 0.22, 0, TAU), 32),
    // Ground
    ...
    ...sampleCurve(arc(0.5, 0.5, 0.46, 0.43, PI * 0.7, PI * 1.8), 20),
  ];
...
function buildCathedral(count: number): ShapePoint[] {
  ...
  const raw: ShapePoint[] = [
    // Left tower: walls
    ...sampleCurve(seg(0.22, 0.95, 0.22, 0.2), 24),
    ...sampleCurve(seg(0.36, 0.95, 0.36, 0.2), 24),
    // Left tower spire
    ...sampleCurve(seg(0.22, 0.2, 0.29, 0.04), 8),
    ...sampleCurve(seg(0.36, 0.2, 0.29, 0.04), 8),
    // Right tower: walls
    ...sampleCurve(seg(0.64, 0.95, 0.64, 0.2), 24),
    ...sampleCurve(seg(0.78, 0.95, 0.78, 0.2), 24),
    // Right tower spire
    ...sampleCurve(seg(0.64, 0.2, 0.71, 0.04), 8),
    ...sampleCurve(seg(0.78, 0.2, 0.71, 0.04), 8),
    // Tower tops (horizontal band)
    ...sampleCurve(seg(0.22, 0.2, 0.36, 0.2), 6),
    ...sampleCurve(seg(0.64, 0.2, 0.78, 0.2), 6),
    ...
    ...sampleCurve(quad(0.36, 0.42, 0.5, 0.26, 0.64, 0.42), 22),
    // Rose window
    ...sampleCurve(arc(0.5, 0.52, 0.1, 0.1, 0, TAU), 22),
    ...sampleCurve(arc(0.5, 0.52, 0.06, 0.06, 0, TAU), 14),
    // Three Gothic portals at base
    ...sampleCurve(arc(0.29, 0.88, 0.07, 0.055, PI, 0), 12),
    ...sampleCurve(arc(0.5, 0.88, 0.07, 0.055, PI, 0), 12),
    ...sampleCurve(arc(0.71, 0.88, 0.07, 0.055, PI, 0), 12),
    // Flying buttresses
    ...sampleCurve(seg(0.36, 0.65, 0.26, 0.8), 8),
    ...sampleCurve(seg(0.64, 0.65, 0.74, 0.8), 8),
    // Ground
    ...sampleCurve(seg(0.1, 0.95, 0.9, 0.95), 24),
  ];
...
function buildGoldenGate(count: number): ShapePoint[] {
  const raw: ShapePoint[] = [
    ...
    ...sampleCurve(seg(0.27, 0.9, 0.27, 0.1), 26),
    ...sampleCurve(seg(0.3, 0.9, 0.3, 0.1), 26),
    // Right tower
    ...sampleCurve(seg(0.7, 0.9, 0.7, 0.1), 26),
    ...sampleCurve(seg(0.73, 0.9, 0.73, 0.1), 26),
    // Tower caps
    ...sampleCurve(seg(0.24, 0.1, 0.33, 0.1), 6),
    ...sampleCurve(seg(0.67, 0.1, 0.76, 0.1), 6),
    // Tower cross-bars
    ...sampleCurve(seg(0.24, 0.28, 0.33, 0.28), 6),
    ...sampleCurve(seg(0.67, 0.28, 0.76, 0.28), 6),
    // Main cable: left anchor → left tower top
    ...sampleCurve(quad(0.02, 0.6, 0.14, 0.28, 0.285, 0.12), 22),
    // Main cable: left tower → right tower (the big sag)
    ...sampleCurve(quad(0.285, 0.12, 0.5, 0.72, 0.715, 0.12), 38),
    // Main cable: right tower → right anchor
    ...sampleCurve(quad(0.715, 0.12, 0.86, 0.28, 0.98, 0.6), 22),
    // Deck
    ...
    // Water
    ...sampleCurve(quad(0.0, 0.82, 0.5, 0.78, 1.0, 0.82), 34),
    ...sampleCurve(quad(0.0, 0.87, 0.5, 0.84, 1.0, 0.87), 28),
  ];
  return resample(scatterAround(raw, 0.005), count);
}
...
function buildBurjKhalifa(count: number): ShapePoint[] {
  ...
  const raw: ShapePoint[] = [
    // Spire
    ...sampleCurve(seg(0.48, 0.1, 0.5, 0.02), 5),
    ...sampleCurve(seg(0.52, 0.1, 0.5, 0.02), 5),
    // Tier 5 (top)
    ...sampleCurve(seg(0.44, 0.22, 0.44, 0.1), 8),
    ...sampleCurve(seg(0.56, 0.22, 0.56, 0.1), 8),
    ...sampleCurve(seg(0.44, 0.1, 0.56, 0.1), 6),
    // Step out
    ...sampleCurve(seg(0.44, 0.22, 0.4, 0.22), 3),
    ...sampleCurve(seg(0.56, 0.22, 0.6, 0.22), 3),
    // Tier 4
    ...sampleCurve(seg(0.4, 0.37, 0.4, 0.22), 9),
    ...sampleCurve(seg(0.6, 0.37, 0.6, 0.22), 9),
    ...sampleCurve(seg(0.4, 0.22, 0.6, 0.22), 8),
    // Step out
    ...sampleCurve(seg(0.4, 0.37, 0.36, 0.37), 3),
    ...sampleCurve(seg(0.6, 0.37, 0.64, 0.37), 3),
    // Tier 3
    ...
    // Step out
    ...sampleCurve(seg(0.33, 0.72, 0.3, 0.72), 2),
    ...sampleCurve(seg(0.67, 0.72, 0.7, 0.72), 2),
    // Tier 1 (base)
    ...sampleCurve(seg(0.3, 0.92, 0.3, 0.72), 14),
    ...sampleCurve(seg(0.7, 0.92, 0.7, 0.72), 14),
    ...sampleCurve(seg(0.3, 0.72, 0.7, 0.72), 14),
    // Ground
    ...sampleCurve(seg(0.1, 0.95, 0.9, 0.95), 22),
  ];
  return resample(scatterAround(raw, 0.005), count);
}
...
function buildSpaceElevator(count: number): ShapePoint[] {
  const raw: ShapePoint[] = [
    ...
    ...sampleCurve(arc(0.5, 1.18, 0.6, 0.28, PI * 1.08, PI * 1.92), 34),
    // Tether (thin double line, very vertical)
    ...sampleCurve(seg(0.488, 0.9, 0.488, 0.12), 35),
    ...sampleCurve(seg(0.512, 0.9, 0.512, 0.12), 35),
    // Orbital station ring (outer)
    ...sampleCurve(arc(0.5, 0.1, 0.16, 0.07, 0, TAU), 28),
    // Orbital station ring (inner)
    ...sampleCurve(arc(0.5, 0.1, 0.1, 0.045, 0, TAU), 18),
    ...
    // Cloud layer
    ...sampleCurve(quad(0.08, 0.78, 0.3, 0.73, 0.5, 0.75), 14),
    ...sampleCurve(quad(0.5, 0.75, 0.7, 0.73, 0.92, 0.78), 14),
    // Stars (scattered points in upper region)
    ...Array.from({ length: 22 }, (_, i) => {
      const a = (i / 22) * TAU + 0.3;
      const r = 0.14 + ((i * 7) % 5) * 0.055;
      return { x: 0.5 + Math.cos(a) * r * 2.2, y: 0.35 + Math.sin(a) * r };
    }),
  ];
  return resample(scatterAround(raw, 0.006), count);
}
