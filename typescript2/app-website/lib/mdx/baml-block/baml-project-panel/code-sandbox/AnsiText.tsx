...
const getLinks = (text: string) => {
  const txt = text.replace(
    /[^<>\s]+\.log\b/gm,
    (str) => `<a href="vscode://file/${str}">${str}</a>`,
  );
  const urlRegex =
    /(<span class="ansi-(?:[^"]+)">)(https?:\/\/[^\s<]+)(<\/span>)/g;
...
const AnsiText: React.FC<{ text: string; className: string }> = ({
  text,
  className,
}) => {
  ...
  return (
    <pre
      className={className}
      dangerouslySetInnerHTML={{ __html: getLinks(html) }}
    />
  );
};
