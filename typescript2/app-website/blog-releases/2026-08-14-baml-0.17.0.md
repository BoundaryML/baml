---
title: BAML 0.17.0
description: Runtime reflection, native BAML model clients, cryptography and filesystem APIs, toolchain pinning, and a rewritten type checker.
slug: baml-0.17.0
date: 2026-08-14
tags: ['Release']
---
...
### `openai-generic` request bodies

`openai.GenericClient` intentionally sends a text-only Chat Completions message as a JSON string. BAML 0.15 sent the same message as an array containing a text part. Messages that contain images or other media continue to use a part array. This changes the serialized request even when the rendered prompt text is otherwise identical. ([#4430](https://github.com/BoundaryML/baml/pull/4430), [#4585](https://github.com/BoundaryML/baml/issues/4585))
...
If a proxy, capture sink, test, or replay system inspects request JSON, update it to accept the string form for text-only messages and the part-array form for messages with media. Regenerate byte-for-byte request fixtures when upgrading; comparing the rendered text separately can distinguish this envelope migration from a prompt-content change.

### Template whitespace after role markers

Classic `#"..."#` prompt blocks and 0.17 backtick templates do not trim whitespace in exactly the same places. Backtick templates remove delimiter-only boundary line breaks and common indentation, but preserve authored whitespace inside the template. In particular, a newline immediately after `${role("user")}` becomes the first character of the user message. ([#4365](https://github.com/BoundaryML/baml/pull/4365), [#4585](https://github.com/BoundaryML/baml/issues/4585))

This mechanical rewrite adds a leading newline:
...
Place the first content on the same line as the role marker when the old prompt did not begin with a newline:

```baml
prompt: `${role("user")}Text: ${txt}`
```

Review rendered prompts and regenerate byte-for-byte prompt fixtures when migrating. Preserve or remove each newline explicitly instead of assuming the old block-string trimming behavior.
