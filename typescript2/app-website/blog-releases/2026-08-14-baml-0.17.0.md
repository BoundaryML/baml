...
- The rewritten type checker no longer accepts unary `!` when its operand has type `bool | null`. Earlier releases accepted expressions such as `!record?.flag`, but crashed at runtime when optional chaining produced `null`. In 0.17, handle the missing case explicitly—for example, use `!(record?.flag ?? true)` when the predicate means "the record exists and its flag is false." This restriction is specific to 0.17; BAML 0.18 introduces truthiness for `!` and other condition positions.
