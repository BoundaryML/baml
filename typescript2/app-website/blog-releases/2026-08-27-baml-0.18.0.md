...
- Allow truthiness conversions in `if`, `while`, match guards, `&&`, `||`, and `!`, and improve type narrowing in conditional branches. Falsy values are `false`, `null`, numeric zero, empty strings, empty arrays and maps, and empty bytes; all other values are truthy. This means `!value?.flag` is `true` both when `flag` is `false` and when optional chaining produces `null`. If those cases have different meanings, provide an explicit fallback: for example, `!(value?.flag ?? true)` is `false` when `value` is missing. ([#4498](https://github.com/BoundaryML/baml/pull/4498)) - Avery Townsend
