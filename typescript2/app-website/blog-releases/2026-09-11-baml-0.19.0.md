...
`baml agent install` installs the skill embedded in your toolchain without downloading it from GitHub. The installed skill records its toolchain version. Authoring commands in detected coding-agent sessions check that the installed skill matches the toolchain, preventing agents from relying on stale instructions after an upgrade. Human sessions retain warnings. ([#4625](https://github.com/BoundaryML/baml/pull/4625), [#4723](https://github.com/BoundaryML/baml/pull/4723))
