{
  ...
  outputs = { self, nixpkgs, nixpkgs-unstable, flake-utils, fenix, crane, ... }:
    ...
    flake-utils.lib.eachDefaultSystem (system:
      ...
      let
        ...
        appleDeps = pkgs.lib.optionals pkgs.stdenv.isDarwin (with pkgs.darwin; [
          libiconv
        ]);
        ...
        commonArgs = {
          inherit src version buildInputs nativeBuildInputs;
          strictDeps = true;
          ...
          RUSTFLAGS = if pkgs.stdenv.isDarwin
            then "--cfg tracing_unstable"
            else "--cfg tracing_unstable -C target-feature=+crt-static";
          OPENSSL_STATIC = "1";
        ...
        bamlRustPackage = {
          ...
          let
            ...
            buildFn = if buildType == "release"
              then craneLib.buildPackage
          ...
          buildFn ({
              ...
              inherit (commonArgs) OPENSSL_STATIC OPENSSL_DIR OPENSSL_LIB_DIR OPENSSL_INCLUDE_DIR;
        ...
        rec {
          ...
          packages."baml-cli-musl" = if pkgs.stdenv.isDarwin
            then throw "musl builds are not supported on macOS - use the default package instead"
            else let
            ...
            in craneLib.buildPackage (muslCommonArgs // {
              pname = "baml-cli";
