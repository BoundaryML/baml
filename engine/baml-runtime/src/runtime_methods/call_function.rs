...
impl InternalBamlRuntime {
    pub(crate) async fn call_function_impl<'ir>(
        ...
        let future = async {
            let func = prepared_func_call.func.as_ref()
                .ok_or_else(|| anyhow::anyhow!("Cannot call expr function through call_function_impl"))?;
            let renderer =
                PromptRenderer::from_function(func, self.ir(), &ctx)?;
