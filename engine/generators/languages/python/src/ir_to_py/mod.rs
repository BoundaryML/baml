...
use crate::r#type::{EscapedPythonString, LiteralValue, MediaTypePy, TypeMetaPy, TypePy, TypeWrapper};
use baml_types::{
...
pub(crate) fn stream_type_to_py(field: &TypeStreaming, _lookup: &impl TypeLookups) -> TypePy {
    ...
    let type_py: TypePy = match field {
        ...
        T::List(type_generic, _) => {
            TypePy::List(Box::new(recursive_fn(type_generic)), meta)
        },
        T::Map(type_generic, type_generic1, _) => {
            TypePy::Map(Box::new(recursive_fn(type_generic)), Box::new(recursive_fn(type_generic1)), meta)
        },
        ...
        T::Tuple(..) => TypePy::Any { reason: "tuples are not supported in Py".to_string(), meta },
        T::Arrow(..) => TypePy::Any { reason: "arrow types are not supported in Py".to_string(), meta },
...
pub(crate) fn type_to_py(field: &Type, _lookup: &impl TypeLookups) -> TypePy {
    use Type as T;
    ...
    let type_py = match field {
        ...
        T::Literal(literal_value, _) => TypePy::Literal(vec![match literal_value {
            baml_types::LiteralValue::String(val) => LiteralValue::String(EscapedPythonString::new(val)),
        ...
        T::Union(union_type_generic, union_meta) => match union_type_generic.view() {
            baml_types::ir_type::UnionTypeViewGeneric::Null => TypePy::Literal(vec![
                LiteralValue::None,
            ], meta),
            baml_types::ir_type::UnionTypeViewGeneric::Optional(type_generic) => {
                let mut type_py = recursive_fn(type_generic);
