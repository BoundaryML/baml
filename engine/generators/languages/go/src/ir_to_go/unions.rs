...
) -> impl Iterator<Item = crate::generated_types::UnionGo<'a>> {
    let go_type = crate::ir_to_go::type_to_go(union, pkg.lookup());
    let result: std::vec::IntoIter<crate::generated_types::UnionGo<'a>> = go_type
        .flatten_unions()
        .into_iter()
        .filter_map(|go_type| {
...
pub fn ir_union_to_go_stream<'a>(
    ...
    let result: Vec<crate::generated_types::UnionGo<'a>> = go_type
        .flatten_unions()
        .into_iter()
        .filter_map(|go_type| {
            if let TypeGo::Union { name, .. } = go_type {
