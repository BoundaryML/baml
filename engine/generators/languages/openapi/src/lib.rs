...
mod tests {
    ...
    fn test_gitignore_and_openapi_generator_ignore_files() {
        ...
        let args = GeneratorArgs {
            ...
            default_client_mode: internal_baml_core::configuration::GeneratorDefaultClientMode::Sync,
            on_generate: vec![],
        ...
        // Generate SDK files
        lang_features.generate_sdk_files(&mut collector, std::sync::Arc::new(ir), &args).unwrap();
        ...
        let ignore_content = result.get(&PathBuf::from(".openapi-generator-ignore")).unwrap();
        assert_eq!(
            ignore_content.trim(), 
            ".gitignore", 
