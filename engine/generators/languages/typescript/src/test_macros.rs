/// Macros for testing TypeScript type serialization.
///
...
macro_rules! test_ts_type {
    ...
    (
        ...
        use $crate::ir_to_ts::classes::{ir_class_to_ts, ir_class_to_ts_stream};
        ...
        if parts.len() == 2 {
            ...
            let class_ts_stream = ir_class_to_ts_stream(class, &pkg);
            let field = class_ts_stream
                .fields
                .iter()
                .find(|f| f.name == field_name)
                .expect(&format!(
            ...
            use $crate::ir_to_ts::type_aliases::{ir_type_alias_to_ts, ir_type_alias_to_ts_stream};
            ...
            // Non-streaming
            pkg.set("baml_client.types");
            let alias_ts = ir_type_alias_to_ts(type_alias, &pkg);
            assert_eq!(
                alias_ts.target_type.serialize_type(&pkg),
                $expected_non_streaming,
            ...
            // Streaming
            pkg.set("baml_client.partial_types");
            let alias_ts_stream = ir_type_alias_to_ts_stream(type_alias, &pkg);
            assert_eq!(
                alias_ts_stream.target_type.serialize_type(&pkg),
                $expected_streaming,
    ...
    (
        ...
        use internal_baml_core::ir::{repr::make_test_ir, IRHelper};
        use $crate::ir_to_ts::enums::ir_enum_to_ts;
        use $crate::package::CurrentRenderPackage;
        ...
        let enum_ts = ir_enum_to_ts(enm);
        assert_eq!(enum_ts.name, $enum_name);

        let expected_values: Vec<&str> = vec![$( $value ),*];
        let actual_values: Vec<&str> = enum_ts.values.iter().map(|(v, _)| v.as_str()).collect();
        assert_eq!(
            actual_values,
            expected_values,
