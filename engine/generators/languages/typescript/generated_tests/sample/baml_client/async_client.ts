...
import type { Check, Checked, Example, Example2, RecursivePartialNull as MovedRecursivePartialNull } from './types'
...
export class BamlAsyncClient {
  ...
  async Bar(x: number, __baml_options__?: BamlCallOptions): Promise<Example | Example2> {
    try {
  ...
  async Foo(x: number, __baml_options__?: BamlCallOptions): Promise<Example2 | Example> {
    try {
      const options = { ...this.bamlOptions, ...(__baml_options__ || {}) }
      const collector = options.collector
        ? Array.isArray(options.collector)
