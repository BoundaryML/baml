...
import type { Check, Checked, RecursivePartialNull as MovedRecursivePartialNull, TestEnum } from './types'
...
export class BamlAsyncClient {
  ...
  async ConsumeTestEnum(input: types.TestEnum, __baml_options__?: BamlCallOptions): Promise<types.TestEnum> {
    try {
  ...
  async FnTestAliasedEnumOutput(input: string, __baml_options__?: BamlCallOptions): Promise<types.TestEnum> {
    try {
      const options = { ...this.bamlOptions, ...(__baml_options__ || {}) }
      const collector = options.collector
        ? Array.isArray(options.collector)
