...
export class BamlAsyncClient {
  ...
  async Foo(x: number, __baml_options__?: BamlCallOptions): Promise<types.JSON> {
    try {
  ...
  async JsonInput(x: types.JSON, __baml_options__?: BamlCallOptions): Promise<types.JSON> {
    try {
      const options = { ...this.bamlOptions, ...(__baml_options__ || {}) }
      const collector = options.collector
        ? Array.isArray(options.collector)
