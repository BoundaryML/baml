/// Macros for testing Rust type serialization.
///
/// These macros make it easy to write tests that verify BAML types
/// convert correctly to Rust streaming and non-streaming type strings.
///
/// Test Rust type serialization with auto-generated test names.
///
/// # Examples
///
/// ```ignore
...
macro_rules! test_rs_type {
    ...
    (
        ...
        use $crate::ir_to_rust::classes::{ir_class_to_rust, ir_class_to_rust_stream};
        ...
        if parts.len() == 2 {
            ...
            use $crate::ir_to_rust::type_aliases::{ir_type_alias_to_rust, ir_type_alias_to_rust_stream};
            ...
            // Non-streaming
            pkg.set("baml_client.types");
            let alias_rust = ir_type_alias_to_rust(type_alias, &pkg, None);
            assert_eq!(
                alias_rust.type_.serialize_type(&pkg),
                $expected_non_streaming,
            ...
            // Streaming
            pkg.set("baml_client.stream_types");
            let alias_rust_stream = ir_type_alias_to_rust_stream(type_alias, &pkg, None);
            assert_eq!(
                alias_rust_stream.type_.serialize_type(&pkg),
                $expected_streaming,
    ...
    (
        ...
        use internal_baml_core::ir::{repr::make_test_ir, IRHelper};
        use $crate::ir_to_rust::enums::ir_enum_to_rust;
        use $crate::package::CurrentRenderPackage;
        ...
        let enum_rust = ir_enum_to_rust(enm, &pkg);
        assert_eq!(enum_rust.name, $enum_name);

        let expected_values: Vec<&str> = vec![$( $value ),*];
        let actual_values: Vec<&str> = enum_rust.values.iter().map(|(v, _)| v.as_str()).collect();
        assert_eq!(
            actual_values,
            expected_values,
