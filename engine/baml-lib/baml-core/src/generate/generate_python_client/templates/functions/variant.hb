...
{{> func_def func_name=name unnamed_args=function.unnamed_args args=function.args return=function.return}}
    ...
    response = await {{client}}.run_{{#if is_chat}}chat{{else}}prompt{{/if}}_template({{#if is_chat}}message_templates{{else}}template{{/if}}=__prompt_template, replacers=__input_replacers, params=dict(arg=adapted_input))
    {{else}}
    response = await {{client}}.run_{{#if is_chat}}chat{{else}}prompt{{/if}}_template({{#if is_chat}}message_templates{{else}}template{{/if}}=__prompt_template, replacers=__input_replacers, params=dict({{> arg_values unnamed_args=function.unnamed_args args=function.args}}))
    {{/if}}
...
def {{name}}_stream({{> func_params unnamed_args=this.function.unnamed_args args=this.function.args}}) -> AsyncStream[{{function.return.0.type}}, {{function.return.0.type_partial}}]:
    ...
    def run_prompt() -> typing.AsyncIterator[LLMResponse]:
        {{#if input_adapter}}
        adapted_input = input_adapter({{> arg_values unnamed_args=function.unnamed_args args=function.args}})
        raw_stream = {{client}}.run_{{#if is_chat}}chat{{else}}prompt{{/if}}_template_stream({{#if is_chat}}message_templates{{else}}template{{/if}}=__prompt_template, replacers=__input_replacers, params=dict(arg=adapted_input))
        {{else}}
        raw_stream = {{client}}.run_{{#if is_chat}}chat{{else}}prompt{{/if}}_template_stream({{#if is_chat}}message_templates{{else}}template{{/if}}=__prompt_template, replacers=__input_replacers, params=dict({{> arg_values unnamed_args=function.unnamed_args args=function.args}}))
        {{/if}}
        return raw_stream
