...
impl ToPython for FieldType {
    fn to_py(&self) -> String {
        match self {
            ...
            FieldType::List(inner) => format!("List[{}]", inner.to_py()),
            FieldType::Map(key, value) => {
                format!("Dict[{}, {}]", key.to_py(), value.to_py())
            }
            FieldType::Primitive(r#type) => r#type.to_py(),
            ...
            FieldType::Optional(inner) => format!("Optional[{}]", inner.to_py()),
        }
    }
}

// Whether the field could theoretically be made optional, like if we're streaming the results
pub(super) fn can_be_null(r#type: &FieldType) -> bool {
    match r#type {
        FieldType::Class(_) => true,
        FieldType::Enum(_) => true,
        // we always vend default empty list
        FieldType::List(_) => false,
        FieldType::Map(_, _) => true,
        FieldType::Primitive(_) => true,
        // There's a bug with unions where we cant parse optionals in unions so
        // we can't use them in streams.
        FieldType::Union(inner) => false,
        FieldType::Tuple(_) => true,
        FieldType::Optional(inner) => true,
        _ => false,
    }
}

pub(super) fn default_value(r#type: &FieldType) -> Option<String> {
    match r#type {
        FieldType::Optional(_) => Some("None".to_string()), // set default value to None in python
        _ => None,                                          // dont add a default value
    }
}
...
pub(super) fn to_internal_type(r#type: &FieldType) -> String {
    match r#type {
        ...
        FieldType::Optional(inner) => format!("{} | null", inner.to_py()),
    }
}
...
pub(super) fn to_internal_type_constructor(variable: &str, r#type: &FieldType) -> String {
    match r#type {
        ...
        FieldType::Map(_key, _value) => {
            unimplemented!("Map type is not supported in Python")
        }
