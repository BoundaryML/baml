...
impl Signature for Function {
    ...
    fn interface(&self) -> Option<String> {
        ...
        for (name, field_type) in self.inputs.iter() {
        ...
        self.output.interface().map(|s| content.push_str(&s));
        Some(content)
    }


    fn impl_(&self) -> Option<String> {
        let Some(config) = self.configs.iter().find(|config| config.name == self.default_config) else {
            return None;
        };
