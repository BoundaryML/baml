...
pub enum UnresolvedBamlMode {
    TextSchema,   // Default - current behavior
    ToolCalling,  // New - OpenAI native tool calling
}

impl UnresolvedBamlMode {
    pub fn required_env_vars(&self) -> HashSet<String> {
        HashSet::new()
    }

    pub fn resolve(&self, _ctx: &impl GetEnvVar) -> Result<BamlMode> {
        match self {
            Self::TextSchema => Ok(BamlMode::TextSchema),
            Self::ToolCalling => Ok(BamlMode::ToolCalling),
        }
    }
}
...
impl UnresolvedToolChoice {
    pub fn required_env_vars(&self) -> HashSet<String> {
        match self {
            Self::Specific(s) => s.required_env_vars(),
            _ => HashSet::new(),
        }
    }

    pub fn resolve(&self, ctx: &impl GetEnvVar) -> Result<ToolChoice> {
        match self {
            Self::Auto => Ok(ToolChoice::Auto),
            Self::None => Ok(ToolChoice::None),
            Self::Required => Ok(ToolChoice::Required),
            Self::Specific(s) => Ok(ToolChoice::Specific(s.resolve(ctx)?)),
        }
    }
}
