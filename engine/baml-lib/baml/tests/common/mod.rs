...
use baml_lib::{Configuration, Diagnostics, SourceFile, FeatureFlags};

#[allow(unused)]
pub(crate) fn parse_unwrap_err(schema: &str) -> String {
    let path = PathBuf::from("./unknown");
    baml_lib::parse_and_validate_schema(&path, vec![SourceFile::from((path.clone(), schema))], FeatureFlags::new())
        .map(drop)
        .unwrap_err()
        .to_pretty_string()
}
...
pub(crate) fn expect_error(schema: &str, expectation: &expect_test::Expect) {
    let path = PathBuf::from("./unknown");
    match baml_lib::parse_and_validate_schema(&path, vec![SourceFile::from((path.clone(), schema))], FeatureFlags::new())
    {
