...
impl BamlPdfPy {
    ...
    pub fn as_url(&self) -> PyResult<String> {
        match &self.inner.content {
            ...
            _ => Err(BamlInvalidArgumentError::new_err("Pdf is not a URL")),
        }
    }
    ...
    pub fn as_base64(&self) -> PyResult<Vec<String>> {
        match &self.inner.content {
            ...
            _ => Err(BamlInvalidArgumentError::new_err("Pdf is not base64")),
        }
    }
    ...
    pub fn __repr__(&self) -> String {
        match &self.inner.content {
            baml_types::BamlMediaContent::Url(url) => {
                format!("BamlPdfPy(url={})", url.url)
            }
            baml_types::BamlMediaContent::Base64(base64) => {
                format!(
                    "BamlPdfPy(base64={}, media_type={})",
                    base64.base64,
                    self.inner.mime_type.clone().unwrap_or("".to_string())
                )
            }
            _ => "Unknown BamlPdfPy variant".to_string(),
        }
    }
