...
pub fn handle_ffi_error(e: anyhow::Error) -> *const crate::ffi::c_void {
    let error_str = e.to_string();
    match CString::new(error_str.clone()) {
        Ok(c_string) => Box::into_raw(Box::new(c_string)) as *const crate::ffi::c_void,
        Err(_) => {
            ...
            match CString::new("Error creating error message") {
                Ok(fallback) => Box::into_raw(Box::new(fallback)) as *const crate::ffi::c_void,
