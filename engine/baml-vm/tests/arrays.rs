...
fn array_constructor() -> anyhow::Result<()> {
    assert_vm_executes_with_inspection(
        Program {
            ...
            ",
            function: "main",
            expected: VmExecState::Complete(Value::Object(ObjectIndex::from_raw(22))),
        },
        |vm| {
            let baml_vm::Object::Array(array) = &vm.objects[ObjectIndex::from_raw(22)] else {
                panic!(
                    "expected Array, got {:?}",
                    &vm.objects[ObjectIndex::from_raw(22)]
                );
            };
