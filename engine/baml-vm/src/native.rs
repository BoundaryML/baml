...
/// Array length.
pub fn array_len(vm: &mut Vm, args: &[Value]) -> Result<Value, VmError> {
...
/// Length of map
pub fn map_len(vm: &mut Vm, args: &[Value]) -> Result<Value, VmError> {
    // Arity is already checked by the VM.
...
/// Map `contains`
pub fn map_has(vm: &mut Vm, args: &[Value]) -> Result<Value, VmError> {
    // Arity is already checked by the VM.
...
pub fn image_from_url(vm: &mut Vm, args: &[Value]) -> Result<Value, VmError> {
    // Arity is already checked by the VM.
...
pub fn audio_from_url(vm: &mut Vm, args: &[Value]) -> Result<Value, VmError> {
    // Arity is already checked by the VM.
    let url = vm.objects.as_string(&args[0])?;
...
pub fn video_from_url(vm: &mut Vm, args: &[Value]) -> Result<Value, VmError> {
    // Arity is already checked by the VM.
    let url = vm.objects.as_string(&args[0])?;
...
pub fn pdf_from_url(vm: &mut Vm, args: &[Value]) -> Result<Value, VmError> {
    // Arity is already checked by the VM.
    let url = vm.objects.as_string(&args[0])?;
...
pub fn image_from_base64(vm: &mut Vm, args: &[Value]) -> Result<Value, VmError> {
    // Arity is already checked by the VM.
...
pub fn audio_from_base64(vm: &mut Vm, args: &[Value]) -> Result<Value, VmError> {
...
pub fn video_from_base64(vm: &mut Vm, args: &[Value]) -> Result<Value, VmError> {
    // Arity is already checked by the VM.
    let media_type = vm.objects.as_string(&args[0])?;
    let base64 = vm.objects.as_string(&args[1])?;
...
pub fn pdf_from_base64(vm: &mut Vm, args: &[Value]) -> Result<Value, VmError> {
    // Arity is already checked by the VM.
...
pub fn media_is_url(vm: &mut Vm, args: &[Value]) -> Result<Value, VmError> {
    // Arity is already checked by the VM.
...
pub fn media_is_base64(vm: &mut Vm, args: &[Value]) -> Result<Value, VmError> {
    // Arity is already checked by the VM.
    let media = vm.objects.as_media(&args[0])?;
...
pub fn media_as_url(vm: &mut Vm, args: &[Value]) -> Result<Value, VmError> {
    // Arity is already checked by the VM.
    let media = vm.objects.as_media(&args[0])?;
...
pub fn media_as_base64(vm: &mut Vm, args: &[Value]) -> Result<Value, VmError> {
    // Arity is already checked by the VM.
    let media = vm.objects.as_media(&args[0])?;
...
pub fn media_mime_type(vm: &mut Vm, args: &[Value]) -> Result<Value, VmError> {
    // Arity is already checked by the VM.
    let media = vm.objects.as_media(&args[0])?;
...
pub fn deep_copy_object(vm: &mut Vm, args: &[Value]) -> Result<Value, VmError> {
    // Arity is already checked by the VM.
...
pub fn any_value_to_string(vm: &mut Vm, args: &[Value]) -> Result<Value, VmError> {
    // Arity is already checked by the VM.
            ...
            Value::Object(obj_idx) => match &vm.objects[*obj_idx] {
                Object::Instance(instance) => {
                    ...
                    for (i, field_value) in fields.iter().enumerate() {
                        ...
                        result
                            .push_str(&format!("{field_indent}{field_name}: {formatted_value}\n"));
                    }
