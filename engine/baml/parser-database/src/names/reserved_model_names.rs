...
pub(crate) fn validate_class_name(ast_model: &ast::Class, block_type: &'static str, diagnostics: &mut Diagnostics) {
...
pub(crate) fn validate_function_name(ast_model: &ast::Function, block_type: &'static str, diagnostics: &mut Diagnostics) {
    if !is_reserved_type_name(ast_model.name()) {
        return;
    }
