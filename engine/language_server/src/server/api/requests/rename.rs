...
use crate::baml_project::{BamlRuntimeExt, ProjectType};
            ...
            Ok(ProjectType::Valid(Some(project))) => {
                let res = {
                    let mut guard: std::sync::MutexGuard<'_, crate::baml_project::Project> =
                        project.lock().unwrap();
                    let document_key =
                        DocumentKey::from_url(&PathBuf::from(guard.root_path()), &url)
                            .internal_error()?;
                    ...
                    let doc = guard
                        ...
                        .internal_error()?;
                    let symbol = get_word_at_position(
                        &doc.contents,
                    ...
                    if !is_valid_class && !is_valid_enum && !is_valid_type_alias {
                        return Err(anyhow::anyhow!("Cannot rename symbol '{}'", symbol))
                            .internal_error();
                    }
