...
1. **A type-safe API for your LLM** -- LLMs can sometimes forget a comma, or may add a comment inside a JSON, or just output the wrong format. BAML parses it correctly (See [Deserialization](/v3/type-deserializer) for more info)
2. **Scoped serializers** -- You can alias your type property names dynamically in prompts (e.g. `duration` -> `duration_in_minutes`) without having to change your code
3. **Compile time checks for template variables in prompts and configs**
4. **Organization** -- everything (prompts, configs, variables, renames) related to each impl are defined and visible in one place
