...
```python
def transform(overridenOutput: IntentWithReasoning) -> Intent:
  return intentWithReasoning.intent

intentWithReasoning = ClassifyIntent("I can't login")
return transform(intentWithReasoning)
``` */}
...
Adapters are a powerful way of transforming LLM outputs to try out different prompting strategies without worrying about having to refactor business logic.
