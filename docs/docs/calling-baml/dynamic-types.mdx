...
def run
  ...
  t.Category.add_value('VALUE3')
  t.Category.add_value('VALUE4')
  res = Baml.Client.dynamic_categorizer(input: "some input", baml_options: {type_registry: t})
  # Now res can be VALUE1, VALUE2, VALUE3, or VALUE4
  puts res
end
```
...
def run
  t = Baml::TypeRegistry.new
  t.User.add_property('email', tb.string)
  t.User.add_property('address', tb.string)
  
  res = Baml.Client.dynamic_user_creator(input: "some user info", baml_options: {type_registry: t})
  # Now res can have email and address fields
  puts res
end
```
...
def run
  t = Baml::TypeRegistry.new
  hobbies_enum = t.add_enum('Hobbies')
  hobbies_enum.add_value('Soccer')
  hobbies_enum.add_value('Reading')

  address_class = t.add_class('Address')
  address_class.add_property('street', t.string)

  t.User.add_property('hobby', hobbies_enum.type.optional)
  t.User.add_property('address', address_class.type.optional)
...
```ruby Ruby
t = Baml::TypeRegistry.new
t.User.add_property("email", t.string).description("The user's email")
```
...
```ruby Ruby
t = Baml::TypeRegistry.new
t.unstable_features.add_json_schema(...)
