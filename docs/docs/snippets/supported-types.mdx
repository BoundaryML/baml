...
## ⚠️ Unsupported
- `bytes` - Not yet supported. Use a `string[]` or `int[]` instead.
- `any/json` - Not supported. We don't want to encourage its use as it defeats the purpose of having a type system. if you really need it, for now use `string` and call `json.parse` yourself. Also, message us on discord so we can understand your use case and consider supporting it.
- `datetime` - Not yet supported. Use a `string` or `int` (milliseconds since epoch) instead.
- `duration` - Not yet supported. Use an `int` (milliseconds) instead.
