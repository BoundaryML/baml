...
## Other providers
You can use the `openai` provider if the provider you're trying to use has the same ChatML response format (i.e. HugginFace via their Inference Endpoints or local endpoint)
...
client<llm> MyClient {
  ...
  options {
    ...
    api_key env.OPEN
    base_url "https://some-custom-url-endpoint-here"
  }
}
```