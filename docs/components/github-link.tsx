...
export function GitHubLinkFallback() {
  return (
    <LinkShell>
      <span className="shadcn-header__stars shadcn-header__stars--loading" />
    </LinkShell>
  );
}
...
export async function GitHubLink() {
  ...
  if (!response?.ok) return <LinkShell />;
  const json = await response.json() as { stargazers_count?: unknown };
  if (typeof json.stargazers_count !== 'number') return <LinkShell />;
  const formattedCount = json.stargazers_count >= 1_000
    ? `${Math.round(json.stargazers_count / 1_000)}k`
    : json.stargazers_count.toLocaleString();

  return (
    <LinkShell>
      <span className="shadcn-header__stars">{formattedCount}</span>
    </LinkShell>
  );
}
