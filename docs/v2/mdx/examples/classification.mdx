...
<Info>If you have the [VSCode extension](/mdx/installation#baml-extension), **.baml** files will have syntax highlighting!</Info>
``` baml main.baml
...
2. Used Baml's **\{@Category.values\}** unique enum property, which writes down each enum and its description in the prompt. You can only use `.values` on enums to list them in your prompt.
