...
A `@variant[llm]` is an implementation of a `@function` which uses an LLM. Baml automatically provides:
...
Baml automatically generates a JSON schema for you, but you can customize it with `@stringify`.
