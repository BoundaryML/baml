...
Lets assume you have a function called `GetSentiment` with two different variants `v1` and `v2` (each variant being a unique prompt). In baml, you'll automatically get tests created for each variant you define with no extra work.
...
Baml will run that assert on every test case in that `@test_group`.

<Info>You don't have to call the function, just define it. Baml will call it for you after each function call.</Info>
```baml Baml
@test_group GroupName for GetSentiment {
    @method assert1 {
        @lang[py] {
...
<Tip>
  ...
  assert1(arg: InputType, result: OutputType):`. Baml defines `InputType` and
