...
<Note>One of the core philosophies of BAML is to allow you to see the prompt as it will be rendered, so you can debug it. Our VSCode extension will eventually show you what the rendered prompt will look like on-hover, so you know exactly what data will be sent to the LLM.</Note>
...
<Tip>
If your output schema contains enums you can add both print_type(..) and print_enum(..) in your prompt. The LLM will use the enum definitions to output the right enum in the appropriate JSON property.
</Tip>
