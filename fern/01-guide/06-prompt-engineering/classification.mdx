...
class SpamClassification {
  label "SPAM" | "NOT_SPAM" @description("The predicted class label")
}
```
...
```baml
function ClassifyText(text: string) -> SpamClassification {
  client "openai/gpt-4o-mini"
  prompt #"
    Classify the following text as either SPAM or NOT_SPAM. Common spam indicators include:
    - Promotional offers that are too good to be true
...
test BasicSpamTest {
  ...
  args {
    text "Buy cheap watches now! Limited time offer!!!"
  }
}
...
test NonSpamTest {
  functions [ClassifyText]
  args {
    text "Hey Sarah, can we meet at 3 PM tomorrow to discuss the project?"
  }
}
```
...
```baml
enum TicketLabel {
  ACCOUNT @description("Account-related issues")
  BILLING @description("Billing and payment related issues")
  GENERAL_QUERY @description("General product inquiries")
}

class TicketClassification {
  labels TicketLabel[] @description("The labels that apply to this support ticket")
}
```
...
- Each label has a description to help with classification
...
For more advanced classification techniques, check out our [Advanced Prompt Engineering Guide](../advanced-prompts).