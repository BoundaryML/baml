...
class PIIData {
  index int @description("Index of the PII data item")
  dataType string @description("Type of PII data (e.g., name, email, phone)")
  value string @description("The actual PII value found")
}

class PIIExtraction {
  privateData PIIData[] @description("List of extracted PII data items")
}
```

This schema defines:
- `PIIData`: A class representing a single piece of PII with its type and value
- `PIIExtraction`: A container class that holds an array of PII data items
