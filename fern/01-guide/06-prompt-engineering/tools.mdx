...
func main() {
    ...
    // Go uses type assertions to handle union types
    switch t := tool.(type) {
    case *types.WeatherAPI:
        fmt.Println("Weather API called:")
        fmt.Printf("City: %s\n", t.City)
        fmt.Printf("Time of Day: %s\n", t.TimeOfDay)
    case *types.MyOtherAPI:
...
func main() {
    ...
    for {
        ...
        // Handle the tool response using type assertion
        switch tool := toolResponse.(type) {
        case *types.WeatherAPI:
            result := handleWeather(tool)
            fmt.Printf("Agent (Weather): %s\n", result)
        case *types.CalculatorAPI:
            result := handleCalculator(tool)
