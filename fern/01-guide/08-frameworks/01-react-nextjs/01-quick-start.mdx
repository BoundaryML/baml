...
export function StoryForm() {
  ...
  const data = writeMeAStory.isLoading ? writeMeAStory.streamingData : writeMeAStory.data;

  return (
    <div>
      <button
        onClick={() => writeMeAStory.mutate("Dragons")}
        disabled={writeMeAStory.isLoading}>
        Generate Story
      </button>

      {writeMeAStory.isLoading && <div>Generating...</div>}
...
export function StreamingStoryForm() {
  const {
    data: story,           // Final complete response
    finalData: fStory,
    streamData: sStory, // Real-time partial updates
    isLoading,
    isError,
    error,
    mutate
  ...
  const data = isLoading ? streamingStory : finalStory;
