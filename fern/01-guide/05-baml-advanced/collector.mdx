...
The Collector works seamlessly with BAML's [`id`](/ref/baml_client/id) functionality for tracking multiple function calls. This is particularly useful when running functions in parallel.
...
def run():
    ...
    first_id, resume = b.id.ExtractResume("...", baml_options={"collector": collector})
    second_id, invoice = b.id.ExtractResume("...", baml_options={"collector": collector})

    # List of all requests
    print(collector.logs)

    # The first request usage is available
    print(collector.id(first_id).usage)

    # The second request usage is also available
    print(collector.id(second_id).usage)
...
async function run() {
    const collector = new Collector("multi-function-collector")
    ...
    // The first request usage is available
    console.log(collector.id(first_id)?.usage)

    // The second request usage is also available
    console.log(collector.id(second_id)?.usage)

    // The total usage of all requests
    console.log(collector.usage)
}
```
</Tab>
...
def run
    ...
    first_id, resume = Baml.Client.id.test_openai_gpt4o_mini(input: 'First call', baml_options: { collector: collector })
    second_id, invoice = Baml.Client.id.test_openai_gpt4o_mini(input: 'Second call', baml_options: { collector: collector })
...
- [Function IDs](/ref/baml_client/id) - Track specific function calls
