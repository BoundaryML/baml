...
const main = () => {
  try {
    ...
    console.log(
      `Check exact_citation_found: ${checks.exact_citation_found.status}`
    );
...
<Tip>
  {" "}
  When using `@check`, all checks on the response data are evaluated even if one
  fails. In contrast, with `@assert`, a failure will stop the parsing process and
