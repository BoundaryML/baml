...
> ⚠️ **Prerequisites**: You'll need a S3 bucket, an IAM role with permissions to access S3 and Bedrock, and access to a Bedrock model. See [AWS docs](https://docs.aws.amazon.com/bedrock/latest/userguide/batch-inference-prereq.html) for setup.
...
new S3Client({ region }).send(new PutObjectCommand({ Bucket: bucket, Key: key, Body: jsonl })).then(() => {
  new BedrockClient({ region }).send(new CreateModelInvocationJobCommand({
