...
Use `b.request` to build one JSON object per line. Each line needs a `recordId` and a `modelInput` shaped for your Bedrock model (here, chat `messages`).
...
def to_jsonl(req):
    return json.dumps({
        'recordId': req.id,
        'modelInput': {'messages': req.body.json().get('messages')}
    })

# 50 × 2 resumes = 100 requests (Bedrock batch minimum)
jsonl = '\n'.join([
    to_jsonl(b.request.ExtractResume("John Doe | Software Engineer | BSc in CS")),
    to_jsonl(b.request.ExtractResume("Jane Smith | Data Scientist | PhD in Statistics")),
] * 50)
...
import type { HttpRequest } from '@boundaryml/baml/native';

const toJsonl = (req: HttpRequest) => JSON.stringify({
  recordId: req.id,
  modelInput: { messages: req.body.json().messages }
});
...
Write `jsonl` to the object key you will reference from the batch job, then create a model invocation job pointing at that object and an output prefix in the same bucket.
...
<Note>
  **Limitations**:
  ...
  - Bedrock batch requires a minimum of 100 requests
  - Jobs are asynchronous—poll. You must use `get_model_invocation_job` for completion results
  - Errors are only visible after the batch job completes
</Note>
