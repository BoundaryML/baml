...
1. Test prompts without API calls
2. See exactly what will be sent
3. Iterate on prompts instantly
...
- **Hidden prompts** - You can't see or control what's sent
- **Limited schema expressiveness** - Zod wasn't built for LLMs
- **Package proliferation** - Different package for each provider
- **No built-in testing** - Mock everything or burn tokens

With BAML:
- **Prompts as code** - See and version control everything
- **Purpose-built types** - Descriptions, aliases, all included
- **Unified interface** - All providers work the same way
- **Test in VSCode** - No API calls needed

We built BAML because we were tired of elegant APIs that fall apart in production.
