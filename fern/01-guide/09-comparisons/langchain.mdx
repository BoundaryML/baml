...
With BAML you get:
- **Full prompt transparency** - See and control exactly what's sent
- **Native testing** - Test in VSCode without API calls  
- **Multi-model by design** - Switch providers with one line
- **Token visibility** - Know exactly what you're paying for
- **Type safety** - Generated Pydantic models that always match your schema

We built BAML because we were tired of wrestling with framework abstractions when all we wanted was reliable structured extraction.
