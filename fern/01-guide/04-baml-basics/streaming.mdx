...
func example1(receipt string) {
    ...
    for value := range stream {
        ...
        if !value.IsFinal && value.Stream() != nil {
            ...
            fmt.Printf("partial: parsed %d items\n", len(partial.Items))
        }
        
        if value.IsFinal && value.Final() != nil {
            final := *value.Final()
            fmt.Printf("final: %d items\n", len(final.Items))
        }
    }
}

// Using stream without iteration - just getting final result
func example2(receipt string) (*types.ReceiptInfo, error) {
    ctx := context.Background()
    ...
    // Skip iteration and just get final result
    for value := range stream {
        if value.IsError {
