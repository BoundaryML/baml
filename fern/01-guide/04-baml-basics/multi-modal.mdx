...
By default, BAML automatically handles URL-to-base64 conversion based on what each provider supports. However, you can customize this behavior using the `media_url_resolver` configuration:
...
client<llm> FastClaude {
  ...
  options {
    ...
    media_url_resolver {
      image "never"     // Anthropic can fetch URLs directly
      pdf "always"      // Required by Anthropic API (As of October 2025)
    }
  }
}
```
...
client<llm> GeminiWithGCS {
  ...
  options {
    ...
    media_url_resolver {
      image "if_google_uri"  // Preserve gs:// URLs, convert others
    }
  }
}
```
...
client<llm> CompatibleClient {
  ...
  options {
    ...
    media_url_resolver {
      image "always"    // Ensure images are embedded
      audio "always"    // OpenAI requires base64 for audio
      pdf "always"      // Embed PDFs for reliability
    }
  }
}
```
...
1. **`never`** - Allows providers to fetch URLs reducing payload size
2. **`always`** - Embedding content avoids external dependencies
3. **`ensure_mime`** - Required for proper media handling for some providers (if the mime type is not provided, it will be downloaded to determine the mime type)
4. **`if_google_uri`** - Preserves Google Cloud Storage URLs for Google providers

See the [provider documentation](/reference/baml/clients/providers) for provider-specific defaults and requirements.
