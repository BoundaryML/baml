...
Configure a default abort controller for all operations through the client, which can be used to cancel operations in progress.
...
// Create a global abort controller
const globalController = new AbortController()

// Create client with default abort controller
const myB = b.withOptions({ 
    abortController: globalController 
})

// All operations will use the global controller
const promise1 = myB.ExtractResume("...")
const promise2 = myB.ExtractInvoice("...")

// Cancel all operations at once
globalController.abort()

// Override with a specific controller for one call
const specificController = new AbortController()
const result = await myB.ExtractResume("...", {
    abortController: specificController
})
```
</Tab>
...
# Create client with default abort controller
my_b = b.with_options(abort_controller=global_controller)

# All operations will use the global controller
task1 = my_b.ExtractResume("...")
task2 = my_b.ExtractInvoice("...")

# Cancel all operations at once
global_controller.abort()

# Override with a specific controller for one call
specific_controller = AbortController()
result = await my_b.ExtractResume("...", 
    baml_options={"abort_controller": specific_controller})
```
</Tab>
</Tabs>
...
| `abortController` | [`AbortController`](/03-reference/baml_client/abort-controller) | (TypeScript) Controller to cancel operations |
| `abort_controller` | [`AbortController`](/03-reference/baml_client/abort-controller) | (Python) Controller to cancel operations |
...
- [AbortController](/03-reference/baml_client/abort-controller) - Cancel in-flight operations
