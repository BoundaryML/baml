...
When you create an Image using `from_url`, BAML processes the URL according to your client's `media_url_resolver` configuration:

- **[OpenAI](/baml/clients/providers/openai#media_url_resolver)**: By default keeps URLs as-is (`never`). Set to `always` to convert to base64.
- **[Anthropic](/baml/clients/providers/anthropic#media_url_resolver)**: By default keeps URLs as-is (`never`). The provider accepts both formats.
- **[Google AI](/baml/clients/providers/google-ai#media_url_resolver)**: By default uses `if_google_uri` to preserve gs:// URLs while converting others.
- **[Vertex AI](/baml/clients/providers/vertex#media_url_resolver)**: By default uses `ensure_mime` to include MIME type information.
- **[AWS Bedrock](/baml/clients/providers/aws-bedrock#media_url_resolver)**: By default converts to base64 (`always`).
