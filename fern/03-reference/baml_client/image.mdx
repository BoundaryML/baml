...
- **OpenAI**: By default keeps URLs as-is (`never`). Set to `always` to convert to base64.
- **Anthropic**: By default keeps URLs as-is (`never`). The provider accepts both formats.
- **Google AI**: By default uses `if_google_uri` to preserve gs:// URLs while converting others.
- **Vertex AI**: By default uses `ensure_mime` to include MIME type information.
- **AWS Bedrock**: By default converts to base64 (`always`).

You can override these defaults in your client configuration. See [media_url_resolver](/reference/baml/clients/providers#media_url_resolver) for details.