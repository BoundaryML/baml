...
const fetchedImage = await Image.fromUrlAsync('https://example.com/image.png')
```
...
export function TestImageInput() {
    ...
    const handleClick = async () => {
        const image = await BamlImage.fromUrl('https://upload.wikimedia.org/wikipedia/en/4/4d/Shrek_%28character%29.png')
        mutate(image)
    }
