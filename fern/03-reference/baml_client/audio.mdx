...
Audio URLs are processed according to your client's `media_url_resolver` configuration:

- **[OpenAI](/baml/clients/providers/openai#media_url_resolver)**: By default converts to base64 (`always`) for compatibility.
- **[Vertex AI](/baml/clients/providers/vertex#media_url_resolver)**: By default uses `ensure_mime` to include MIME type.
- **[Anthropic](/baml/clients/providers/anthropic#media_url_resolver)**: By default keeps URLs as-is (`never`).
- **[Google AI](/baml/clients/providers/google-ai#media_url_resolver)**: By default keeps URLs as-is (`never`).
- **[AWS Bedrock](/baml/clients/providers/aws-bedrock#media_url_resolver)**: By default converts to base64 (`always`).

Note: OpenAI requires audio to be base64-encoded, which is why the default is `always`.