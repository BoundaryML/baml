...
<Info>
  `Pdf` instances can be created from URLs, Base64 data, or local files. URL processing is controlled by your client's `media_url_resolver` configuration.
...
- **Anthropic**: By default converts to base64 (`always`) as required by their API.
