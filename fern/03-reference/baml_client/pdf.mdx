...
PDF URLs are processed according to your client's `media_url_resolver` configuration:

- **[Anthropic](/baml/clients/providers/anthropic#media_url_resolver)**: By default converts to base64 (`always`) as required by their API.
- **[AWS Bedrock](/baml/clients/providers/aws-bedrock#media_url_resolver)**: By default converts to base64 (`always`).
- **[OpenAI](/baml/clients/providers/openai#media_url_resolver)**: By default keeps URLs as-is (`never`).
- **[Google AI](/baml/clients/providers/google-ai#media_url_resolver)**: By default keeps URLs as-is (`never`).
- **[Vertex AI](/baml/clients/providers/vertex#media_url_resolver)**: By default keeps URLs as-is (`never`).

<Warning>
  Many websites block direct PDF fetching. If you encounter issues with URL-based PDFs, try:
  1. Using `media_url_resolver.pdf = "always"` to fetch and embed the content
