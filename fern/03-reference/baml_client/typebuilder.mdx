...
# Remove a property from an existing class.
tb.User.remove_property('address')

# List the properties of the class as [(name, field_type)]
for prop_name, prop_builder in tb.User.list_properties():
    if prop_builder.get_type() == tb.string(): # Check type of the property
        prop_builder.type(tb.int())            # Modify the property type

# Remove all properties of the class.
tb.User.reset()
```
</Tab>
...
// Remove a property from the existing class.
tb.User.removeProperty('address')

// List the properties of the class as Array<[name, field_type]>
for (const [prop_name, prop_builder] of tb.User.listProperties()) {
    if (prop_builder.getType().equals(tb.string())) { // Check type of the property
        prop_builder.setType(tb.int())                // Modify the property type
    }
}
...
# Remove a property from the new class.
address.remove_property("city")

# List the properties of the class as [(name, field_type)]
for prop_name, prop_builder in address.list_properties():
    if prop_builder.get_type() == tb.string(): # Check type of the property
        prop_builder.type(tb.int())            # Modify the property type
...
// Remove a property from the new class.
address.removeProperty("city")

// List the properties of the class as Array<[name, field_type]>
for (const [prop_name, prop_builder] of address.listProperties()) {
    if (prop_builder.getType().equals(tb.string())) { // Check type of the property
        prop_builder.setType(tb.int())                // Modify the property type
    }
}
