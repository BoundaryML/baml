...
| --- | --- |
| Class | One element with a child element for each field |
| Enum | Element text containing one enum value |
| List | A wrapper element containing repeated `<item>` elements |
| Map | A wrapper containing repeated `<entry><key>…</key><value>…</value></entry>` elements |
| Optional | Omit the element for `null`, or use `null` as its text |
| Union | Use exactly one of the rendered alternative element templates |
