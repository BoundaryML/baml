...
Set `base_url` to the provider's API root. BAML appends `/chat/completions` when it sends a chat request, so a provider that exposes `GET /v1/models` and `POST /v1/chat/completions` needs a `base_url` ending in `/v1`. An origin-only URL would instead send the request to `/chat/completions` and commonly returns a 404.
