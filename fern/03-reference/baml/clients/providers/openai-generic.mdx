...
The `openai-generic` provider supports third-party APIs that implement OpenAI's Chat Completions request and response format, such as Groq, HuggingFace, Ollama, OpenRouter, and Together AI. Use [`openai`](/ref/llm-client-providers/open-ai), which targets `api.openai.com`, for OpenAI's first-party API. Use [`openai-responses`](/ref/llm-client-providers/open-ai-responses-api) for an API that implements the `/responses` endpoint.

Set `base_url` to the provider's API root. BAML appends `/chat/completions` when it sends a chat request, so a provider that exposes `GET /v1/models` and `POST /v1/chat/completions` needs a `base_url` ending in `/v1`. For that provider, an origin-only URL would send the request to `/chat/completions` and can return a 404. Providers that serve `/chat/completions` at the origin should use the origin-only URL.
...
Use a named client such as `MyClient` when configuring a custom endpoint. The `"openai-generic/<model>"` shorthand cannot carry the required `base_url`; pointing the function at the named client preserves all of its options.
...
| Respan | [Respan](/ref/llm-client-providers/respan) |
...
<ParamField path="base_url" type="string">
  The required API root for the provider. BAML appends `/chat/completions` for chat requests.

  For example, if the provider's models endpoint is `https://api.provider.com/v1/models`, set this to `https://api.provider.com/v1`.
</ParamField>
