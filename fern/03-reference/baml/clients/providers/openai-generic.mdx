...
The `openai-generic` provider supports third-party APIs that implement OpenAI's Chat Completions request and response format, such as Groq, HuggingFace, Ollama, OpenRouter, and Together AI. Use [`openai`](/ref/llm-client-providers/open-ai) for OpenAI's first-party API or [`openai-responses`](/ref/llm-client-providers/open-ai-responses-api) for an API that implements the Responses endpoint.
