...
<Markdown src="/snippets/media-url-resolver.mdx" />

<Note>
  Anthropic's default behavior is to convert PDFs to base64 (`always`) while keeping other media types as URLs (`never`). This is because Anthropic's API requires PDFs to be base64-encoded.
</Note>
