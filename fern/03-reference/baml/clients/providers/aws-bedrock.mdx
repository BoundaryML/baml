...
<Note>
  AWS Bedrock converts most media to base64 by default (`always` for images, audio, and PDFs). Consider using S3 presigned URLs with `never` mode for large files to avoid base64 overhead.
</Note>
