...
<Note>
  When `tenant_id` or `client_id` is present, BAML uses Microsoft Entra ID (formerly Azure Active Directory)
  OAuth2 bearer token authentication instead of the static `api-key` header. The `api_key` field is ignored
  when Entra ID auth is configured. You cannot provide both `api_key` and `tenant_id`/`client_id` — doing so
...
>
  ...
  If omitted when `tenant_id`/`client_id` are set, BAML uses the system default credential chain:
  managed identity (Azure VMs, App Service, AKS) first, then developer tools (Azure CLI / azd CLI).
</ParamField>
