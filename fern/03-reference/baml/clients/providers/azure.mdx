...
>
  ...
  Set to `env.AZURE_TENANT_ID` to read from the environment. If only `client_id` is provided without
  `tenant_id`, the missing value defaults to the `AZURE_TENANT_ID` environment variable.
...
>
  ...
  Set to `env.AZURE_CLIENT_ID` to read from the environment. If only `tenant_id` is provided without
  `client_id`, the missing value defaults to the `AZURE_CLIENT_ID` environment variable.
</ParamField>
