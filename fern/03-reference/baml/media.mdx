...
You can control how BAML processes media URLs before sending them to providers using the `media_url_resolver` configuration option:
...
client<llm> MyClient {
  ...
  options {
    media_url_resolver {
      image "always"     // Convert URLs to base64
      pdf "never"        // Keep URLs as-is
    }
  }
}
```

This allows you to override the default behavior for each provider and media type combination. See [client configuration](/reference/baml/clients/providers#media_url_resolver) for details.
