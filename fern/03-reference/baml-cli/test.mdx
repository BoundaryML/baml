...
| `--evalport <DIR>`        | Export selected tests and results as EvalPort JSON documents      | None          |
...
Use `--evalport <DIR>` to export selected `.baml` test definitions and their results for eval dashboards, regression tools, or other EvalPort consumers:

```bash
baml-cli test --evalport ./evalport
```

The directory contains `suite.json`, an EvalPort `EvalSuite`, and `results.json`, an EvalPort `ResultSet`. With `--list`, BAML writes only `suite.json` and does not execute the tests.

BAML exports each `Function::Test` pair as an EvalPort test case and each `@@assert` or `@@check` as a custom grader. Because EvalPort inputs are strings, the callable function and JSON-serialized BAML arguments are encoded in `input`; the original structured arguments, constraint expressions, constraint levels, labels, and source locations are also preserved under `baml.*` metadata or custom grader parameters. Export is intentionally one-way because arbitrary EvalPort graders cannot be translated losslessly into BAML/Jinja constraints.
