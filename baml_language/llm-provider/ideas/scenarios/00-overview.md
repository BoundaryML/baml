...
|---|---|---|---|
| [01](01-single-turn-text/) | Single-turn text | Workable-with-additions | One codec absorbs the 3 system-prompt placements + role split, but usage/refusal don't fit `call<T> -> T` and force a `Metered` capability + `Refused` error + a readable prompt view. |
...
| [06](06-non-text-output/) | Non-text output (image/speech/STT) | Workable-with-additions | The `Body` codec absorbs bytes/audio/text, but dedicated-vs-in-conversation is exposed as two T types (`Media` vs `MixedReply`) that no single function spans. |
| [07](07-reasoning/) | Reasoning models | Workable-with-additions | Reasoning is an out-of-band `Thinks.think -> WithReasoning<T>` companion; cross-turn continuity adds an opaque `ContinuationState` that breaks under `Fallback`. |
| [08](08-enriched-outputs/) | Logprobs / citations / grounding | Workable-with-additions | Each enrichment is its own `requires Provider` capability with a value+sidecar `*$Result<T>`; no shared citation type, no honest fallback, runtime-only. |
...
| [11](11-parallel-tools/) | Parallel tool calls | Workable-with-additions | Parallelism slots below `ctx.dispatch` as a pluggable `Dispatcher`; needs a `Tool.effect` annotation and host concurrency (`gather`/`Semaphore`) BAML lacks. |
...
| [15](15-guardrails/) | Input/output tripwires | Workable-with-additions | A `Guarded` combinator wraps any provider; input guards race via a `race_cancel` host primitive, but output-trip-mid-loop can only throw-and-restart. |
...
| [32](32-observability/) | Spans / cost / wire-vs-UI | Workable-with-additions | A `Traced` combinator wraps any provider; usage is an out-of-band side-channel (races under fallback) and the wire-vs-UI split has no home in the model. |
...
| [34](34-cost-and-batch/) | Cost / tokens + batch | Workable-with-additions | A `Metering` capability returns `Metered<T>`; a `Budget` combinator sums usage via a host cell; batch is a submit→poll→download lifecycle. Base contract + sugar leak under the weight. |
...
| # | Title | Verdict | One-line |
|---|---|---|---|
| [43](43-workflow-graph/) | Composing a durable step graph | Workable-with-additions | A step is a typed function and a model call is `provider.call<T>`; fan-in/join + durable resume are net-new host (`baml.async`) and app (`Checkpoint`) layers outside the Provider hierarchy. |
...
| [47](47-workflow-agent-nesting/) | Agents-in-workflows, workflows-in-agents | Workable-with-additions | Workflow-as-tool is free (workflow→function→Provider→Tool); agent-as-step collapses a loop into one `Durable`-wrapped `call`; determinism is host-convention, not typed. |
