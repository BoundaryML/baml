...
- [x] Make one mutable `ToolRegistry` the source of truth for a run. Hooks
  mutate it through `StepContext`; duplicate names fail visibly unless the
  caller explicitly replaces an entry.
...
- [ ] Authorized live OpenAI and Anthropic testsets pass, with unavailable
  credentials or provider features reported separately from code failures.
  Current blocker: neither API key is in the shell and the repo's Infisical
  CLI has no authenticated session.
- [x] `v2_deviations.md`, normative signatures, and user-guide examples match
