...
- `open_live` returns a long-lived `Live` resource, while `drive<T>` must
  terminate with one `Response<T>`;
...
```baml
let task = Talk.task("Hello", $provider = RealtimeOnly)
...
**Normative usage rule:** realtime interaction uses `.task(...)` plus
`ai.drivers.open_live(task, channel)`. The caller retains the `Channel` for
ongoing input/output and the returned `Live` resource for events,
interruptions, and cleanup. A realtime-only provider MUST NOT implement
`DriveProvider` merely to make a direct function returning `void` compile.
`void` answers “what value is returned?”; it does not answer “when is this
live interaction finished?” Returning it immediately would discard the live
resource, while waiting for disconnect would hide the interaction controls
the caller needs.
