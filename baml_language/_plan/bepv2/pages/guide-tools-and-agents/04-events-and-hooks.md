...
let events = ai.drivers.stream_agent(
  ...
  ai.AgentOptions.new(tools = [lookup_order, search_policy]),
)
defer { events.cleanup() }

while (true) {
  match (events.next()) {
    let _: baml.stream.StreamFinished => break,
...
let outcome = events.final()
```

The stream includes model, reasoning-summary, tool, provider-change,
tool-roster, usage, and terminal events. `final()` preserves the task's typed
result as `AgentRun<Resolution>` after event consumption finishes.
...
- [Running and observing the loop](../specification/05-tools-and-agents.md#running-and-observing-the-loop)
