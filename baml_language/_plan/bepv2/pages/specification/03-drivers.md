...
In `Task<T>`, `T` is the LLM function's declared result. Every safe driver that
accepts `Task<T>` must preserve `T` in at least one successful observable
terminal path. It may:

- return `T` directly;
- wrap it in `Response<T>`;
- defer it behind `Stream<_, T>`, `Job<T>`, or `Batch<T>`; or
- expose it through a terminal branch such as `Done<T>` in `AgentRun<T>`.

An event stream preserves `T` through its terminal accessor. For example,
`AgentEventStream<T>.next()` exposes events as they happen, while
`AgentEventStream<T>.final()` returns the completed `AgentRun<T>`.
...
| `stream_agent` | `Task<T>` or `StreamTask<T, TPartial, P>` | `AgentEventStream<T>` | observable tool loop; `final()` returns `AgentRun<T>` |
...
### Resource construction and open-ended lifecycles

These drivers do not all consume `Task<T>` at construction time. Their result
type explains where typed work happens later.
...
These operations use fixed domain result types rather than a generic `T`.
...
Their inputs and outputs are not always prompt-shaped, so they may use
specialized task types. They still follow the same ownership rule: an intent
describes the work, and the driver owns execution and lifecycle.
...
class StepPlan {
  ...
  tools: Tool[]?,        // complete next roster; null keeps the current roster
  stop: AgentStop?,
}
```
...
`StepContext.tools` is a snapshot. `StepContext` does not expose the mutable
registry. Observers also receive immutable event views.

## Dynamic tools

The driver creates its active application-tool roster from `AgentOptions` and
the task. Before each provider step, it gives the hook a snapshot of that
roster:

```text
current = active registry snapshot
plan = prepare_step(StepContext { tools: current, ... })
if plan.tools is non-null:
  validate every tool name in plan.tools
  replace the active registry with plan.tools
send the active registry snapshot to the provider
```

`StepPlan.tools` is the complete next roster, not a list of changes. `null`
keeps the current roster. `[]` replaces it with no application tools. A
non-empty list replaces it with exactly those tools. The replacement persists
for later steps and also updates an attached `ToolRegistry`.

Provider-owned tools remain typed provider configuration and are translated
separately by the provider adapter.
...
```baml
function prepare_step(ctx: StepContext) -> StepPlan throws baml.errors.ToolError {
...
A tool may itself discover or authorize more tools. The hook returns the
complete next roster; it does not mutate the task declaration or the driver's
registry directly.

Removing one application tool means returning every tool except that one:

```baml
function prepare_step(ctx: StepContext) -> StepPlan {
  if (ctx.step == 2 && refund_permission_expired(ctx)) {
    let next_tools = ctx.tools.filter((tool: Tool) -> bool {
...
An explicit empty replacement removes all application tools and remains active
until a later hook returns another replacement:
...
token. [Conversations and transcripts](./06-conversations-and-transcripts.md)
defines these interfaces and rules.
