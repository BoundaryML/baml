# 1. Tasks and Why They Come First

In BAML, the reusable unit is the typed task, not a configured model object.
The task says what work to do. The provider says which model performs it. This
separation lets the same task use direct calls, streaming, agents, background
jobs, or another provider without rewriting the prompt and result type.

## What to remember

- Call the LLM function directly for the normal one-result path.
- Use `.task(...)` when another driver should control the lifecycle.
- Treat the task's return type as a promise that the driver must preserve.
- Override the provider without changing the task's business meaning.
- Keep model-specific transport details inside provider implementations.

## Two ways to organize AI code

Every LLM library must choose what developers declare once and reuse: the
*model* or the *task*.
...
In `Task<T>`, `T` is the LLM function's declared return type. It is not just a
hint for prompt rendering. It is the application value the task promises to
produce. If `ExtractInvoice.task(scan)` creates a `Task<Invoice>`, a safe
driver must make that `Invoice` observable on at least one successful terminal
path.

Different lifecycles may preserve it differently:

```text
direct call       Task<Invoice> -> Invoice
with metadata     Task<Invoice> -> Response<Invoice>
background job    Task<Invoice> -> Job<Invoice> -> Response<Invoice>
agent loop        Task<Invoice> -> Done<Invoice> | BudgetReached | Handoff
```

Some lifecycles intentionally do not have one final application value. A
realtime conversation may contain many user turns, assistant responses, tool
calls, interruptions, and media events. Such a lifecycle accepts `Task<null>`:
the task supplies instructions and tools, while the returned resource supplies
events and controls.

This gives driver authors one simple rule: a driver that accepts `Task<T>` must
preserve `T`; a lifecycle that cannot name one successful final `T` must accept
`Task<null>` or a more specific non-generic intent type. The
[Drivers](./03-drivers.md#what-happens-to-t) page applies this rule to the
standard catalog.
...
([Tools and agents](./05-tools-and-agents.md)), and a lazy prompt template is a
first-class expression ([Desugaring](./02-desugaring.md)).
...
and everything downstream ([sessions](./07-resources.md#sessions)) still benefits from the typed
turn.
...
template plus a manual task ([Desugaring](./02-desugaring.md)):
...
- **You want an execution mode BAML does not ship** → write an ordinary
  function consuming `ai.Task<T>` ([Custom capabilities](./08-custom-capabilities.md)).
...
(model + tools + policy), it exists as a provider value
([Tools and agents](./05-tools-and-agents.md)), and the
