...
Retry and fallback can repeat work. That is unsafe after a tool changed data,
a stream showed output, or a provider accepted a billed job. A safe decision
uses two inputs: facts reported by the failure and the replay policy of the
...
Business routing is ordinary code returning a provider
([Providers and capabilities](./04-providers-and-capabilities.md)):
