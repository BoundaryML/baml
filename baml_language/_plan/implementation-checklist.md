...
- [ ] Parse `//baml:llm_capability` (interfaces) + `//baml:llm_companion(<suffix>)` (driver fns) via `has_baml_marker`; AST fields; package-wide syntactic pre-pass; stdlib registry table baked at build time (like `PROVIDER_CONFIGS`).
- [ ] Semantic validation diagnostics: marker on non-interface / not requiring `Provider`; driver signature nonconforming (client+prompt params, generic arity 1/2); duplicate/shadowed suffix.
