...
- Made a Session `let` binding behave like an ordinary binding. A fresh literal initializer now widens to its base type, so `s.eval("let n = 5")` binds `int` rather than the literal type `5`, and a method call on a binding reaches its receiver instead of failing with an internal `got any` error — `n.abs()`, `text.to_upper_case()` and the same call inside the introducing submission all work. Field access and indexing on a binding are unchanged. Container methods whose owner is generic (`v.length()` on an array or map) are not covered and still fail as before. ([#4529](https://github.com/BoundaryML/baml/pull/4529)) - Antonio Sarosi
