...
- Carried a runtime-minted type's identity through interface dispatch: `type.of<T>()` inside an implements-block or inherited default method is now the exact `type` value the caller minted, instead of an `==`-distinct re-derivation of the same type. Identity-keyed patterns — a registry keyed by type, a comparison against a stored type — match inside an impl body instead of silently missing. Statically instantiated type parameters and derived types (`t.array()`, `type.meta(…)`) are unaffected. ([#4516](https://github.com/BoundaryML/baml/pull/4516)) - Antonio Sarosi
