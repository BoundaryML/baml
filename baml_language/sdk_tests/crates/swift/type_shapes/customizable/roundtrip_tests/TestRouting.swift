...
final class TestRouting: XCTestCase {
    ...
    func test_round_trip_root_foo() throws {
        let f = Baml.Foo(v: 6)
        XCTAssertEqual(try Baml.lorem.round_trip_root_foo(f: f), f)
    }

    func test_round_trip_lorem_resume_from_ipsum() throws {
