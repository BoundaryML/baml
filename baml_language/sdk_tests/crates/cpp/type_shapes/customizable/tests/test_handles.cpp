...
public:
    OneShotHttpServer() {
        ...
        addr.sin_port = 0;
        BAML_ASSERT(::bind(fd_, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) == 0);
        socklen_t len = sizeof(addr);
        BAML_ASSERT(::getsockname(fd_, reinterpret_cast<sockaddr*>(&addr), &len) == 0);
...
BAML_TEST(http_get_response_fields_and_methods) {
    ...
    const baml_sdk::baml::http::Response resp = baml_sdk::baml::http::fetch(server.url());
