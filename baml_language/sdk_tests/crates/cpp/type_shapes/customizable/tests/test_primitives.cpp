...
BAML_TEST(return_int) { BAML_ASSERT_EQ(primitives::return_int(), int64_t{42}); }

BAML_TEST(return_float) { BAML_ASSERT_EQ(primitives::return_float(), 3.14); }

BAML_TEST(return_string) {
    BAML_ASSERT_EQ(primitives::return_string(), std::string("hello"));
}

BAML_TEST(return_bool) { BAML_ASSERT(primitives::return_bool() == true); }

BAML_TEST(return_null) { (void)primitives::return_null(); }

BAML_TEST(round_trip_int) { BAML_ASSERT_EQ(primitives::round_trip_int(7), int64_t{7}); }

BAML_TEST(round_trip_float) { BAML_ASSERT_EQ(primitives::round_trip_float(2.5), 2.5); }

BAML_TEST(round_trip_float_accepts_int) {
    BAML_ASSERT_EQ(primitives::round_trip_float(7), 7.0);
}

BAML_TEST(round_trip_string) {
    BAML_ASSERT_EQ(primitives::round_trip_string("hi"), std::string("hi"));
}

BAML_TEST(round_trip_bool) { BAML_ASSERT(primitives::round_trip_bool(false) == false); }

BAML_TEST(round_trip_null) { (void)primitives::round_trip_null(std::monostate{}); }

BAML_TEST(round_trip_uint8_array) {
    const std::vector<uint8_t> bytes = {0x00, 0x01, 0x02};
    BAML_ASSERT(primitives::round_trip_uint8_array(bytes) == bytes);
}

BAML_TEST(round_trip_primitives) {
...
BAML_TEST(round_trip_primitives_float_field_accepts_int) {
