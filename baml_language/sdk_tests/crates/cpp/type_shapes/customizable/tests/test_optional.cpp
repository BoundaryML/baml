...
BAML_TEST(round_trip_optional_int) {
    BAML_ASSERT(baml_sdk::optional::round_trip_optional_int(int64_t{5}) == int64_t{5});
    BAML_ASSERT(!baml_sdk::optional::round_trip_optional_int(std::nullopt).has_value());
}

BAML_TEST(round_trip_optional_resume) {
    const Resume r{"ada"};
    BAML_ASSERT(baml_sdk::optional::round_trip_optional_resume(r) == r);
    BAML_ASSERT(!baml_sdk::optional::round_trip_optional_resume(std::nullopt).has_value());
}

BAML_TEST(round_trip_optional_union) {
    using U = std::variant<int64_t, std::string>;
    BAML_ASSERT(baml_sdk::optional::round_trip_optional_union(U{int64_t{3}}) == U{int64_t{3}});
    BAML_ASSERT(baml_sdk::optional::round_trip_optional_union(U{std::string("s")}) ==
                U{std::string("s")});
    BAML_ASSERT(!baml_sdk::optional::round_trip_optional_union(std::nullopt).has_value());
}
