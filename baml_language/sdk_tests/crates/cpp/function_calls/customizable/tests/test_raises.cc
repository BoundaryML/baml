...
static size_t FindOrFail(const std::string& text, const std::string& needle) {
...
BAML_TEST(union_throws_lists_all_names) {
  ...
  const size_t raises =
      FindOrFail(text, "/// Raises: ParseError, TimeoutError");
...
BAML_TEST(async_sibling_shares_raises_block) {
  ...
  const size_t raises =
      FindOrFail(text, "/// Raises: ParseError, TimeoutError");
  const size_t async_decl =
      text.find("::baml::Future<std::string> LoadDoc_async(", raises);
...
BAML_TEST(summary_precedes_raises_block) {
  const std::string text = HeaderText();
  const size_t summary = FindOrFail(text, "/// Load a document from a path.");
  const size_t raises =
      FindOrFail(text, "/// Raises: ParseError, TimeoutError");
...
BAML_TEST(inferred_contract_without_clause_still_raises) {
  const std::string text = HeaderText();
  const size_t decl = FindOrFail(text, "std::string InferredThrow(");
  const std::string before =
      text.substr(decl < 300 ? 0 : decl - 300, std::min(decl, size_t{300}));
  BAML_ASSERT(before.find("/// Raises: ParseError") != std::string::npos);
}

BAML_TEST(non_throwing_function_has_no_raises_block) {
  const std::string text = HeaderText();
  const size_t decl = FindOrFail(text, "int64_t PureLen(");
  const std::string before =
      text.substr(decl < 150 ? 0 : decl - 150, std::min(decl, size_t{150}));
  BAML_ASSERT(before.find("Raises:") == std::string::npos);
}

BAML_TEST(method_raises_blocks_in_struct) {
  const std::string text = HeaderText();
  const size_t struct_start = FindOrFail(text, "struct DocLoader {");
