...
BAML_TEST(identity_explicit) {
  ...
  const GenericBox<GenericBox<std::string>> box{
      GenericBox<std::string>{"hello"}};
  BAML_ASSERT((generic_tests::identity<GenericBox<GenericBox<std::string>>>(
                  box)) == box);
  ...
  BAML_ASSERT((generic_tests::identity<
                  GenericTriple<GenericBox<std::string>, double, bool>>(
                  triple)) == triple);
}
...
BAML_TEST(make_triple_explicit) {
  ...
  const GenericTriple<int64_t, std::string, bool> t =
      generic_tests::make_triple<int64_t, std::string, bool>(1, {"a", "b"},
                                                             {{"k", true}});
...
BAML_TEST(one_type_arg_explicit) {
  ...
  BAML_ASSERT_EQ(generic_tests::one_type_arg<std::string>(),
                 std::string("string"));
...
BAML_TEST(consume_int_wrapper_baseline) {
  ...
  BAML_ASSERT_EQ(generic_tests::consume_int_wrapper(GenericBox<int64_t>{9}),
                 int64_t{9});
}
...
BAML_TEST(named_static_distinct_typevar_names) {
  ...
  BAML_ASSERT_EQ(
      (NamedStatic<int64_t, int64_t, int64_t>::make<int64_t, std::string>(1,
                                                                          "x")),
      std::string("int | string"));
}
...
BAML_TEST(extract_explicit) {
  ...
  BAML_ASSERT_EQ(
      (generic_tests::extract<int64_t, std::string, bool, double>(pair)),
...
BAML_TEST(parse_as_explicit) {
  ...
  const StringIntPair pair = generic_tests::parse_as<StringIntPair>(
...
BAML_TEST(choose_explicit) {
  ...
  BAML_ASSERT_EQ(generic_tests::choose<std::string>("a", "b"),
                 std::string("a"));
}
...
BAML_TEST(make_nested_box_reified) {
  ...
  const GenericBox<GenericBox<int64_t>> outer =
      generic_tests::make_nested_box();
