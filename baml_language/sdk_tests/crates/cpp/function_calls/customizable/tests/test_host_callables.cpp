...
// `raise BamlError(ValidationError(...))` is baml::HostThrow<ValidationError>;
...
BAML_TEST(two_arg_callable_unpacks_positional_args) {
    const std::string result = baml_sdk::host_callable_tests::call_with_two_args(
        [](int64_t x, std::string prefix) { return prefix + ":" + std::to_string(x); }, 7,
        "answer");
...
BAML_TEST(throwing_callable_round_trips_original_host_exception) {
    ...
    // flattened into a BamlError(HostCallable(...)) wrapper.
    bool threw = false;
    try {
...
BAML_TEST(throwing_callable_custom_host_exception_round_trips_with_identity) {
    ...
    try {
        baml_sdk::host_callable_tests::call_with_callback(
            [](int64_t) -> std::string { throw MyDomainError("custom domain failure", 42); },
            1);
        baml_test::fail("call_with_callback did not throw");
...
BAML_TEST(throwing_callable_hostthrow_codegenned_class_is_caught_in_baml) {
    // A baml::HostThrow<ValidationError> crosses as the real BAML class on
    // the wire, so the BAML side's typed `catch (e: ValidationError)`
    // matches structurally and reads e.message as a real field.
    const std::string result = baml_sdk::host_callable_tests::call_with_typed_throws(
        [](int64_t) -> std::string {
            throw baml::HostThrow<ValidationError>(
                ValidationError{4, "bad shape", {"name", "age", "email", "phone"}});
        },
        1);
...
BAML_TEST(throwing_callable_hostthrow_propagates_back_with_typed_fields) {
    // The same HostThrow<ValidationError>, when not caught in BAML,
    // propagates back out as a BamlError whose payload decodes to the
...
BAML_TEST(class_callback_round_trips_class_value) {
    ...
    const std::string result = baml_sdk::host_callable_tests::call_with_class_callback(
...
BAML_TEST(call_repeatedly_invokes_callback_n_times) {
    ...
    const std::vector<std::string> results = baml_sdk::host_callable_tests::call_repeatedly(
    ...
    BAML_ASSERT(
        (results ==
         std::vector<std::string>{"item-0", "item-1", "item-2", "item-3", "item-4"}));
...
BAML_TEST(call_with_throwing_in_baml_catches_host_callable_error) {
    ...
    const std::string result = baml_sdk::host_callable_tests::call_with_throwing(
        [](int64_t) -> std::string { throw std::runtime_error("boom from host"); }, 1);
...
static int64_t optional_args_cb(int64_t x, baml::Arg<int64_t> y, baml::Arg<int64_t> z) {
...
BAML_TEST(optional_args_partially_set_deliver_by_name) {
    ...
    // is supplied.
    const std::vector<int64_t> results =
        baml_sdk::host_callable_tests::call_callback_with_optional_args_partially_set(
            optional_args_cb, 5);
