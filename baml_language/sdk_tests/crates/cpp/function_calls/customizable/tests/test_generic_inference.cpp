...
BAML_TEST(identity_infers_generic_instance) {
    ...
    const GenericBox<GenericBox<std::string>> nested{GenericBox<std::string>{"hello"}};
...
BAML_TEST(make_triple_infers_multiple_typevars) {
    ...
    const GenericTriple<int64_t, std::string, bool> t = generic_tests::make_triple(
        int64_t{1}, std::vector<std::string>{"a", "b"}, std::map<std::string, bool>{{"k", true}});
...
BAML_TEST(second_of_infers_from_nested_generic) {
    ...
    BAML_ASSERT_EQ(generic_tests::second_of(GenericPair<int64_t, std::string>{1, "hi"}),
                   std::string("hi"));
...
BAML_TEST(read_items_infers_from_instance_wire_args) {
    ...
    BAML_ASSERT(generic_tests::read_items(container) == (std::vector<int64_t>{1, 2, 3}));

    const ContainerShapes<int64_t> empty_fields{1, {}, {}, std::nullopt, std::nullopt};
    BAML_ASSERT(generic_tests::read_items(empty_fields) == (std::vector<int64_t>{}));
}

BAML_TEST(list_head_infers_from_recursive_generic) {
    // T11: GenericRecursive<T> bottoms out at next=nullopt; T deduced.
    const GenericRecursive<int64_t> linked{7, GenericRecursive<int64_t>{8, std::nullopt}};
...
BAML_TEST(extract_infers_four_typevars_from_nesting) {
    ...
    BAML_ASSERT_EQ(generic_tests::extract(pair), std::string("int | string | bool | float"));
}
...
BAML_TEST(choose_infers_unified_typevar) {
    ...
    BAML_ASSERT_EQ(generic_tests::choose(std::string("a"), std::string("b")), std::string("a"));
}
...
BAML_TEST(make_triple_partial_explicit_then_infer) {
    ...
    const GenericTriple<int64_t, std::string, bool> t = generic_tests::make_triple<int64_t>(
        1, std::vector<std::string>{"x", "y"}, std::map<std::string, bool>{{"k", true}});
    BAML_ASSERT_EQ(t.first, int64_t{1});
...
BAML_TEST(named_static_infers_distinct_typevars) {
    ...
    BAML_ASSERT_EQ((NamedStatic<int64_t, int64_t, int64_t>::make(int64_t{1}, std::string("x"))),
...
BAML_TEST(union_with_concrete_sibling_infers_typevar) {
    ...
    BAML_ASSERT_EQ(generic_tests::tag_or_value(std::optional<std::variant<int64_t, std::string>>{
...
BAML_TEST(pair_invariant_list_agree_binds) {
    ...
    BAML_ASSERT_EQ(generic_tests::pair(std::vector<int64_t>{1, 2}, std::vector<int64_t>{3, 4}),
                   std::string("int"));
}
...
BAML_TEST(make_triple_heterogeneous_list_element_unions) {
    ...
    const GenericTriple<int64_t, IntOrString, bool> t = generic_tests::make_triple(
        int64_t{1}, std::vector<IntOrString>{int64_t{1}, std::string("x")},
        std::map<std::string, bool>{{"k", true}});
    BAML_ASSERT(t.second == (std::vector<IntOrString>{int64_t{1}, std::string("x")}));
}
...
BAML_TEST(values_of_nonempty_returns_values) {
    ...
    BAML_ASSERT(generic_tests::values_of(std::map<std::string, int64_t>{{"a", 1}, {"b", 2}}) ==
...
BAML_TEST(make_triple_subscript_fully_bound) {
    ...
    const GenericTriple<int64_t, std::string, bool> t =
        generic_tests::make_triple<int64_t, std::string, bool>(5, {"x"}, {{"k", true}});
...
BAML_TEST(maybe_id_present_value_infers) {
    ...
    BAML_ASSERT(generic_tests::maybe_id(std::optional<int64_t>{5}) == std::optional<int64_t>{5});
}
...
BAML_TEST(elem_type_homogeneous_array_is_single_type) {
    ...
    BAML_ASSERT_EQ(generic_tests::elem_type(std::vector<int64_t>{1, 2, 3}), std::string("int"));
}
