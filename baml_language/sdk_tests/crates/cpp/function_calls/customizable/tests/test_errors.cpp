...
BAML_TEST(baml_error_carries_baml_trace) {
    ...
    try {
        ...
        const std::regex frame(
            "File \"[^\"]*types\\.baml\", line [0-9]+, in user\\.throws_test\\.ThrowMyError");
...
#ifndef _WIN32
static int run_self_child(const char* name) {
...
BAML_TEST(clean_exit_terminates_process_with_code) {
    BAML_ASSERT_EQ(run_self_child("do_exit_0"), 0);
    BAML_ASSERT_EQ(run_self_child("do_exit_7"), 7);
}
#endif
