...
// intrinsics check greps the single generated header and bindings.cpp
...
// binary.
static std::string generated_sdk_file(const std::string& rel_path) {
...
BAML_TEST(compiler_intrinsics_are_not_emitted_as_entry_points) {
    const char* files[] = {
        "baml_sdk/include/baml_sdk.hpp",
        "baml_sdk/src/bindings.cpp",
    };
    ...
    for (const char* file : files) {
        const std::string contents = generated_sdk_file(file);
        if (contents.empty()) {
            continue;
        }
        saw_any_file = true;
