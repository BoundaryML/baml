...
// hitting OpenAI: ReplayServer (see replay_harness.hpp) replays a
...
BAML_TEST(stream_async) {
    ...
    replay::ReplayServer server("replay_extract_string");
    auto stream = lorem::stream_e2e_extract_stream_async("ignored-by-replay-server").get();
    int results = 0;
    while (true) {
...
BAML_TEST(stream_doc) {
    ...
    auto stream = lorem::stream_e2e_extract_doc_stream("ignored-by-replay-server");
    int results = 0;
...
BAML_TEST(stream_doc_async) {
    replay::ReplayServer server("replay_extract_doc");
    auto stream =
        lorem::stream_e2e_extract_doc_stream_async("ignored-by-replay-server").get();
    int results = 0;
    while (true) {
        const auto v = stream.next_async().get();
        if (std::holds_alternative<StreamFinished>(v)) {
