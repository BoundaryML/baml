#ifndef REPLAY_HARNESS_HPP
#define REPLAY_HARNESS_HPP
...
inline std::string recording_path(const std::string& name) {
...
inline void post_empty(const std::string& host_port, const std::string& path) {
...
public:
    explicit ReplayServer(const std::string& recording) {
        const std::string rec = recording_path(recording);
    ...
    ~ReplayServer() {
        post_empty(addr_, "/__replay__/shutdown");
        if (thread_.joinable()) {
            thread_.join();
        }
        std::remove(addr_file_.c_str());
