...
describe("roundtrip forward_refs", () => {
  ...
  it("test_round_trip_g_node_int", () => {
    // Leaf node carries children=[] — exercises the empty-list round trip.
    const g = new GNode<number>({
      children: [new GNode<number>({ children: [] })],
    });
