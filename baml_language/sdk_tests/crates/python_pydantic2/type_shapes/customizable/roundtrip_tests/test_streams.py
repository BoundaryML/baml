...
These are the riskiest shapes in the fixture:

* `$stream` companion types (`Resume$stream`, `Foo$stream`) are normally
  engine-internal *partial* values. Whether a host-constructed
  `stream_types.*` pydantic model can be encoded and round-tripped
  through a `$stream`-typed parameter is unverified — these tests probe
  exactly that.
* `baml.http.Response` carries a `_body: _BamlPyHandle` field that only
  the engine can mint, so any function taking a `Response` (bare, in a
  list, or as a `$stream`) can't be driven from pure Python. Those are
  skipped with a reason but imported, proving the symbols emit.

If a `$stream` probe fails with an encode/decode/type-mismatch error
from the bridge, that's recorded in 35b as a bridge-surface limitation
...
from baml_sdk.lorem import (
    ...
    round_trip_baml_http_response_stream,  # noqa: F401 — handle-backed; import-only
    round_trip_box_of_resume_stream,
    round_trip_list_of_http_response,  # noqa: F401 — handle-backed; import-only
...
_HANDLE_REASON = (
    "baml.http.Response carries a `_body: _BamlPyHandle` that only the "
    "engine can mint; the value can't be constructed host-side."
)
...
@pytest.mark.skip(reason=_HANDLE_REASON)
def test_round_trip_baml_http_response_stream():
    pass


@pytest.mark.skip(reason=_HANDLE_REASON)
def test_round_trip_list_of_http_response():
    pass
