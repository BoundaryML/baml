...
if (args.Length == 2 && args[0] == ExitChildArgument)
{
    ...
    Console.WriteLine("phase7_exit_before");
    Console.Out.Flush();
    try
    {
    ...
    {
        Console.WriteLine("phase7_exit_unreachable_finally");
    }
...
Require(
    error.ErrorName == "user.csharp_phase7.Failure"
        && error.BamlFunction == "user.csharp_phase7.throw_failure"
...
Require(
    mismatch.ErrorName == "baml.errors.TypeMismatch"
        && mismatch.BamlFunction == "user.csharp_phase7.throw_type_mismatch",
...
Require(
    panic.BamlFunction == "user.csharp_phase7.do_panic"
...
using (var caller = new CancellationTokenSource())
{
    ...
    Require(
        cancellation.Origin == BamlCancellationOrigin.Caller
            && cancellation.BamlFunction == "user.csharp_phase7.sleep_ms"
...
using (var uncanceledCaller = new CancellationTokenSource())
{
    ...
    Require(
        cancellation.Origin == BamlCancellationOrigin.Engine
            && cancellation.BamlFunction == "user.csharp_phase7.engine_cancel"
...
Console.WriteLine("csharp_phase7_failures=ok");
return 0;
...
static async Task VerifyHardExitChildAsync(int exitCode)
{
    ...
    Require(output.Contains("phase7_exit_before", StringComparison.Ordinal),
        "hard-exit child omitted its pre-exit marker");
    Require(!output.Contains("phase7_exit_unreachable_finally", StringComparison.Ordinal),
