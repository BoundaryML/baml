...
#[test]
fn test_round_trip_root_foo() {
    let f = Foo { v: 6 };
    assert_eq!(round_trip_root_foo(f.clone()).unwrap(), f);
}

#[test]
fn test_round_trip_lorem_resume_from_ipsum() {
    let r = Resume {
