...
//! `crates/cpp/<fixture>/generated/baml_sdk/` (via `sdkgen_cpp`, currently a
//! stub — failures land in build diagnostics), symlinks
...
//!
//! While `sdkgen_cpp` is a stub, every emitted test carries
//! `#[ignore = "sdkgen_cpp is a stub"]` (the same wholesale-ignore state
//! typescript_node shipped with before its emitter landed). Drop the ignores
//! in [`write_fixtures_tests_rs`] when the emitter produces real output.
...
const STUB_IGNORE_REASON: &str = "sdkgen_cpp is a stub";
...
fn write_fixtures_tests_rs(out_dir: &Path, fixtures: &[String]) {
    ...
    buf.push_str(&format!(
        "::sdk_test_harness_runner::build_diagnostics!(ignore = {STUB_IGNORE_REASON:?});\n"
    ));
    buf.push_str(&format!(
        "::sdk_test_harness_runner::setup_guard!(ignore = {STUB_IGNORE_REASON:?}, {SETUP_ENV_VAR:?});\n"
    ));
    for fixture in fixtures {
        buf.push_str(&format!(
            r#"
...
"#,
            ...
            ignore_reason = STUB_IGNORE_REASON,
        ));
    }
