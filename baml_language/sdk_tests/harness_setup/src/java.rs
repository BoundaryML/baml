...
//!
//! `sdkgen_java` is currently a stub, so codegen runs under
//! `catch_unwind` (with the panic hook silenced) and a panic is
//! downgraded to a `codegen` record in `$OUT_DIR/build_diagnostics.txt`
//! — mirroring how `typescript_node` behaved before its emitter
//! landed. The emitted scaffold `#[ignore]`s `build_diagnostics`,
//! `setup_guard`, and every per-fixture test with [`IGNORE_REASON`];
//! un-ignore them (and run codegen directly, aborting loudly on
//! panic) once the Java emitter is real.
//!
...
const IGNORE_REASON: &str = "sdkgen_java is a stub — un-ignore as the Java emitter lands";
...
fn codegen_fixture(
    ...
    // `sdkgen_java` is a stub that panics. Run it under catch_unwind with
    // the panic hook silenced (four fixtures would otherwise print four
    // backtraces on every build) and record a soft `codegen` failure,
    // leaving `baml_sdk/` empty. Switch to a direct call — like
    // `typescript_node.rs` — once the emitter lands, so real codegen bugs
    ...
    let output = match codegen_result {
        Ok(output) => output,
        Err(_) => {
            diagnostics.record(
                "codegen",
                fixture,
                "sdkgen_java::to_source_code_with_bytecode panicked (stub)",
            );
            Default::default()
        }
    };
