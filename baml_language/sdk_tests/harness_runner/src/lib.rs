...
pub fn assert_typescript_web_generated_esm(fixture: &str, runtime_dir: &str) {
    ...
    // The bytecode is emitted as a single base64 line; an empty program would
    // still emit the declaration, so non-emptiness means a populated literal.
    let has_bytecode_payload = inlined_bytecode.lines().any(|line| {
        line.starts_with("const BYTECODE_BASE64 = \"")
            && line.len() > "const BYTECODE_BASE64 = \"\";".len()
    });
    assert!(
