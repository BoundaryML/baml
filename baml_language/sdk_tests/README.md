...
cargo nextest run -p sdk_test_python_pydantic2 -p sdk_test_typescript
...
- **Generator directory** (under `crates/`): lowercase snake (`python_pydantic2`, `typescript`). The TypeScript crate uses the Cargo package name `sdk_test_typescript` because it covers the full runtime matrix.
