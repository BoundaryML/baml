...
/**
 * Host-callable dispatch: BAML invokes a host closure.
 * Mirrors `sys_native::host_dispatch::HostDispatchFn`.
 * Complete each dispatched call_id exactly once via `complete_host_call`;
 * never issue a blocking BAML call from inside the callback (deadlock).
 */
typedef void (*HostDispatchFn)(uint64_t host_value_key, uint32_t call_id, const uint8_t *args, uintptr_t length);

/**
 * Drop-on-last-clone notification fired to the host language.
 * Mirrors `bex_resource_types::host_value::HostReleaseFn`.
 */
