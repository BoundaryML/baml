...
/// The bytecode payload (an LZ4 frame, which the bridge decompresses),
/// base64-encoded, as ONE single-line string literal. Two rejected
/// alternatives, both fatal at engine sizes: a `[UInt8]` literal
/// type-checks element-by-element, and a `"…" + "…"` chunk chain builds a
/// `+` expression whose type-check is super-linear in the number of chunks
/// (observed: 55+ minutes for a multi-MB payload). A single literal is a
/// single token — instant.
...
mod tests {
    ...
    #[test]
    fn bytecode_payload_round_trips_via_base64_lz4() {
        ...
        assert_eq!(decoded.as_ref(), bytecode.as_slice());
