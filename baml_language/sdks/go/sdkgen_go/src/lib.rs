...
fn is_stream_companion(function: &Function) -> bool {
...
fn render_function_options(
    ...
    if is_stream_companion(function.function) {
        for (control, surface, encoder) in [
            (
                "client",
                "any".to_string(),
                format!("{runtime_package}.Any"),
            ),
            (
                "on_event",
                format!("{runtime_package}.EventCallback"),
                format!("{runtime_package}.EventCallbackInput"),
            ),
        ] {
            let control = BaseName::new(control);
            let setter = context
                .names
                .project(
                    &function.fqn.member(&control),
            ...
            let _ = writeln!(
                out,
                "\nfunc {setter}({value_parameter} {surface}) {option_type} {{"
            );
            let _ = writeln!(
                out,
                "\treturn {runtime_package}.WithArgument({:?}, {encoder}({value_parameter}))",
                control.as_str()
            );
            out.push_str("}\n");
        }
