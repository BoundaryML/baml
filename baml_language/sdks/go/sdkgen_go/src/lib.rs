...
use rendering::{ClassCodecDirection, ClassCodecIdent, GeneratorIdent, GoImports};
...
pub fn to_source_code_with_bytecode(
    ...
    for package in packages.iter() {
        ...
        let codecs = ClassCodecs::for_functions(&functions, pool);
...
impl ClassCodecs {
    fn for_functions(functions: &[GeneratedFunction<'_>], pool: &SymbolPool) -> Self {
        ...
        for routed in functions {
            ...
            {
                collect_codec_classes(ty, pool, &mut names);
            }
        }
...
fn collect_codec_classes(ty: &Ty, pool: &SymbolPool, names: &mut BTreeSet<Name>) {
    ...
    for property in &class.properties {
        collect_codec_classes(&property.ty, pool, names);
    }
}
...
fn input_expression(ty: &Ty, value: &str, codecs: &ClassCodecs) -> String {
    let runtime_package = GeneratorIdent::RuntimePackage;
    if primitive_kind(ty).is_some() {
...
fn output_expression(ty: &Ty, value: &str, codecs: &ClassCodecs) -> String {
    let runtime_package = GeneratorIdent::RuntimePackage;
    if primitive_kind(ty).is_some() {
