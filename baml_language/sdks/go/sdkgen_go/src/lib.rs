...
fn is_ai_prompt(name: &Name) -> bool {
    name.to_string() == AI_PROMPT
}
...
fn public_interface_tokens(pool: &SymbolPool) -> BTreeSet<Name> {
    fn collect_function(function: &Function, out: &mut BTreeSet<Name>) {
        ...
        if let Some(spec) = &function.operations.spec {
            collect_interface_tys(&spec.return_type, out);
        }
        if let Some(stream) = &function.operations.stream {
            collect_interface_tys(&stream.return_type, out);
            collect_interface_tys(&stream.partial_type, out);
            collect_interface_tys(&stream.item_type, out);
            for argument in &stream.control_arguments {
                collect_interface_tys(&argument.ty, out);
            }
        }
    }
...
impl WireCodecs {
    ...
    fn for_functions(
        ...
        for routed in functions {
            ...
            if let Some(spec) = &routed.function.operations.spec {
                collect_projected_codec_types(
                    &projection.project(&spec.return_type),
                    current_package,
                    pool,
                    projection,
                    &mut collected,
            ...
            if let Some(stream) = &routed.function.operations.stream {
                for ty in [&stream.return_type, &stream.partial_type, &stream.item_type] {
                    collect_projected_codec_types(
                        &projection.project(ty),
                        current_package,
                        pool,
                ...
                for argument in &stream.control_arguments {
                    collect_projected_codec_types(
                        &projection.project(&argument.ty),
                        current_package,
                        pool,
                        projection,
                        &mut collected,
...
fn renderable_class_names(pool: &SymbolPool, projection: &GoTypeProjection<'_>) -> BTreeSet<Name> {
    ...
    loop {
        let next = candidates
            .iter()
            .filter(|name| {
                if is_ai_prompt(name) {
                    // ai.Prompt is a runtime-owned portable facade. Its
                    // implementation-only fields are never rendered or
                    // encoded by generated Go, so unsupported internal field
                    // shapes must not suppress the public alias.
                    return true;
                }
                let Symbol::Class(class) = &pool[*name] else {
                    unreachable!()
...
fn type_var_candidate_reifiable(ty: &GoTy, classes: &BTreeSet<Name>) -> bool {
    ...
    match ty {
        ...
        GoTy::FunctionSpec { output } => type_var_candidate_reifiable(output, classes),
        GoTy::Stream { partial, final_ } => {
            type_var_candidate_reifiable(partial, classes)
                && type_var_candidate_reifiable(final_, classes)
        }
...
fn generic_class_argument_reifiable(ty: &GoTy, classes: &BTreeSet<Name>) -> bool {
    match ty {
        ...
        GoTy::FunctionSpec { output } => generic_class_argument_reifiable(output, classes),
        GoTy::Stream { partial, final_ } => {
            generic_class_argument_reifiable(partial, classes)
                && generic_class_argument_reifiable(final_, classes)
        }
        GoTy::List(inner) | GoTy::Optional(inner) => {
...
fn supported_function(
    ...
    function.arguments.iter().all(|arg| {
        arg.injected || supported_function_argument(&arg.ty, wireable_classes, projection)
    }) && (function.generic_params.is_empty()
        || (function
            .arguments
            .iter()
            .all(|argument| argument.injected || argument.default.is_none())
            && function.arguments.iter().all(|argument| {
                argument.injected || !matches!(projection.project(&argument.ty), GoTy::Function(_))
            })))
...
fn render_functions(
    ...
    for routed in functions {
        ...
        out.push_str("}\n");
        render_flat_function_operations(&mut out, routed, &codec_context, render);
    }

    out
}

fn render_flat_function_operations(
    out: &mut String,
    routed: &GeneratedFunction<'_>,
    codecs: &CodecRenderContext<'_, '_>,
    render: &FunctionRenderContext<'_>,
) {
    if routed.owner_name.is_some() {
        return;
    }
    let Some(spec_operation) = &routed.function.operations.spec else {
        return;
    };

    let runtime = GeneratorIdent::RuntimePackage;
    let context_package = GeneratorIdent::ContextPackage;
    let bootstrap = GeneratorIdent::BootstrapPackage;
    let context_parameter = GeneratorIdent::ContextParameter;
    let error_local = GeneratorIdent::ErrorLocal;
    let result_local = GeneratorIdent::ResultLocal;
    let zero_local = GeneratorIdent::ZeroLocal;
    ...
    let current_package = render.current_package;
    let current_baml_package = render.current_baml_package;
    let type_vars = codecs.type_vars;

    let generic_names = routed
        .function
        .generic_params
        .iter()
        .map(|parameter| {
        ...
        .collect::<Vec<_>>();
    let type_parameter_clause = if generic_names.is_empty() {
        String::new()
    } else {
        format!(
            "[{}]",
    ...
    let arguments = routed
        .arguments
        .iter()
        .filter(|argument| !argument.argument.injected && argument.argument.default.is_none())
        .collect::<Vec<_>>();
    let defaulted_arguments = routed
        .arguments
        .iter()
        .filter(|argument| !argument.argument.injected && argument.argument.default.is_some())
        .collect::<Vec<_>>();
    let params = std::iter::once(format!("{context_parameter} {context_package}.Context"))
    ...
    let arguments_expr = if arguments.is_empty() {
        format!("map[string]{runtime}.Input{{}}")
    } else {
    ...
    let spec_type = function_return_go_type(
        &spec_operation.return_type,
        current_baml_package,
        current_package,
        codecs.names,
        codecs.projection,
    ...
    let spec_decoder = projected_output_decoder(
        &codecs.projection.project(&spec_operation.return_type),
        current_baml_package,
        current_package,
        codecs.names,
        codecs.codecs,
    ...
    let direct_name = routed.go_name.identifier(current_package);
    let spec_name = format!("{direct_name}Spec");
    let spec_option_type = format!("{spec_name}Option");
    let wire_name = routed.go_name.wire().to_string();

    if !defaulted_arguments.is_empty() {
        let _ = writeln!(
            out,
            "\n// {spec_option_type} configures authored defaulted arguments while constructing the FunctionSpec.\ntype {spec_option_type} = {runtime}.CallOption"
        );
        for argument in &defaulted_arguments {
            let suffix = exported_identifier_component(
                &argument.go_name.identifier(current_package).to_string(),
            );
            let setter = format!("{spec_name}With{suffix}");
            let _ = writeln!(
                out,
                "\n// {setter} overrides the authored {wire} argument.\nfunc {setter}(value_ {surface}) {spec_option_type} {{\n\treturn {runtime}.WithArgument({wire:?}, {encoder})\n}}",
                wire = argument.go_name.wire().to_string(),
                surface = function_go_type(
                    &argument.argument.ty,
                    current_baml_package,
                    current_package,
                    codecs.names,
                ...
                encoder = input_expression(&argument.argument.ty, "value_", codecs),
            );
        }
    }

    let spec_params = if defaulted_arguments.is_empty() {
        params.clone()
    } else {
        format!("{params}, options_ ...{spec_option_type}")
    };

    let _ = writeln!(
        out,
        "\n// {spec_name} returns the bound FunctionSpec for {direct_name}.\nfunc {spec_name}{type_parameter_clause}({spec_params}) ({spec_type}, error) {{"
    );
    let _ = writeln!(
        out,
        "\tif {error_local} := {bootstrap}.Ensure(); {error_local} != nil {{"
    );
    let _ = writeln!(out, "\t\tvar {zero_local} {spec_type}");
    let _ = writeln!(out, "\t\treturn {zero_local}, {error_local}\n\t}}");
    let _ = writeln!(out, "\t{arguments_local} := {arguments_expr}");
    if !defaulted_arguments.is_empty() {
        let _ = writeln!(
            out,
            "\t{runtime}.ApplyCallOptions({arguments_local}, nil, options_...)"
        );
    }

    if routed.function.generic_params.is_empty() {
        let _ = writeln!(
            out,
            "\t{result_local}, {error_local} := {runtime}.CallOperation({context_parameter}, {wire_name:?}, {runtime}.FunctionOperationSpec, {arguments_local})"
        );
    } else {
        let bindings = routed
            .function
        ...
        let _ = writeln!(
            out,
            "\t{result_local}, {error_local} := {runtime}.CallWithTypeArgsOperation({context_parameter}, {wire_name:?}, {runtime}.FunctionOperationSpec, {arguments_local}, []{runtime}.TypeArgument{{{bindings}}})"
        );
    }
    let _ = writeln!(out, "\tif {error_local} != nil {{");
    let _ = writeln!(out, "\t\tvar {zero_local} {spec_type}");
    let _ = writeln!(out, "\t\treturn {zero_local}, {error_local}\n\t}}");
    let _ = writeln!(out, "\treturn {spec_decoder}({result_local})\n}}");

    let Some(stream_operation) = &routed.function.operations.stream else {
        return;
    };
    let stream_type = function_return_go_type(
        &stream_operation.return_type,
        current_baml_package,
        current_package,
        codecs.names,
        codecs.projection,
    ...
    let stream_decoder = projected_output_decoder(
        &codecs.projection.project(&stream_operation.return_type),
        current_baml_package,
        current_package,
        codecs.names,
        codecs.codecs,
    ...
    let stream_name = format!("{direct_name}Stream");
    let stream_option_type = format!("{stream_name}Option");
    let has_stream_controls = !stream_operation.control_arguments.is_empty();
    let has_stream_options = !defaulted_arguments.is_empty() || has_stream_controls;
    if has_stream_options {
        let _ = writeln!(
            out,
            "\n// {stream_option_type} configures authored arguments and stream controls.\ntype {stream_option_type} = {runtime}.CallOption"
        );
        ...
        for argument in &defaulted_arguments {
            ...
            let setter = format!("{stream_name}With{suffix}");
            let _ = writeln!(
                out,
                "\n// {setter} overrides the authored {wire} argument before streaming.\nfunc {setter}(value_ {surface}) {stream_option_type} {{\n\treturn {runtime}.WithArgument({wire:?}, {encoder})\n}}",
                wire = argument.go_name.wire().to_string(),
                surface = function_go_type(
                    &argument.argument.ty,
                    current_baml_package,
        ...
        for control in &stream_operation.control_arguments {
            let suffix = match control.name.as_str() {
                "client" => "Client",
                "on_event" => "OnEvent",
                other => panic!("unsupported flat stream control {other:?}"),
            };
            let setter = format!("{stream_name}{suffix}");
            let projected = codecs.projection.project(&control.ty);
            let value_type = match &projected {
                GoTy::Optional(inner) => inner.as_ref(),
                other => other,
            };
            let (surface, encoder) =
                if control.name.as_str() == "on_event" && matches!(value_type, GoTy::Unsupported) {
                    (
                        format!("{runtime}.EventCallback"),
                        format!("{runtime}.EventCallbackInput"),
                    )
                } else if matches!(value_type, GoTy::Unsupported) {
                    ("any".to_string(), format!("{runtime}.Any"))
                } else {
                    (
            ...
            let _ = writeln!(
                out,
                "\n// {setter} overrides the stream {wire} control.\nfunc {setter}(value_ {surface}) {stream_option_type} {{\n\treturn {runtime}.WithArgument({wire:?}, {encoder}(value_))\n}}",
                wire = control.name.as_str(),
            );
        }
    }
    let stream_params = if has_stream_options {
        format!("{params}, options_ ...{stream_option_type}")
    } else {
        params.clone()
    };
    let _ = writeln!(
        out,
        "\n// {stream_name} starts the stream projection of {direct_name}.\nfunc {stream_name}{type_parameter_clause}({stream_params}) ({stream_type}, error) {{"
    );
    let _ = writeln!(
        out,
        "\tif {error_local} := {bootstrap}.Ensure(); {error_local} != nil {{"
    );
    let _ = writeln!(out, "\t\tvar {zero_local} {stream_type}");
    let _ = writeln!(out, "\t\treturn {zero_local}, {error_local}\n\t}}");
    let _ = writeln!(out, "\t{arguments_local} := {arguments_expr}");
    if has_stream_options {
        let _ = writeln!(
    ...
    if routed.function.generic_params.is_empty() {
        let _ = writeln!(
            out,
            "\t{result_local}, {error_local} := {runtime}.CallOperation({context_parameter}, {wire_name:?}, {runtime}.FunctionOperationStream, {arguments_local})"
        );
    } else {
        let bindings = routed
            .function
        ...
        let _ = writeln!(
            out,
            "\t{result_local}, {error_local} := {runtime}.CallWithTypeArgsOperation({context_parameter}, {wire_name:?}, {runtime}.FunctionOperationStream, {arguments_local}, []{runtime}.TypeArgument{{{bindings}}})"
        );
    }
    let _ = writeln!(out, "\tif {error_local} != nil {{");
    let _ = writeln!(out, "\t\tvar {zero_local} {stream_type}");
    let _ = writeln!(out, "\t\treturn {zero_local}, {error_local}\n\t}}");
    let _ = writeln!(out, "\treturn {stream_decoder}({result_local})\n}}");
}

fn exported_identifier_component(identifier: &str) -> String {
...
fn projected_output_decoder(
    ...
    match ty {
        ...
        GoTy::Class(name, _) if is_ai_prompt(name) => format!("{runtime}.Value.Prompt"),
        GoTy::Class(name, arguments) => {
            if arguments.is_empty() {
...
fn projected_contains_class(ty: &GoTy) -> bool {
    match ty {
        GoTy::Class(..) => true,
        GoTy::FunctionSpec { output } => projected_contains_class(output),
        GoTy::Stream { partial, final_ } => {
            projected_contains_class(partial) || projected_contains_class(final_)
        }
...
fn projected_contains_type_var(ty: &GoTy) -> bool {
    match ty {
        ...
        GoTy::FunctionSpec { output } => projected_contains_type_var(output),
        GoTy::Stream { partial, final_ } => {
            projected_contains_type_var(partial) || projected_contains_type_var(final_)
        }
...
fn baml_type_descriptor(ty: &GoTy, names: &GoNames) -> String {
    ...
    match ty {
        ...
        GoTy::Class(name, arguments) => {
            if is_ai_prompt(name) {
                return format!("{runtime}.PromptBAMLType()");
            }
            let wire_name = names
                .project(
                    &BamlFqn::symbol(name),
...
mod tests {
    ...
    use baml_codegen_types::{
        ...
        Freshness, FunctionArgument, FunctionArgumentDefault, FunctionOperations, Name, Origin,
    ...
    #[test]
    fn llm_spec_and_stream_options_dispatch_authored_defaults_and_controls() {
        let name = Name::new(BaseName::new("user"), vec![], BaseName::new("extract"));
        let spec_type = ty_class(
            Name::new(BaseName::new("ai"), vec![], BaseName::new("FunctionSpec")),
            vec![ty_string()],
        );
        let stream_type = ty_class(
            Name::new(
                BaseName::new("ai"),
                ...
                stream: Some(StreamOperation {
                    ...
                    control_arguments: vec![
                        FunctionArgument {
                            ...
                            docstring: None,
                            ty: ty_union(vec![ty_string(), ty_null()]),
                        ...
                        FunctionArgument {
                            ...
                            docstring: None,
                            ty: ty_union(vec![on_event, ty_null()]),
                            default: Some(FunctionArgumentDefault::Null),
                        },
                    ],
                }),
        ...
        assert!(
            functions.contains("type ExtractSpecOption = baml_go.CallOption"),
            "{functions}"
        );
        assert!(
            functions.contains("func ExtractSpecWithTone(value_ string) ExtractSpecOption"),
            "{functions}"
        );
        assert!(
            functions.contains(
                "func ExtractSpec(ctx_ context.Context, text string, options_ ...ExtractSpecOption)"
            ),
            "{functions}"
        );
        assert!(
            functions.contains("baml_go.ApplyCallOptions(arguments_, nil, options_...)"),
            "{functions}"
        );
        assert!(
            functions.contains("baml_go.FunctionOperationSpec, arguments_)"),
            "{functions}"
        );

        assert!(
            functions.contains("type ExtractStreamOption = baml_go.CallOption"),
            "{functions}"
        );
        assert!(
            functions.contains("func ExtractStreamWithTone(value_ string) ExtractStreamOption"),
            "{functions}"
        );
        assert!(
            functions.contains("return baml_go.WithArgument(\"tone\", baml_go.String(value_))"),
            "{functions}"
        );
        assert!(
            functions.contains("func ExtractStreamClient("),
            "{functions}"
        );
        assert!(
            functions.contains("func ExtractStreamOnEvent("),
            "{functions}"
        );
        assert!(
            functions.contains("func ExtractStream(ctx_ context.Context, text string, options_ ...ExtractStreamOption)"),
            "{functions}"
        );
        assert!(
            functions.contains("baml_go.FunctionOperationStream, arguments_)"),
            "{functions}"
        );
        assert!(!functions.contains("spec_.Stream("), "{functions}");
    }
