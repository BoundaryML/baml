...
fixture="$language_root/sdk_tests/crates/csharp/basic_calls"
...
test -f "$generated_source_root/CsharpBasicCalls/Functions.g.cs"
...
mkdir -p "$mismatch"
cp "$test_dir/Baml.Bridge.NuGetPackageSmoke.csproj" "$mismatch/"
...
if env NUGET_PACKAGES="$dotnet_mismatch_packages" \
  dotnet restore "$mismatch/Baml.Bridge.NuGetPackageSmoke.csproj" \
...
echo "csharp_nuget_package_smoke=ok"
