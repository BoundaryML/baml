...
//!
//! Each media class wraps a `BamlHandle` whose backing `HANDLE_TABLE` row
//! is a `CffiHandleTableEntry::Adt(BexExternalAdt::Media(arc))`. Static
//! constructors (`fromUrl`/`fromFile`/`fromBase64`) and instance methods
//! (`url`/`file`/`base64`/`mimeType`) dispatch natively here.
