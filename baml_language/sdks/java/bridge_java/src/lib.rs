...
/// error handling already lives in `bridge_cffi::invoke_prepared`.
fn call_sync_to_bytes(args_proto: &[u8]) -> Vec<u8> {
    ...
    rt.block_on(bridge_cffi::invoke_prepared(runtime, decoded))
}
...
fn spawn_async_call(call_id: u64, args_proto: Vec<u8>) {
    ...
    rt.spawn(async move {
        ...
        let inner = tokio::spawn(bridge_cffi::invoke_prepared(runtime, decoded));
