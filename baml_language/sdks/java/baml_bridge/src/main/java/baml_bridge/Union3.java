 ...
 * multi-arm BAML union of <em>3</em> arms renders as {@code Union3}, a
 ...
 * stripped). Every arm record declares <em>all</em> 3 type parameters and
 * wraps only its own positional arm's type, so Java&nbsp;21+ consumers get an
 * exhaustive {@code switch} with record patterns
 * (e.g. {@code case Union3.Arm0(var x) -> ...}); Java&nbsp;17 consumers use
...
public sealed interface Union3<T0, T1, T2> extends BamlUnion permits Union3.Arm0, Union3.Arm1, Union3.Arm2 {
    record Arm0<T0, T1, T2>(T0 value) implements Union3<T0, T1, T2> {}
    record Arm1<T0, T1, T2>(T1 value) implements Union3<T0, T1, T2> {}
    record Arm2<T0, T1, T2>(T2 value) implements Union3<T0, T1, T2> {}
}
