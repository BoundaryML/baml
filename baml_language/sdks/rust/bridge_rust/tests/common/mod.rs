//! Shared support for the integration tests: locating the
//! workspace-built engine cdylib. (Mirror of `src/test_support.rs`, which
//! is `#[cfg(test)]`-private and not visible here.)
...
pub(crate) fn locate_dev_engine() {
    ...
    ONCE.call_once(|| {
        ...
        baml_bridge::loader::set_shared_library_path(path);
    });
}
