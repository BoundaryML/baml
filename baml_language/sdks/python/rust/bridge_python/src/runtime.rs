                ...
                Ok((runtime, decoded)) => {
                    ...
                    match decoded.target {
                        bridge_ctypes::baml_bridge::cffi::call_function_args::CallTarget::FunctionName(function_name) => {
                            bridge_cffi::call_operation_and_encode(runtime, function_name, decoded.operation, decoded.kwargs, call_ctx).await
                        }
                        bridge_ctypes::baml_bridge::cffi::call_function_args::CallTarget::FunctionHandle(handle_key) => {
                            bridge_cffi::call_handle_operation_and_encode(runtime, handle_key, decoded.operation, decoded.kwargs, call_ctx).await
                        }
                    }
                }
