...
/// Full installation banner for the SDK's root runtime module. The lint
/// directives must appear at the top of every generated Python source file.
...
/// Repeated across the SDK's many leaf and data modules. Keep tool directives
/// and the edit warning here; installation instructions remain in the root.
const PYTHON_LEAF_BANNER: &str = "\
# ruff: noqa
# flake8: noqa
# pylint: skip-file
# mypy: ignore-errors
...
# Generated by BAML; edit the BAML source and regenerate.

";

/// Full root prefix, including `from __future__ import annotations`.
#[cfg(test)]
...
fn to_source_code_internal(
    ...
    // Keep installation help at the root without repeating it in every leaf.
    for (path, content) in &mut out {
        ...
        if is_python {
            let banner = if path == &PathBuf::from("__init__.py") {
                PYTHON_BANNER
            } else {
                PYTHON_LEAF_BANNER
            };
...
/// Render bytes as adjacent Python bytes literals. Larger chunks reduce the
/// generated source line count while preserving the decoded byte sequence.
...
mod tests {
    ...
    fn empty_pool_emits_structural_files() {
        ...
        assert_eq!(
            baml_init.as_str(),
            format!("{PYTHON_LEAF_BANNER}from __future__ import annotations\n")
        );
    ...
    fn generated_sdk_lists_python_runtime_install_commands_in_preferred_order() {
        ...
        assert!(root.contains(expected));
        assert!(
            root.contains(
                "#  Welcome to Baml! To use this generated code, run one of the following:"
            )
        );
        assert!(root.contains("# BAML files and re-generate this code using `baml generate`"));
        assert!(!root.contains("install baml\n"));
        assert!(!root.contains("baml package"));
        assert!(!root.contains("baml-cli generate"));
        assert!(baml_init.starts_with(PYTHON_LEAF_BANNER));
        assert!(!baml_init.contains("uv add baml-bridge"));
    }
    ...
    fn class_body_renders() {
        ...
        assert!(leaf.starts_with(PYTHON_LEAF_BANNER));
