...
from baml_core import BamlRuntime, BamlCtxManager, Collector, call_function, call_function_sync
...
class TestBasicCollection:
    ...
    def test_collector_captures_function_name(self, rt):
        ...
        last = collector.last
        assert last is not None
        assert last.function_name == "AddNumbers"
    ...
    def test_collector_basic_sync_expr(self, rt):
        ...
        last = collector.last
        assert last is not None
        assert last.function_name == "ReturnOne"
...
class TestGCAndCleanup:
    ...
    def test_collector_ref_counting_two_collectors(self, rt):
        ...
        assert len(c2.logs) == 1
        c2_last = c2.last
        assert c2_last is not None
        assert c2_last.function_name == "ReturnOne"
...
class TestNestedTraceWithCollector:
    ...
    def test_collector_nested_sync_with_trace(self, llm_rt, llm_ctx):
        ...
        last = collector.last
        assert last is not None
        assert last.function_name == "TestLLM"
