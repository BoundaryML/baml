...
# NOTE: a plain `uv sync` does NOT rebuild baml_core after incremental
# Rust edits. baml_core is an editable maturin install, but uv keys the
...
#   uv sync --reinstall-package baml_core   # forces maturin to rebuild
