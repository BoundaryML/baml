...
`CffiHandleTableEntry::Adt(BexExternalAdt::Stream(handle))`. `next` /
`final` (sync + async) round-trip through `BamlRuntime.call_function`
exactly like any codegen-emitted instance method — `encode_call_args`
emits `handle_value(ADT_STREAM)` for `{"self": self}`, the engine
substitutes `T` / `S`, and `decode_call_result` produces the typed
Python return.
...
`BexExternalAdt::Stream` variant, and the `ADT_STREAM` proto tag stay
in Rust — those are engine-side and not reachable from Python.
"""
...
class BamlStream(Generic[T, S]):
    """Opaque wrapper around a streaming-call handle.

    `T` / `S` are erased at runtime — `BamlStream[T, S]` is just a
    `typing.Generic` subscription, handled natively by Python. Codegen
    emits `Stream[X, Y]` annotations in generated leaves; they evaluate
    to a parameterized alias whose `isinstance` falls back to the
