#ifndef BAML_HANDLE_HPP
#define BAML_HANDLE_HPP
...
public:
    ...
    Handle(uint64_t key, int32_t handle_type) : key_(key), handle_type_(handle_type) {}

    Handle(const Handle& other) : key_(0), handle_type_(other.handle_type_) {
        if (other.key_ != 0) {
            key_ = other.cloned_key("baml::Handle copy");
        }
    }
    ...
    uint64_t clone_key_for_wire() const { return cloned_key("baml::Handle wire encode"); }
    ...
    // unequal.
    friend bool operator==(const Handle& a, const Handle& b) { return a.key_ == b.key_; }
...
private:
    uint64_t cloned_key(const char* context) const {
        uint64_t out_key = 0;
