...
| Compiler IR — HIR/MIR/bytecode/diagnostics of code that compiles | `crates/baml_tests/baml_src/ns_fixtures/ns_<topic>/`; IR goldens are opt-in via `src/corpus_snapshot_policy.rs` |
...
| Snapshot | Contents |
|----------|----------|
| `hir.snap` | HIR (canonical item tree) |
| `mir.snap` | MIR |
...
# Run just the corpus snapshot pass (HIR/MIR/bytecode/diagnostics, one compile)
