...
|-------|----------|---------|
| `baml_tests` | `crates/baml_tests/` | Tiered project snapshots with detailed lexer/CST/HIR/TIR/MIR/bytecode output, plus focused compiler and VM tests |
...
```bash
mkdir -p crates/baml_tests/projects/compiles/my_repro/
```

Add a `.baml` file with the minimal repro case:
```bash
# crates/baml_tests/projects/compiles/my_repro/repro.baml
```

### 3. Run and generate snapshots

```bash
cargo test --package baml_tests compiles::my_repro::
```
...
Snapshots are created in `crates/baml_tests/snapshots/compiles/my_repro/`:
...
| `*_04_tir.snap` | TIR with type inference |
| `*_04_5_mir.snap` | MIR control-flow graph |
...
cargo test --package baml_tests compiles::my_project_name::
...
BAML="$PWD/target/debug/baml-cli" # run this from baml_language/
```
...
Do not preserve an old bug report here as a statement of current behavior. Re-run the minimal reproducer against the current checkout, then record the source, command, actual output, expected output, and affected commit in the issue or regression test.
