...
mod tests {
    ...
    /// Build a synthetic corpus of `n` files, each a class + constructor +
    /// accessor, for the cold-open measurement.
        ...
        (0..n)
            .map(|i| {
                (
                    ...
                    format!(
                        "class T{i} {{\n  a: int\n  b: string\n}}\n\n\
                         function make{i}(n: int) -> T{i} {{\n  T{i} {{ a: n, b: \"x\" }}\n}}\n\n\
                         function use{i}(n: int) -> int {{\n  let t = make{i}(n);\n  t.a\n}}\n"
                    ),
                )
            })
            .collect()
    ...
    fn honest_derivations() -> usize {
        baml_db::baml_compiler2_tir::package_interface::stdlib_honest_derivations()
    }

    /// Time one cold first-diagnostics pass on a fresh database with the given
    /// seeds, reporting the stdlib honest-derivation delta it triggered. Uses the
    /// LSP's actual path — `check_file` per *user* file (as
    /// `collect_diagnostic_candidate` does) — not the whole-project collector,
    /// which also re-checks every builtin stub (the LSP never does).
    fn time_cold_check(
        files: &[(&str, &str)],
        cache: Option<LspSeedCache>,
        per_file: Option<&PerFileSeeds>,
    ) -> (f64, usize) {
        let mut db = build_db(files);
        let root = project_root(&db);
        if let Some(cache) = cache {
            let _ = cache.install_stdlib(&mut db);
        }
        if let Some(per_file) = per_file {
            assert!(per_file.apply(&mut db, &root));
        }
        let before = honest_derivations();
        let start = std::time::Instant::now();
        for file in db.get_source_files() {
            let _ = baml_lsp2_actions::check_file(&db, file);
        }
        let ms = start.elapsed().as_secs_f64() * 1000.0;
        (ms, honest_derivations().saturating_sub(before))
    }

    // Constraint 3 measurement: cold-open first-diagnostics wall time, seeded vs
    // unseeded, across project sizes. Printed with `--nocapture`. Numbers are
    // debug-build and single-run (noisy in absolute terms); the mechanism proof
    // is the stdlib honest-derivation count dropping to 0 under seeding.
    #[test]
    #[allow(clippy::print_stderr)] // measurement harness reports numbers on stderr
    fn measure_cold_open_first_diagnostics() {
        for n in [4usize, 24] {
            let files = corpus(n);
            ...
            // Warm reference → extract the same seeds the disk path decodes.
            let reference = build_db(&files_ref);
            let _ = rendered_diagnostics(&reference);
            let (cache, per_file) = extract_seeds(&reference);
            let (stdlib_only, _) = extract_seeds(&reference);
            let stdlib_only = LspSeedCache {
                stdlib_interface: stdlib_only.stdlib_interface,
                per_file: None,
            };

            let (unseeded_ms, unseeded_deriv) = time_cold_check(&files_ref, None, None);
            let (stdlib_ms, stdlib_deriv) = time_cold_check(&files_ref, Some(stdlib_only), None);
            let (seeded_ms, seeded_deriv) =
                time_cold_check(&files_ref, Some(cache), Some(&per_file));

            eprintln!(
                "[cold-open first-diagnostics] {n} files: \
                 unseeded={unseeded_ms:.1}ms (stdlib derivations={unseeded_deriv}) | \
                 stdlib-seed-only={stdlib_ms:.1}ms (derivations={stdlib_deriv}) | \
                 all-seeds={seeded_ms:.1}ms (derivations={seeded_deriv})"
            );
        }
    }
}
