...
The search methods are named `find()` and `find_all()` by API decision; BEP 52 calls these operations `match()` and `match_all()`. Their matching and capture semantics are unchanged.
...
- Some inputs directly exercise examples in BEP 52: named date captures, `${1}mm` replacement, whole-word matching of `2.5%`, and opt-in lookaround/backreferences. Other small inputs cover the specified behavior: Unicode codepoint offsets, absent groups, stateless reuse, captured split delimiters, empty matches, and literal versus template replacement. Matching a BEP example does not establish that every original case was copied from the BEP.
...
After renaming the search methods to `find()` and `find_all()`, the qualified and optional regex access cases were retained under the new names; the unrelated interface-method case and parser keyword support were removed. This gives 51 BAML runtime tests and 13 `sys_regex` unit tests. The related string-search oracle lives in its owning Rust module; it exercises implementation details that a BAML runtime assertion cannot inspect directly.
