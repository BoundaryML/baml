...
- Some inputs directly exercise examples in BEP 52: named date captures, `${1}mm` replacement, and opt-in lookaround/backreferences. Other small inputs cover the specified behavior: Unicode codepoint offsets, absent groups, stateless reuse, captured split delimiters, empty matches, literal versus template replacement, and callback effects. Matching a BEP example does not establish that every original case was copied from the BEP.
...
- During the Codex takeover, four BAML regressions were added for trailing extended-mode comments/captures in full matching, batched offsets for optional/nested groups, optional/qualified interface calls, and dense Unicode replacement with empty literal searches. These were constructed for issues found while reviewing the implementation against BEP 52 and CodeRabbit's findings, rather than imported from another project's suite.
...
After adopting `new()`, `has_match()`, `match_once()`, `match_all()`, and `match_full()`, the qualified and optional regex access cases were retained under the new names; the unrelated interface-method case and parser keyword support were removed. This gives 56 BAML runtime tests and 12 `sys_regex` unit tests. The related string-search oracle and compiler closure-effect tests live in their owning Rust modules; they exercise implementation details that a BAML runtime assertion cannot inspect directly.
