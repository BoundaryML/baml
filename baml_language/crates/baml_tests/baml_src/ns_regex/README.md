...
After renaming the search methods to `find()` and `find_all()`, the qualified and optional regex access cases were retained under the new names; the unrelated interface-method case and parser keyword support were removed. This gives 62 BAML runtime tests and 13 `sys_regex` unit tests. The related string-search oracle and compiler closure-effect tests live in their owning Rust modules; they exercise implementation details that a BAML runtime assertion cannot inspect directly.
