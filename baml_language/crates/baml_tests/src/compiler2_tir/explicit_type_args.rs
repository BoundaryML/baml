...
#[test]
fn missing_constructor_type_arg_suppresses_dependent_field_mismatch() {
    let mut db = make_db();
    let file = db.add_file(
        "test.baml",
        r#"
...
function repro() -> int {
    let pair = Pair<int> { first: "one", second: "two" }
    0
}
"#,
    );
    ...
    function user.repro() -> int throws never {
      { : 0
        let pair = Pair { first: "one", second: "two" } : user.Pair<int>
        0 : 0
      }
      !! 89..130: type `Pair` expects 2 type argument(s), got 1
      !! 108..113: type mismatch: expected int, got "one"
    }
