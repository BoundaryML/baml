...
fn generate_benchmarks(out_dir: &str, manifest_dir: &str) {
    ...
    writeln!(file, "// Do not edit this file directly").unwrap();
    writeln!(file).unwrap();
    writeln!(file, "// Note: codspeed_bencher_compat::Bencher, baml_db::*, and std::hint::black_box are already imported in the main file").unwrap();
    writeln!(file).unwrap();
    ...
    if !incremental_bench_names.is_empty() {
        ...
        // Create empty group if no benchmarks
        writeln!(file).unwrap();
        writeln!(file, "// No incremental benchmarks found").unwrap();
        writeln!(file, "benchmark_group!(generated_incremental_benches);").unwrap();
    }

    if !scale_bench_names.is_empty() {
        writeln!(file).unwrap();
        writeln!(file, "// Generated benchmark group for scale benchmarks").unwrap();
        writeln!(file, "benchmark_group!(").unwrap();
        write!(file, "    generated_scale_benches").unwrap();
        for name in &scale_bench_names {
            write!(file, ",\n    {}", name).unwrap();
        }
        writeln!(file, "\n);").unwrap();
    } else {
        // Create empty group if no benchmarks
        writeln!(file).unwrap();
        writeln!(file, "// No scale benchmarks found").unwrap();
        writeln!(file, "benchmark_group!(generated_scale_benches);").unwrap();
    }
}
