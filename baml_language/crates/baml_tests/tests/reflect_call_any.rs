...
async fn runtime_enum_renders_and_alias_round_trips_through_sap() {
    ...
    base_url = "http://localhost:1234",
        ...
        function main() -> string {
            ...
            type CategoryOutput = unreflect(t)
            let prompt = Classify@spec<CategoryOutput>("sample").prompt().text()
            let parsed = Classify@spec<CategoryOutput>("sample").parse(`"k7"`)
...
function main() -> string {
    ...
    type RuntimeTranscript = unreflect(runtime_class.as_type())
    Extract@spec<Wrapper<RuntimeTranscript>>("sample").prompt().text()
}
"##
    );
...
async fn runtime_enum_identity_and_metadata_are_preserved() {
    ...
    base_url = "http://localhost:1234",
        ...
        function main() -> string {
            ...
            type LeftOutput = unreflect(left)
            type RightOutput = unreflect(right)
            let left_prompt = Classify@spec<LeftOutput>("sample").prompt().text()
            let right_prompt = Classify@spec<RightOutput>("sample").prompt().text()
...
async fn empty_runtime_enum_fails_at_the_render_boundary() {
    ...
    base_url = "http://localhost:1234",
        ...
        function main() -> string throws never {
            ...
            let rendered = {
                type CategoryOutput = unreflect(t)
                Classify@spec<CategoryOutput>("sample").prompt()
            } catch (e) {
                reflect.errors.CompilationError => e.diagnostics[0].code + "|" + e.diagnostics[0].message,
...
#[tokio::test]
async fn named_runtime_type_arguments_preserve_identity_on_stream_projections() {
    let output = baml_test!(
        r##"
        enum Category {
            STATIC
        ...
        class StreamScript {
            ...
            implements ai.stream.StreamingClient {
                function invoke_stream(self, input: ai.ModelTurnInput) -> ai.stream.TurnStream {
                    let _ = input;
                    ai.stream.TurnStream.from_chunks([`"RED"`])
                }
            }
        }
        ...
        function main() -> bool throws unknown {
            ...
            type Output = unreflect(t.as_type())
            let stream = Classify@stream<Output>("sample", client = StreamScript {})
            let partial = stream.next()
            let final = stream.final()
            reflect.enum.get_value(partial) == "RED"
                && reflect.enum.get_value(final) == "RED"
        }
        "##
    );
...
/// A specialized generic `@spec` projection is an ordinary first-class
/// callable. Its realized type arguments survive storage and invocation.
#[tokio::test]
async fn specialized_generic_spec_projection_is_first_class() {
    let output = baml_test!(
        r#"
        client TestClient = openai.ResponsesClient.new(
            model = "gpt-4o-mini",
        ...
        type SpecFn = (topic: string) -> ai.FunctionSpec<string[]> throws unknown;
        ...
        function main() -> bool throws unknown {
            let callable: SpecFn = GenericList@spec<string>
            let spec = callable("items")
            spec.prompt().text().includes("string")
                && spec.output_type() == reflect.Type.of<string[]>()
        }
        "#
    );
    assert_eq!(output.result, Ok(BexExternalValue::Bool(true)));
...
/// Removed synthetic function spellings are absent from package reflection;
/// the authored function remains the sole declaration.
#[tokio::test]
async fn package_reflection_omits_removed_function_companions() {
    let output = baml_test!(
        r#"
        client TestClient = openai.ResponsesClient.new(
            model = "gpt-4o-mini",
        ...
        function main() -> bool {
            ...
            package.functions().get("root.Plain") != null
                && package.functions().get("root.Plain$spec") == null
                && package.functions().get("root.Plain$render_prompt") == null
                && package.functions().get("root.Plain$build_request") == null
                && package.functions().get("root.Plain$parse") == null
                && package.functions().get("root.Plain$stream") == null
                && package.functions().get("root.Plain@stream") == null
                && package.get_function<reflect.AnyFunction<Returns = unknown, Throws = unknown>>(
                    "root.Plain@stream"
                ) == null
        }
        "#
