...
enum State { Ready }
function ReflectedItem() -> type { type.of<Item>() }
function ReflectedState() -> type { type.of<State>() }
"# })
  ...
  let reflected_item = pkg.get_function<() -> type>("root.ReflectedItem")
    ?? throw "missing ReflectedItem"
  let reflected_state = pkg.get_function<() -> type>("root.ReflectedState")
