...
async fn witness_inherits_interface_default_methods() {
    ...
    base_url = "http://localhost:1234",
        ...
        function main() -> string {
            ...
            let is_member = person_t.as_type().implements(type.of<Greeter>())
        ...
        function main() -> string throws unknown {
            ...
            s.eval(#"
                function build() -> type {
                    ...
                    let person_t = reflect.class.new("Person", { "who": type.of<string>() }, implementations = [witness])
                    person_t.as_type()
                }
            "#)
            ...
            let is_member = s.eval<bool>(#"type.of<Greeter>().implemented_by(build())"#)
            if is_member {
                return "witnessed"
            }
