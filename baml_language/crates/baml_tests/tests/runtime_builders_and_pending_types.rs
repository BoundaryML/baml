...
async fn self_recursive_employee_renders_and_parses() {
    let output = baml_test!(
        ...
        function main() -> string {
            ...
            type EmployeeOutput = unreflect(employee_t.as_type())
            let prompt = Extract@spec<EmployeeOutput>().prompt().text()
            let value = Extract@spec<EmployeeOutput>().parse(
...
async fn frozen_mutation_and_unresolved_call_name_the_builder() {
    let output = baml_test!(
        ...
        function main() -> string {
            ...
            let pending_error = {
                type PendingOutput = unreflect(erased)
                Extract@spec<PendingOutput>().prompt()
            } catch (e) {
                reflect.errors.CompilationError => e.diagnostics[0].message
            }
...
async fn recursive_pending_field_metadata_reaches_the_rendered_schema() {
    let output = baml_test!(
        ...
        function main() -> string throws unknown {
            ...
            type NodeOutput = unreflect(built)
            Extract@spec<NodeOutput>().prompt().text()
        }
        "##
    );
