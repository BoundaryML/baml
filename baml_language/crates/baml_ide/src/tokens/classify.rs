...
/// Classify a name iff it is a builtin type spelling (`string`, `int`, `json`,
/// ...): a `defaultLibrary` `Type`. Driven by [`baml_type::BuiltinTypeName`] —
/// the compiler's own registry of builtin type names — so the set can never
/// drift from the language. Used for both type-position names and
/// value-position path roots (`string.from(...)`), so a builtin is highlighted
/// identically wherever it appears.
pub(super) fn classify_primitive(name: &str) -> Option<(SemanticTokenType, ModifierSet)> {
    baml_type::BuiltinTypeName::from_alias(name)
        .map(|_| (SemanticTokenType::Type, ModifierSet::DEFAULT_LIBRARY))
}
...
mod tests {
    ...
    #[test]
    fn builtin_type_spellings_come_from_the_language_registry() {
        // Every builtin spelling the language defines classifies as a
        // `defaultLibrary` type...
        for name in [
            "int",
            "bigint",
            ...
            assert_eq!(
                classify_primitive(name),
                Some((SemanticTokenType::Type, ModifierSet::DEFAULT_LIBRARY)),
                "{name}"
            );
        }
        // ...and nothing else does — names outside the registry stay neutral
        // instead of being guessed off a hand-maintained list.
        for name in ["map", "bytes", "Map", "str", "String", ""] {
            assert_eq!(classify_primitive(name), None, "{name}");
        }
    }
}
