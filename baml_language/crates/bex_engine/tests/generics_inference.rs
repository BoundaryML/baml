...
const IDENTITY: &str = r#"
    function identity<T>(x: T) -> string { reflect.Type.of<T>().to_string() }
    function main() -> int { 0 }
"#;
...
async fn infer_make_triple_structural() {
    ...
    let source = r#"
        function make_triple<A, B, C>(a: A, b: B[], c: map<string, C>) -> string {
            reflect.Type.of<A | B | C>().to_string()
        }
        function main() -> int { 0 }
    "#;
    let b = BEV::Array {
...
async fn infer_second_of_nested_generic() {
    ...
    let source = r#"
        ...
        function second_of<T>(p: GenericPair<int, T>) -> string { reflect.Type.of<T>().to_string() }
        function main() -> int { 0 }
    "#;
    let arg = BEV::instance_generic(
        "GenericPair",
...
const CHOOSE: &str = r#"
    function choose<T>(left: T, right: T) -> string { reflect.Type.of<T>().to_string() }
    function main() -> int { 0 }
"#;
...
const MAYBE_ID: &str = r#"
    function maybe_id<T>(x: T?) -> string { reflect.Type.of<T>().to_string() }
    function main() -> int { 0 }
"#;
...
async fn infer_nonempty_map_binds_key_despite_valueless_values() {
    ...
    let src = r#"
        function map_key<K>(m: map<K, int[]>) -> string { reflect.Type.of<K>().to_string() }
        function main() -> int { 0 }
    "#;
...
async fn gatea_class_method_body_only_var_should_reject() {
    ...
    let src = r#"
        class Helper {
            function reflect_t<T>() -> string { reflect.Type.of<T>().to_string() }
        }
        function main() -> int { 0 }
    "#;
...
const PAIR: &str = r#"
    function pair<T>(a: T[], b: T[]) -> string { reflect.Type.of<T>().to_string() }
    function main() -> int { 0 }
"#;
...
async fn infer_glue_invariant_vs_covariant_conflict_rejects() {
    ...
    let src = r#"
        function glue<T>(bare: T, arr: T[]) -> string { reflect.Type.of<T>().to_string() }
        function main() -> int { 0 }
    "#;
...
async fn b7_first_or_empty_list_free_fn_binds_rust_type() {
    ...
    let src = r#"
        function first_or<T>(xs: T[]) -> string { reflect.Type.of<T>().to_string() }
        function main() -> int { 0 }
    "#;
...
const GENERIC_BOX_METHODS: &str = r#"
    class GenericBox<T> {
      value T
      function get(self) -> string { reflect.Type.of<T>().to_string() }
...
async fn l5_unbound_receiver_method_recovers_class_var_from_field() {
    ...
    // left as host-only rust_type. `reflect.Type.of<T | U>` then renders
...
async fn het_array_element_type_unifies() {
    ...
    let src = r#"
        function elem_type<T>(xs: T[]) -> string { reflect.Type.of<T>().to_string() }
        function main() -> int { 0 }
    "#;
