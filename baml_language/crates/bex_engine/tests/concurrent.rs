...
async fn test_concurrent_calls_no_race() {
    ...
    for _ in 0..10 {
        ...
        handles.push(tokio::spawn(async move {
            engine.call_function("test_function", vec![], None, &[]).await
        }));
    }
...
async fn test_concurrent_allocations_no_overlap() {
    ...
    for _ in 0..5 {
        ...
        handles.push(tokio::spawn(async move {
            ...
            let result = engine.call_function("allocate_many", vec![], None, &[]).await?;
...
async fn test_concurrent_string_allocations() {
        ...
        .take(20)
    {
        ...
        handles.push(tokio::spawn(async move {
            let result = engine.call_function(&func, vec![], None, &[]).await?;
...
async fn test_concurrent_array_allocations() {
    ...
    for (func_name, size) in [
        ...
        let engine = Arc::clone(&engine);
        handles.push(tokio::spawn(async move {
            let result = engine.call_function(func_name, vec![], None, &[]).await?;
...
async fn test_call_function_with_external_args() {
    ...
    let result = engine
        .call_function("concat_strings", vec!["Hello".into(), "World".into()], None, &[])
        .await
    ...
    let result = engine
        .call_function("sum_array", vec![arr], None, &[])
        .await
        .expect("call_function failed");
