    ...
    fn execute_sys_op(&self, op: SysOp, args: Vec<BexExternalValue>) -> SysOpResult {
        match op {
            ...
            SysOp::LlmRenderPrompt => {
                SysOpResult::Ready(llm::render_prompt(&args).map(BexExternalValue::PromptAst))
            }
            SysOp::LlmGetJinjaTemplate => {
                SysOpResult::Ready(llm::get_jinja_template(&self.snapshot, &args))
            }
            SysOp::LlmBuildPrimitiveClient => {
                SysOpResult::Ready(llm::build_primitive_client(&args))
            }
            SysOp::LlmGetClientFunction => SysOpResult::Ready(llm::get_client_function(
                &self.snapshot,
