...
impl BexEngine {
    ...
    fn execute_get_jinja_template(&self, args: &[BexValue]) -> Result<BexExternalValue, OpError> {
    ...
    fn execute_build_primitive_client(args: &[BexValue]) -> Result<BexExternalValue, OpError> {
        // Extract name
    ...
    fn execute_get_client_function(&self, args: &[BexValue]) -> Result<BexExternalValue, OpError> {
        // Extract function name
        let function_name = match &args[0] {
            BexValue::External(BexExternalValue::String(s)) => s.as_str(),
            other => {
