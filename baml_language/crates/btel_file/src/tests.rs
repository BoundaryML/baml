...
fn writes_exact_publisher_bytes_and_finish_is_idempotent() {
    ...
    let sink = FileSink::create(root.path(), id, FileSinkConfig::default()).unwrap();
    let files = files(id, 3);
    for file in &files {
        sink.sender().send(file.clone()).unwrap();
    }
    ...
    for file in &files {
        let path = sink.directory().join(format!("{:020}.pb", file.sequence()));
        assert_eq!(fs::read(path).unwrap(), file.bytes());
    }
    assert!(sink.sender().send(files[0].clone()).is_err());
...
#[test]
fn stalled_writer_has_bounded_nonblocking_admission() {
    let root = tempfile::tempdir().unwrap();
    ...
    sink.sender().send(files.next().unwrap()).unwrap();
    started.recv_timeout(Duration::from_secs(5)).unwrap();
    sink.sender().send(files.next().unwrap()).unwrap();
    assert!(
        sink.sender()
            .send(files.next().unwrap())
            .unwrap_err()
            .to_string()
            .contains("full")
    );
...
fn disk_failure_and_duplicate_sequence_are_observable() {
    ...
    sink.sender().send(original).unwrap();
    ...
    let sink = FileSink::create(root.path(), id, FileSinkConfig::default()).unwrap();
    let original = files(id, 1).pop().unwrap();
    sink.sender().send(original.clone()).unwrap();
    sink.sender().send(original.clone()).unwrap();
    assert!(sink.finish().unwrap_err().to_string().contains("sequence"));
    assert_eq!(
        fs::read(sink.directory().join("00000000000000000001.pb")).unwrap(),
        original.bytes()
    );
}
...
fn recovery_keeps_later_files_when_a_middle_file_is_missing_or_corrupt() {
    ...
    for file in files(id, 4) {
        sink.sender().send(file).unwrap();
    }
    sink.finish().unwrap();
    fs::remove_file(sink.directory().join("00000000000000000002.pb")).unwrap();
    fs::write(sink.directory().join("00000000000000000003.pb"), [0xff]).unwrap();
    fs::write(
        sink.directory().join("00000000000000000005.pb.part"),
        [0x08],
    )
    .unwrap();
    let read = read_directory(sink.directory()).unwrap();
...
fn later_definitions_resolve_a_live_prefix_and_wrong_identity_is_rejected() {
    ...
    let mut second = proto::RecordingFile::decode(original.bytes()).unwrap();
    second.sequence = 2;
    ...
    let path = sink.directory().join("00000000000000000002.pb");
