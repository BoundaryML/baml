...
//! local delivery writes its exact bytes, without reencoding or per-delivery aggregation.
//!
...
use btel_recorder::{RecordingId, SealedFile};
...
pub struct LocalDeliveryError(pub String);
impl fmt::Display for LocalDeliveryError {
...
fn io_error(path: &Path, error: &io::Error) -> LocalDeliveryError {
    LocalDeliveryError(format!("telemetry file {}: {error}", path.display()))
}
...
pub struct LocalDeliveryHandle {
    sender: mpsc::SyncSender<Command>,
    result: Arc<OnceLock<Result<(), LocalDeliveryError>>>,
}
impl LocalDeliveryHandle {
    ...
    pub fn send(&self, file: SealedFile) -> Result<(), LocalDeliveryError> {
    ...
    fn enqueue(&self, command: Command) -> Result<(), LocalDeliveryError> {
        if let Some(result) = self.result.get() {
            return result.clone().and(Err(LocalDeliveryError(
                "telemetry local delivery is closed".into(),
            )));
        }
        self.sender.send(command).map_err(|_| {
            ...
            LocalDeliveryError("telemetry file writer stopped".into())
        })
    }
}
...
pub struct LocalDelivery {
    ...
    sender: mpsc::SyncSender<Command>,
    result: Arc<OnceLock<Result<(), LocalDeliveryError>>>,
    delivery: Option<LocalDeliveryHandle>,
...
impl LocalDelivery {
    ...
    pub fn create(root: &Path, id: RecordingId, config: LocalDeliveryConfig) -> io::Result<Self> {
    ...
    fn start_with_snapshots(
        directory: PathBuf,
        config: LocalDeliveryConfig,
        mut write: impl FnMut(&SealedFile) -> Result<(), LocalDeliveryError> + Send + 'static,
        mut write_snapshot: impl FnMut(&btel_snapshot::Snapshot) -> Result<(), LocalDeliveryError>
        + Send
        + 'static,
        on_failure: impl FnOnce(&LocalDeliveryError) + Send + 'static,
    ) -> io::Result<Self> {
        ...
        Ok(Self {
            directory,
            delivery: Some(LocalDeliveryHandle {
                sender: sender.clone(),
    ...
    pub fn take_handle(&mut self) -> LocalDeliveryHandle {
        self.delivery
            .take()
            .expect("recording already has its sole handle")
    }
    /// None while running. Disk errors are observable even without another file.
    pub fn result(&self) -> Option<Result<(), LocalDeliveryError>> {
    ...
    pub fn finish(&self) -> Result<(), LocalDeliveryError> {
        let mut worker = self
            .worker
            .lock()
        ...
        if let Some(handle) = worker.take() {
            ...
            if handle.join().is_err() {
                let _ = self.result.set(Err(LocalDeliveryError(
                    "telemetry file writer panicked".into(),
                )));
            }
        }
        self.result().unwrap_or_else(|| {
            Err(LocalDeliveryError(
                "telemetry file writer stopped without a result".into(),
            ))
        })
    }
}
impl Drop for LocalDelivery {
    fn drop(&mut self) {
        let _ = self.finish();
    }
}
