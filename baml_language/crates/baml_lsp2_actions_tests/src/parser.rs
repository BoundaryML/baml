...
fn parse_source_section(
...
) -> (IndexMap<String, VirtualFile>, Vec<CursorMarker>) {
    let mut files: IndexMap<String, VirtualFile> = IndexMap::new();
    ...
    for (filename, vfile) in &mut files {
        let (clean_content, markers) = extract_cursors_from_content(&vfile.content, filename);
        if !markers.is_empty() {
            vfile.content = clean_content;
            cursor_markers.extend(markers);
        }
    }
...
/// Extract all cursor markers from file content.
/// Returns the cleaned content (with all markers removed) and a list of markers.
fn extract_cursors_from_content(content: &str, filename: &str) -> (String, Vec<CursorMarker>) {
    let mut markers = Vec::new();
    let mut clean = content.to_string();
    let marker_len = CURSOR_MARKER.len();

    // Each time we find and remove a marker, subsequent offsets shift by marker_len.
    // We search in the progressively cleaned string so offsets are already adjusted.
    while let Some(marker_pos) = clean.find(CURSOR_MARKER) {
        let before_marker = &clean[..marker_pos];
        ...
        // Remove this marker from the clean string
        clean = format!(
            "{}{}",
            &clean[..marker_pos],
            &clean[marker_pos + marker_len..]
        );
    }
...
// <no-diagnostics-expected>"#;
    ...
    fn test_extract_cursor_from_content() {
        ...
        let (clean, markers) = extract_cursors_from_content(content, "test.baml");

        assert_eq!(markers.len(), 1);
        assert_eq!(clean, "class Foo {}");
        assert_eq!(markers[0].file, "test.baml");
        assert_eq!(markers[0].offset, 9);
        assert_eq!(markers[0].line, 0);
        assert_eq!(markers[0].column, 9);
    }
    ...
    fn test_cursor_marker_multiline() {
        ...
        assert_eq!(clean, "class Foo {\n    name string\n}");
        assert_eq!(markers[0].file, "test.baml");
        assert_eq!(markers[0].offset, 20); // "class Foo {\n" (12) + "    name" (8) = 20
        assert_eq!(markers[0].line, 1);
        assert_eq!(markers[0].column, 8); // 4 spaces + "name" (4) = 8 chars from start of line
    }

    #[test]
    fn test_extract_multiple_cursors() {
        let content = "class Foo<[CURSOR] {\n    name<[CURSOR] string\n}";
        let (clean, markers) = extract_cursors_from_content(content, "test.baml");
        ...
        // First marker: after "class Foo"
        assert_eq!(markers[0].offset, 9);
        assert_eq!(markers[0].line, 0);
        assert_eq!(markers[0].column, 9);

        // Second marker: after "    name" on line 1
        // In cleaned content: "class Foo {\n    name" = 12 + 8 = 20
        assert_eq!(markers[1].offset, 20);
        assert_eq!(markers[1].line, 1);
        assert_eq!(markers[1].column, 8);
    }
