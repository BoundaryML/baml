...
use super::runner::{CompletionTestResult, TestResult};
...
pub fn update_test_file(
    ...
    // Generate completion section from actual results (keeping only found expected labels)
    let completion_section = generate_completion_section(result.completion_result.as_ref());
...
/// Generate the completion section from actual test results.
/// Keeps only the originally-expected labels that were actually found.
fn generate_completion_section(result: Option<&CompletionTestResult>) -> Option<String> {
    let result = result?;

    // Keep expected labels that are present in found_labels
    let rebaselined: Vec<&String> = result
        .expected_labels
        .iter()
        .filter(|label| result.found_labels.contains(label))
        .collect();
    ...
    let labels = rebaselined
        .iter()
        .map(|s| s.as_str())
        .collect::<Vec<_>>()
        .join(", ");

    Some(format!("// SHOULD_CONTAIN: {labels}"))
}
