...
mod tests {
    use bex_events::{
        ...
        run::{
            AttachRootTraceResult, ExecutionRequest, HeaderObservation, HostCallId,
    ...
    fn run_wire_projection_uses_opaque_ids_and_no_trace_refs() {
        ...
        let run = store.snapshot(start.run_id).unwrap();

        let wire = run_to_wire(&run);
        assert_eq!(wire["runId"], start.run_id.to_wire_string());
    ...
    fn payload_wire_projection_redacts_fetch_values() {
        ...
        let run_wire = run_to_wire(&store.snapshot(start.run_id).unwrap());
    ...
    fn profile_observer_broadcasts_reconstruction_patches() {
        ...
        assert_eq!(patch["runId"], start.run_id.to_wire_string());
        assert!(
            patch["changes"]
                .as_array()
                .unwrap()
        ...
        let snapshot_wire = run_to_wire(&store.snapshot(start.run_id).unwrap());
        let overlay = &snapshot_wire["graphRuntimeOverlay"];
        assert_eq!(overlay["runId"], start.run_id.to_wire_string());
