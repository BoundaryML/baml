...
/// Resolves a run's control-flow-node spans from the overlay graph pinned
/// at launch, so highlighting stays correct after later recompiles.
///
/// Keyed by the run's own `(project_id, project_generation)`: the project id
/// is the workspace root path the run launched under, and the generation is
/// the engine identity it pinned. A retired generation whose graph the cache
/// has evicted resolves as unavailable — never against newer code.
pub struct ProjectGraphRuntimeOverlaySpanProvider {
    runtimes: Arc<crate::engine::RuntimeRegistry>,
}

impl ProjectGraphRuntimeOverlaySpanProvider {
    pub fn new(runtimes: Arc<crate::engine::RuntimeRegistry>) -> Self {
        Self { runtimes }
    }
}
...
impl GraphRuntimeOverlaySpanProvider for ProjectGraphRuntimeOverlaySpanProvider {
    fn cfg_node_spans_for_run(&self, run: &Run) -> GraphRuntimeOverlaySpanResolution {
        ...
        let root = PathBuf::from(&run.request.project_id.0);
        let Some(graph) = self.runtimes.existing(&root).and_then(|runtime| {
            runtime.overlay_graph(run.request.project_generation.0, function_name)
        }) else {
            return GraphRuntimeOverlaySpanResolution::Unavailable(project_generation_unavailable(
                run,
