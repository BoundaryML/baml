...
fn if_else_comparison_condition() -> anyhow::Result<()> {
    assert_compiles(Program {
        ...
        expected: vec![(
            ...
            // - Comparison operand temps eliminated
            vec![
                Instruction::LoadConst(Value::Null),
                Instruction::LoadConst(Value::Null),
                Instruction::LoadConst(Value::Null),
...
fn else_if_with_comparisons() -> anyhow::Result<()> {
    assert_compiles(Program {
        ...
        expected: vec![(
            "main",
            // MIR codegen with stackification:
            // - Comparison operand temporaries eliminated
            // - x is kept as user-declared variable
            vec![
                Instruction::LoadConst(Value::Null),
                Instruction::LoadConst(Value::Null),
                Instruction::LoadConst(Value::Null),
...
fn block_expr() -> anyhow::Result<()> {
    assert_compiles(Program {
        ...
        expected: vec![(
            "main",
            // MIR codegen with stackification:
            // - All temporaries and unused locals eliminated
