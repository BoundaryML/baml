...
fn respond(state: &MockState, base: &str, path: &str, body: &str) -> (&'static str, String) {
    ...
    let claimed_jwt = mock_jwt("agent_reg_test1", Some("dhilan@test.dev"));
    match path {
        ...
        "/oauth2/token" => {
            ...
            let (token, email_part) = if state.claimed.load(Ordering::SeqCst) {
                ("at_claimed", r#","user":{"email":"dhilan@test.dev"}"#)
            } else {
                ("at_anon", "")
            };
            (
...
fn run_baml(home: &std::path::Path, base: &str, args: &[&str], stdin: Option<&str>) -> (bool, String) {
...
fn anonymous_first_lifecycle_with_claim_ceremony() {
    ...
    assert!(ok, "{out}");
    assert!(out.contains("Started anonymous project agent_reg_test1"), "{out}");
    ...
    let (ok, out) = run_baml(
        home.path(),
        &base,
        &["auth", "login", "--no-open", "--email", "dhilan@test.dev"],
        Some("WRONG-0000\nWXYZ-7890\n"),
    );
    assert!(ok, "{out}");
    assert!(out.contains("didn't match"), "wrong code should re-prompt: {out}");
    assert!(
    ...
    assert!(!json.contains("claim_token"), "claim_token must be dropped: {json}");

    // 5. whoami reports the claimed identity.
    let (ok, out) = run_baml(home.path(), &base, &["auth", "whoami"], None);
    assert!(ok, "{out}");
    assert!(out.contains("Logged in as dhilan@test.dev"), "{out}");
}
