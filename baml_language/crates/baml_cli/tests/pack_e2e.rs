...
/// `--function` takes a different pack-resolution path than root main:
/// the envelope's `target_name` is a qualified function rather than
/// `user.main`. Verifies that code path end-to-end.
#[test]
fn pack_e2e_function_target() {
    ...
    let out = run(&bin, &["--who", "World"]);
    assert!(out.status.success());
    assert!(String::from_utf8_lossy(&out.stdout).contains("hello World"));
}

/// `.baml` positional takes the hermetic-load path — a different
