...
pub(crate) fn refresh(ids: &[Uuid], force: bool, notice: bool, installed: &str) -> Result<()> {
    ...
    if force || now.saturating_sub(cache.last_attempt) >= DAY {
        ...
        if let Some(key) = KEY.filter(|k| k.starts_with("sb_publishable_") && k.len() < 256) {
            let base = reqwest::Url::parse(if URL.is_empty() {
                "https://igraichzcidsylvzkjlc.supabase.co"
            } else {
                URL
            })?;
            anyhow::ensure!(
    ...
    let messages = if notice {
        update_notices(&cache, installed, now)
    } else {
        Vec::new()
    };
...
fn update_notices(cache: &Cache, installed: &str, now: u64) -> Vec<String> {
    if now.saturating_sub(cache.last_notice) < DAY {
        return Vec::new();
    }
...
mod tests {
    ...
    #[test]
    fn notices_are_daily_and_stop_after_upgrade() {
        let id = Uuid::new_v4();
        let mut cache = Cache::default();
        cache
            .reports
            .insert(id, vec![issue("ISSUE-fixed", "shipped", Some("0.20.2"))]);
        let messages = update_notices(&cache, "0.20.1", DAY);
        assert_eq!(messages.len(), 1);
        assert!(messages[0].contains("fixed in BAML 0.20.2"));
        assert!(messages[0].contains("baml toolchain update"));
        assert!(update_notices(&cache, "0.20.2", DAY).is_empty());
        cache.last_notice = DAY;
        assert!(update_notices(&cache, "0.20.1", 2 * DAY - 1).is_empty());
        assert_eq!(update_notices(&cache, "0.20.1", 2 * DAY).len(), 1);
        cache.reports.insert(
            id,
            vec![issue(
                "ISSUE-nightly",
                "shipped",
                Some("0.20.2-nightly.20260920.a"),
            )],
        );
        assert!(
            update_notices(&cache, "0.20.1", 2 * DAY)[0].contains("baml toolchain use nightly")
        );
        assert!(update_notices(&cache, "0.20.2-nightly.20260920.a", 2 * DAY).is_empty());
        cache
            .reports
            .insert(id, vec![issue("ISSUE-open", "open", Some("0.20.2"))]);
        assert!(update_notices(&cache, "0.20.1", 2 * DAY).is_empty());
    }

    #[test]
    fn existing_baml_issue_ids_are_accepted_but_control_characters_are_rejected() {
        let id = Uuid::new_v4();
        let mut cache = Cache::default();
        for issue_id in [
            "ISSUE-baml_id_1_example",
            "ISSUE-mini-0123456789abcdef01234567",
        ] {
            apply(
                &mut cache,
                &[id],
                vec![Row {
                    ...
                    state: Some("shipped".into()),
                    fixed_in: Some("0.20.2".into()),
                }],
            )
            .unwrap();
            assert_eq!(cache.reports[&id][0].issue_id, issue_id);
        }
        let before = cache.reports[&id].clone();
        for issue_id in ["", "ISSUE-unsafe\n", "ISSUE-\u{1b}[31m", "../issue"] {
            assert!(
                apply(
                    &mut cache,
                    &[id],
            ...
            assert_eq!(cache.reports[&id], before);
        }
    }
