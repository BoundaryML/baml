...
//
// Both produce a `baml.toml` with a `[package]` table and a
// `baml_src/main.baml` starter file. The BAML wrapper installs the agent
// bootstrap after this command succeeds. The
...
///
/// Creates `baml.toml` and `baml_src/main.baml`. The
...
///
/// Creates the destination directory, `baml.toml`, and `baml_src/main.baml`.
/// Use `baml init` when the directory already exists.
#[derive(Args, Clone, Debug)]
#[command(after_long_help = "\
Examples:
