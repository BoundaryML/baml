...
fn normalize_generated_files(
    ...
    for (path, content) in generated {
        ...
        normalized.insert(relative, content.as_str());
    }
    Ok(normalized)
}
...
fn discover_generators(root: &Path) -> (Vec<GeneratorDef>, Vec<Diagnostic>) {
    ...
    for (name, spanned) in &manifest.generator {
        ...
        let output_type = parse_required_property::<OutputType>(
            ...
            r#"one of: "python/pydantic", "python/pydantic/v1", "typescript/node", "csharp""#,
            table_range,
            &mut diags,
        );
...
mod generated_output_tests {
    ...
    fn generated(entries: &[(&str, &str)]) -> HashMap<PathBuf, String> {
        entries
            .iter()
