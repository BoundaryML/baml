...
pub struct RunArgs {
    ...
    /// Evaluate a BAML expression with optional `--file` or project context.
    ///
    ...
    ///
    /// With `--expression`, makes this file's declarations available to the
    /// expression. Otherwise, runs its `main` function when no target is
    /// supplied. Mutually exclusive with `--project`.
...
impl RunArgs {
    ...
    fn run_expression(&self, expr_body: &str, reporter: &Reporter) -> Result<crate::ExitCode> {
        ...
        let expression_path = isolated_root.join("__expr__.baml");
        let expression_path = match standalone.as_ref() {
            Some(source) if source.file == expression_path => {
                isolated_root.join("__baml_run_expr__.baml")
            }
            _ => expression_path,
        };
...
mod tests {
    ...
    fn test_run_help_documents_expression_file_context() {
        ...
        assert!(
            normalized_help
                .contains("Evaluate a BAML expression with optional `--file` or project context"),
            "{help}"
        );
