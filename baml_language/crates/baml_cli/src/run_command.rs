...
"#;
    ...
    // ========================================================================
    // coerce_json_union — union type coercion
    // ========================================================================

    /// First variant matches immediately — must return its result.
    #[test]
    fn test_coerce_json_union_first_variant_wins() {
        let variants = vec![ty_int(), ty_string()];
        let val = coerce_json_union(&serde_json::json!(42), &variants).unwrap();
        assert!(matches!(val, BexExternalValue::Int(42)));
    }

    /// First variant fails, second matches — must fall through and return
    /// the second variant's result.
    #[test]
    fn test_coerce_json_union_falls_through_to_second_variant() {
        let variants = vec![ty_bool(), ty_int()];
        // A JSON number is not a bool, so ty_bool fails → falls through to ty_int.
        let val = coerce_json_union(&serde_json::json!(7), &variants).unwrap();
        assert!(matches!(val, BexExternalValue::Int(7)));
    }

    /// All variants fail — must return an error.
    #[test]
    fn test_coerce_json_union_all_fail_is_error() {
        // A JSON array can't be coerced to int or bool.
        let variants = vec![ty_int(), ty_bool()];
        let result = coerce_json_union(&serde_json::json!([1, 2, 3]), &variants);
        assert!(result.is_err(), "all variants failing must return an error");
    }

    /// Empty variant list — must return an error (nothing to match against).
    #[test]
    fn test_coerce_json_union_empty_variants_is_error() {
        let result = coerce_json_union(&serde_json::json!(1), &[]);
        assert!(
            result.is_err(),
            "empty union variant list must return an error"
        );
    }

    /// String value matches Enum variant correctly within a union.
    #[test]
    fn test_coerce_json_union_enum_variant_in_union() {
        let variants = vec![ty_int(), Ty::Enum(tn("Status"), Default::default())];
        let val = coerce_json_union(&serde_json::json!("Active"), &variants).unwrap();
        match val {
            BexExternalValue::Variant {
                enum_name,
                variant_name,
    ...
    // ========================================================================
    // RunArgs::parse_scripts — static TOML parsing
    // ========================================================================

    /// Invalid TOML returns an empty map without panicking.
    #[test]
    fn test_parse_scripts_invalid_toml_returns_empty() {
        let result = RunArgs::parse_scripts("{ this is not valid toml !!!");
        assert!(result.is_empty(), "invalid TOML must yield empty map");
    }

    /// A TOML file with no `[scripts]` section returns an empty map.
    #[test]
    fn test_parse_scripts_no_scripts_section_returns_empty() {
        let toml = "[project]\nname = \"myproject\"\n";
        let result = RunArgs::parse_scripts(toml);
        assert!(result.is_empty(), "no [scripts] section must yield empty map");
    }

    /// Empty `[scripts]` table returns an empty map.
    #[test]
    fn test_parse_scripts_empty_scripts_table_returns_empty() {
        let toml = "[scripts]\n";
        let result = RunArgs::parse_scripts(toml);
        assert!(result.is_empty(), "empty [scripts] must yield empty map");
    }

    /// String-form entry is split on whitespace (like Cargo aliases).
    #[test]
    fn test_parse_scripts_string_form_is_split_on_whitespace() {
        let toml = "[scripts]\nBackfill = \"--function scripts.Backfill -- --verbose=true\"\n";
        let result = RunArgs::parse_scripts(toml);
        assert_eq!(result.len(), 1);
        let tokens = result.get("Backfill").unwrap();
        assert_eq!(
            tokens,
            &vec!["--function", "scripts.Backfill", "--", "--verbose=true"]
        );
    }

    /// Array-form entry keeps each element verbatim (values with spaces preserved).
    #[test]
    fn test_parse_scripts_array_form_is_kept_verbatim() {
        let toml = r#"[scripts]
Greet = ["--function", "scripts.Greet", "--", "--name", "Ada Lovelace"]
"#;
        let result = RunArgs::parse_scripts(toml);
        assert_eq!(result.len(), 1);
        let tokens = result.get("Greet").unwrap();
        assert_eq!(tokens[0], "--function");
        assert_eq!(tokens[4], "Ada Lovelace"); // space preserved
    }

    /// Numeric TOML values (not string or array) are filtered out.
    #[test]
    fn test_parse_scripts_numeric_value_is_filtered_out() {
        let toml = "[scripts]\nbad = 42\ngood = \"--function main\"\n";
        let result = RunArgs::parse_scripts(toml);
        assert!(!result.contains_key("bad"), "numeric value must be filtered");
        assert!(result.contains_key("good"), "string value must be kept");
    }

    /// An empty array value is filtered out (no tokens to tokenize).
    #[test]
    fn test_parse_scripts_empty_array_is_filtered_out() {
        let toml = "[scripts]\nempty = []\ngood = \"--function main\"\n";
        let result = RunArgs::parse_scripts(toml);
        assert!(!result.contains_key("empty"), "empty array must be filtered");
        assert!(result.contains_key("good"));
    }

    /// Multiple entries are all captured correctly.
    #[test]
    fn test_parse_scripts_multiple_entries_all_captured() {
        let toml = "[scripts]\nA = \"--function A\"\nB = \"--function B\"\n";
        let result = RunArgs::parse_scripts(toml);
    ...
    // ========================================================================
    // format_value — Map rendering
    // ========================================================================

    /// A Map value renders as `{"key": value, ...}`.
    #[test]
    fn test_format_value_map_renders_entries() {
        let v = BexExternalValue::Map {
            key_type: ty_string(),
            value_type: ty_int(),
            entries: [
                ("alpha".to_string(), BexExternalValue::Int(1)),
                ("beta".to_string(), BexExternalValue::Int(2)),
            ]
            .into_iter()
            .collect(),
        };
        ...
        assert!(rendered.contains("alpha"), "got: {rendered}");
        assert!(rendered.contains("beta"), "got: {rendered}");
    }

    /// An empty Map renders as `{}`.
    #[test]
    fn test_format_value_empty_map_renders_as_braces() {
        let v = BexExternalValue::Map {
            key_type: ty_string(),
            value_type: ty_int(),
    ...
    // ========================================================================
    // external_to_json — Map rendering
    // ========================================================================

    /// A Map renders as a JSON object with key-value pairs.
    #[test]
    fn test_external_to_json_map() {
        let v = BexExternalValue::Map {
            key_type: ty_string(),
            value_type: ty_int(),
            entries: [
                ("x".to_string(), BexExternalValue::Int(10)),
                ("y".to_string(), BexExternalValue::Int(20)),
            ]
            .into_iter()
            .collect(),
        };
        let j = external_to_json(&v);
        assert_eq!(j["x"], serde_json::json!(10));
        assert_eq!(j["y"], serde_json::json!(20));
    }

    /// An Array of strings renders as a JSON array.
    #[test]
    fn test_external_to_json_array_of_strings() {
        let v = BexExternalValue::Array {
            element_type: ty_string(),
            items: vec![
    ...
    // ========================================================================
    // format_value — Union passthrough
    // ========================================================================

    /// A Union value renders by delegating to its inner value.
    #[test]
    fn test_format_value_union_delegates_to_inner() {
        use bex_engine::UnionMetadata;
        let inner = Box::new(BexExternalValue::Int(99));
        let union_ty = Ty::Union(vec![ty_int(), ty_string()], Default::default());
        let v = BexExternalValue::Union {
            value: inner,
            metadata: UnionMetadata::new(union_ty, ty_int()),
        };
        assert_eq!(format_value(&v), "99");
    }

    // ========================================================================
    // external_to_json — Union passthrough
    // ========================================================================

    /// external_to_json on a Union delegates to its inner value.
    #[test]
    fn test_external_to_json_union_delegates_to_inner() {
        use bex_engine::UnionMetadata;
        let inner = Box::new(BexExternalValue::Bool(false));
        let union_ty = Ty::Union(vec![ty_bool(), ty_int()], Default::default());
        let v = BexExternalValue::Union {
            value: inner,
            metadata: UnionMetadata::new(union_ty, ty_bool()),
        };
        assert_eq!(external_to_json(&v), serde_json::json!(false));
    }
}