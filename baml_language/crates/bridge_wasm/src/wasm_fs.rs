...
extern "C" {
    ...
    // readDirEntries(path) -> WasmVfsDirEntry[]
    //
    // Returns directory entries with type information attached, so callers
    // (e.g. baml.fs.read_dir, baml.glob.scan) don't have to issue an
    // additional `metadata` round-trip per entry. JS hosts that don't yet
    // implement this method cause the `Result` to resolve as an error; the
    // Rust-side caller falls back to readDir + per-entry metadata.
...
/// One directory entry returned by `readDirEntries`. Carries the same type
/// information `metadata` does, so a directory listing only needs one
/// JS-WASM round-trip total instead of N+1 (where N = number of entries).
#[derive(tsify::Tsify, Serialize, Deserialize)]
#[tsify(into_wasm_abi, from_wasm_abi)]
pub struct WasmVfsDirEntry {
    pub name: String,
    /// `"file"`, `"directory"`, or any host-specific string. WASM consumers
    /// only check for the `"file"`/`"directory"` cases today.
    pub file_type: String,
    pub is_symlink: bool,
}

/// Crate-internal accessor methods on `WasmVfs` for use by `wasm_io_fs` and
/// `wasm_io_glob`. The `extern "C"` bindings generated by `wasm_bindgen` are
/// inherently private to this module, so we expose them here.
impl WasmVfs {
    pub(crate) fn vfs_read_dir(&self, path: &str) -> Result<js_sys::Array, JsValue> {
        self.read_dir(path)
    }

    pub(crate) fn vfs_read_dir_entries(&self, path: &str) -> Result<js_sys::Array, JsValue> {
