...
pub enum RawRecord<'a> {
    ...
    CallFunction {
        ...
        parent_call_id: u64,
        /// Per-run function id (interim provider until the M0 id table).
        function_id: u32,
...
pub fn decode(buf: &[u8]) -> Result<(RawRecord<'_>, usize), DecodeError> {
    ...
    let rec = match tag {
        ...
        TAG_START_THREAD => {
            let flags = r.u8()?;
            let thread_id = r.u64()?;
            let parent_thread_id = r.u64()?;
            let parent_call_id = r.u64()?;
            let ts_ns = r.u64()?;
            let name_len = r.u16()?;
    ...
    fn roundtrip_all_variants() {
        for v in [0u64, 1, 42, u64::MAX] {
            roundtrip(RawRecord::CallFunction {
                flags: v as u8,
                thread_id: v,
                call_id: v.wrapping_add(1),
                parent_call_id: v / 2,
                function_id: v as u32,
                ts_ns: v,
            });
