...
pub fn check_file(db: &dyn Db, file: SourceFile) -> Vec<Diagnostic> {
    ...
    // ── 8. LLM capability markers (DCP §1.2) ─────────────────────────────────
    //
    // E0150–E0152: `//baml:llm_capability` interfaces must hang off
    // `baml.ai.Provider`; `//baml:llm_companion(<suffix>)` drivers must match
    // the driver convention and register a unique suffix.
    diagnostics.extend(check_llm_capability_markers(
        db,
        file,
        file_id,
        &ast_items,
...
/// `true` when `qtn` is the `baml.ai.Provider` marker interface.
fn is_provider_qtn(qtn: &QualifiedTypeName) -> bool {
    qtn.package().as_str() == "baml"
        && qtn.namespace().len() == 1
        && qtn.namespace()[0].as_str() == "ai"
        && qtn.name().as_str() == "Provider"
}

/// Walk `iface`'s transitive `requires` closure and report whether it reaches
/// `baml.ai.Provider`. Each parent's own `requires` clauses are resolved in
/// the *parent's* package/namespace context (the E0125 lesson — a stdlib
/// capability writes `requires Provider` unqualified inside `baml.ai`, which
/// must not be resolved against the user's package).
fn capability_requires_provider<'db>(
    db: &'db dyn Db,
    iface: &baml_compiler2_ast::InterfaceDef,
    pkg_items: &baml_compiler2_hir::package::PackageItems<'db>,
    namespace_path: &[Name],
    ...
    // Frontier entry: a `requires` target plus the (pkg_items, namespace)
    // context it must be resolved in.
    struct Pending<'db> {
        target: baml_compiler2_ast::TypeExpr,
        pkg_items: baml_compiler2_hir::package::PackageItems<'db>,
    ...
    let mut frontier: Vec<Pending<'db>> = iface
        .requires
        .iter()
        .map(|te| Pending {
            target: te.clone(),
        ...
        .collect();
    let mut visited: HashSet<QualifiedTypeName> = HashSet::new();

    while let Some(pending) = frontier.pop() {
        let Some(parent) = resolve_interface_path(
            db,
            &pending.target,
            &pending.pkg_items,
            &pending.namespace_path,
        ) else {
            continue; // unresolvable target — E0112/E0133 report it elsewhere
        };
        if is_provider_qtn(&parent.qtn) {
            return true;
        }
        if !visited.insert(parent.qtn.clone()) {
            continue;
        }
        // Recurse into the parent's own `requires`, in the parent's context.
        let parent_file = parent.loc.file(db);
        let parent_pkg_info = baml_compiler2_hir::file_package::file_package(db, parent_file);
        let parent_pkg_id =
            baml_compiler2_hir::package::PackageId::new(db, parent_pkg_info.package.clone());
        let parent_pkg_items = baml_compiler2_hir::package::package_items(db, parent_pkg_id);
        for grand_te in &parent.iface.requires {
            if matches!(&grand_te.kind, TypeExprKind::Path { segments, .. } if !segments.is_empty())
            {
                frontier.push(Pending {
                    target: grand_te.clone(),
                    pkg_items: parent_pkg_items.clone(),
                    namespace_path: parent_pkg_info.namespace_path.clone(),
                });
            }
        }
    }
...
/// `true` when `s` is a valid BAML identifier (companion-suffix rule).
fn is_valid_suffix_ident(s: &str) -> bool {
...
/// Validate the two LLM capability markers (DCP §1.2): E0150 capability
/// interfaces must reach `baml.ai.Provider` through `requires`; E0151 driver
/// functions must match the driver convention; E0152 companion suffixes must
/// be unique session-wide (first declaration wins).
fn check_llm_capability_markers<'db>(
    db: &'db dyn Db,
    file: SourceFile,
    file_id: FileId,
    items: &[baml_compiler2_ast::Item],
    ...
    use baml_compiler2_ast::{Item, TypeExprKind};
    use baml_compiler2_hir::diagnostic::Hir2Diagnostic;

    let mut diagnostics = Vec::new();

    // ── E0150: marked interfaces must reach baml.ai.Provider ────────────────
    for item in items {
        if let Item::Interface(iface) = item
            && iface.is_llm_capability
            && interface_has_cycle(db, iface, pkg_items, namespace_path).is_none()
            && !capability_requires_provider(db, iface, pkg_items, namespace_path)
        {
            diagnostics.push(
                Hir2Diagnostic::LlmCapabilityMissingProvider {
    ...
    // The marker only takes effect on top-level functions — flag it anywhere
    // else (class/implements methods, interface default methods).
    for item in items {
        let misplaced: Vec<&baml_compiler2_ast::FunctionDef> = match item {
            Item::Class(c) => c
                .methods
                .iter()
                .chain(c.implements.iter().flat_map(|b| b.methods.iter()))
                .filter(|m| m.llm_companion_suffix.is_some())
                .collect(),
            Item::Interface(i) => i
                .default_methods
                .iter()
        ...
        for m in misplaced {
            diagnostics.push(driver_invalid(
                &m.name,
                "the marker must be on a top-level function, not a method".to_string(),
                m.name_span,
            ));
        }
    }
    ...
    for item in items {
        ...
        let Some(suffix) = &f.llm_companion_suffix else {
            continue;
        };

        if !is_valid_suffix_ident(suffix.as_str()) {
            diagnostics.push(driver_invalid(
                &f.name,
                format!("suffix `{suffix}` is not a valid identifier"),
                f.name_span,
            ));
        }
        ...
        if f.params.len() < 2 {
            diagnostics.push(driver_invalid(
                &f.name,
                "expected at least 2 parameters: `client: baml.ai.Provider` then \
        ...
        let client_param = &f.params[0];
        if client_param.name.as_str() != "client" {
            diagnostics.push(driver_invalid(
                &f.name,
                format!(
                    "first parameter must be named `client`, found `{}`",
                    client_param.name
                ),
                client_param.span,
            ));
        } else {
            let resolves_to_provider = client_param.type_expr.as_ref().is_some_and(|te| {
                resolve_interface_path(db, te, pkg_items, namespace_path)
                    .is_some_and(|r| is_provider_qtn(&r.qtn))
            });
            if !resolves_to_provider {
                diagnostics.push(driver_invalid(
                    &f.name,
                    "first parameter `client` must be typed `baml.ai.Provider`".to_string(),
                    client_param.span,
                ));
            }
        }

        let prompt_param = &f.params[1];
        if prompt_param.name.as_str() != "prompt" {
            diagnostics.push(driver_invalid(
                &f.name,
                format!(
                    "second parameter must be named `prompt`, found `{}`",
                    prompt_param.name
                ),
                prompt_param.span,
            ));
        } else {
            // `PromptAst` is an opaque class, not an interface — a syntactic
            // last-segment check is the honest v1 (no class-identity resolver
            // on this path yet).
            let is_prompt_ast = prompt_param.type_expr.as_ref().is_some_and(|te| {
                matches!(
                    &te.kind,
                    TypeExprKind::Path { segments, .. }
                        if segments.last().is_some_and(|s| s.as_str() == "PromptAst")
                )
            });
            if !is_prompt_ast {
                diagnostics.push(driver_invalid(
                    &f.name,
                    "second parameter `prompt` must be typed `baml.llm.PromptAst`".to_string(),
                    prompt_param.span,
                ));
            }
        }
    ...
    // ── E0152: session-wide duplicate suffixes (first declaration wins) ─────
    ...
    for item in items {
        ...
        let Some(first) = registry.driver_for_suffix(suffix.as_str()) else {
            continue;
        };
        let this_is_first = first.file == file && first.function == f.name && first.span == f.span;
        if !this_is_first {
            diagnostics.push(
                Hir2Diagnostic::LlmCompanionSuffixDuplicate {
                    suffix: suffix.clone(),
                    existing: first.function.clone(),
                    span: f.name_span,
                }
                .to_diagnostic(file_id),
            );
