...
fn lower_base_type(
    ...
    if let Some(name) = type_expr.dotted_name() {
        ...
        if name == "map" && args.len() == 2 {
            let key = lower_type_expr_inner(&args[0], false, diags, owner);
            let value = lower_type_expr_inner(&args[1], false, diags, owner);
    ...
    if let Some(name) = parts.dotted_name() {
        ...
        if name == "map" {
            if type_arg_exprs.len() == 2 {
                let key = lower_type_expr_inner(&type_arg_exprs[0], false, diags, owner);
                let value = lower_type_expr_inner(&type_arg_exprs[1], false, diags, owner);
                return TypeExprKind::Map {
                    key: Box::new(key),
                    value: Box::new(value),
                    attrs: vec![],
