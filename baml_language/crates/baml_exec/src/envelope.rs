...
// embedded section of a packaged binary, and that `baml-host` reads back
// at startup. It wraps the compiled program with the entry metadata the
// host needs to dispatch — the target function to invoke and the output
// format to use when printing the return value.
...
///
/// Stable across `baml pack` / `baml-host` versions built from the same
/// source tree. Version-skew is the author's responsibility; a binary
/// packed by `baml pack` ships its own host.
