...
impl BytecodeCache {
    ...
    pub fn store_raw(&self, key: &CacheKey, payload: &[u8]) -> io::Result<()> {
        ensure_baml_gitignore(&self.dir);
...
mod tests {
    ...
    #[test]
    fn external_cache_does_not_manage_gitignore() {
        let dir = tempfile::tempdir().expect("tempdir");
        let cache_dir = dir.path().join("custom-cache");
        let cache = BytecodeCache::open(cache_dir.clone());
        let files = vec![("a.baml".to_string(), "fn x {}")];
        let key = compute_key(&dummy_inputs(&files));

        cache
            .store(&key, &Program::default())
            .expect("store in external cache");

        assert!(cache_dir.is_dir(), "cache storage was created");
        assert!(
            !cache_dir.join(".gitignore").exists(),
            "explicit cache directories remain user-managed"
        );
    }
