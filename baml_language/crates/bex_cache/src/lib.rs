...
const GENERATED_GITIGNORE: &[u8] = b"# Generated by BAML. Do not edit manually.\n\
# This file causes git and AI coding agents to skip generated output.\n\
*\n\
!.gitignore\n";
...
impl BytecodeCache {
    pub fn open(dir: PathBuf) -> Self {
        // The default cache lives below `<project>/.baml`. Put the ignore
        // marker at that generated-directory boundary so tools that honor
        // nested .gitignore files skip every BAML artifact, not just cache
        // entries. This is best-effort because caching must remain optional.
        ensure_baml_gitignore(&dir);
    ...
    pub fn store_raw(&self, key: &CacheKey, payload: &[u8]) -> io::Result<()> {
        ensure_cache_contents_ignored(&self.dir);
...
fn ensure_cache_contents_ignored(cache_dir: &Path) {
    if nearest_baml_dir(cache_dir).is_some() {
        ensure_baml_gitignore(cache_dir);
        return;
    }

    // Preserve the existing behavior for an explicit BAML_CACHE_DIR outside
    // `.baml`: keep that cache self-contained and do not rewrite a user-owned
    // ignore file.
...
fn ensure_baml_gitignore(cache_dir: &Path) {
    let Some(baml_dir) = nearest_baml_dir(cache_dir) else {
        return;
    };
    if fs::create_dir_all(baml_dir).is_err() {
        return;
    }
...
fn nearest_baml_dir(path: &Path) -> Option<&Path> {
    path.ancestors()
...
mod tests {
    ...
    #[test]
    fn project_local_cache_creates_gitignore_at_baml_root() {
        let dir = tempfile::tempdir().expect("tempdir");
        let baml_dir = dir.path().join("project/.baml");
        let cache_dir = baml_dir.join("cache");

        let _cache = BytecodeCache::open(cache_dir.clone());

        assert_eq!(
            fs::read(baml_dir.join(".gitignore")).expect("read generated .gitignore"),
            GENERATED_GITIGNORE
        );
        assert!(
            !cache_dir.join(".gitignore").exists(),
            "the marker belongs at .baml so sibling artifacts are ignored too"
        );
    }

    #[test]
    fn project_local_cache_refreshes_generated_gitignore() {
        let dir = tempfile::tempdir().expect("tempdir");
        let baml_dir = dir.path().join("project/.baml");
        fs::create_dir_all(&baml_dir).expect("create .baml");
        fs::write(baml_dir.join(".gitignore"), "*\n").expect("write old .gitignore");
