...
```bash
# Run all LSP tests
cargo test -p lsp_actions_tests test_files

# Run a specific test folder
cargo test -p lsp_actions_tests test_files::type_errors

# Run a specific test
cargo test -p lsp_actions_tests test_files::hover::test_class
```
...
```bash
UPDATE_EXPECT=1 cargo test -p lsp_actions_tests test_files
```
...
4. Run with `UPDATE_EXPECT=1` to generate expectations:
   ```bash
   UPDATE_EXPECT=1 cargo test -p lsp_actions_tests test_files::your_folder::test_your_file
   ```
