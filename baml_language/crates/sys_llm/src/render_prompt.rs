...
pub fn execute_render_prompt(args: &[BexValue]) -> Result<PromptAst, OpError> {
    ...
    // Call the Jinja runtime — now returns external PromptAst directly
    let prompt_ast = bex_jinja_runtime::render_prompt(template, &template_args, &render_ctx)?;

    Ok(prompt_ast)
}

/// Convert VM `bex_vm_types::PromptAst` to external `bex_external_types::PromptAst`.
pub fn vm_prompt_ast_to_external(
    ast: &bex_vm_types::PromptAst,
    heap: &std::sync::Arc<bex_heap::BexHeap>,
) -> PromptAst {
    match ast {
        bex_vm_types::PromptAst::String(s) => PromptAst::String(s.clone()),
        bex_vm_types::PromptAst::Media { handle, kind } => PromptAst::Media {
            handle: heap.create_handle(*handle),
            kind: *kind,
        },
        bex_vm_types::PromptAst::Message {
            ...
            let ext_content = vm_prompt_ast_to_external(content, heap);
        ...
        bex_vm_types::PromptAst::Vec(items) => {
            let ext_items: Vec<_> = items
                .iter()
                .map(|item| vm_prompt_ast_to_external(item, heap))
                .collect();
