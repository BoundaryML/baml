...
impl IoSysOpsBuilder {
    ...
    pub fn with_fs_instance(
        ...
        self.inner.baml_fs_read_dir = {
            let t = instance.clone();
            Arc::new(move |heap, args, ctx, call_id| {
                t.__glue_baml_fs_read_dir(heap, args, ctx, call_id)
            })
        };
        self.inner.baml_fs_mkdir = {
            let t = instance;
            Arc::new(move |heap, args, ctx, call_id| {
                t.__glue_baml_fs_mkdir(heap, args, ctx, call_id)
            })
        };
        self
    }
    ...
    pub fn with_glob_instance(
        ...
        self.inner.baml_glob_new = {
            let t = instance.clone();
            Arc::new(move |heap, args, ctx, call_id| {
                t.__glue_baml_glob_new(heap, args, ctx, call_id)
            })
        };
        self.inner.baml_glob_glob_scan = {
            let t = instance.clone();
            Arc::new(move |heap, args, ctx, call_id| {
                t.__glue_baml_glob_glob_scan(heap, args, ctx, call_id)
            })
        };
        self.inner.baml_glob_glob_matches = {
            let t = instance;
            Arc::new(move |heap, args, ctx, call_id| {
                t.__glue_baml_glob_glob_matches(heap, args, ctx, call_id)
            })
        };
        self
    }
