...
impl<'db> TypeInferenceBuilder<'db> {
    ...
    fn runtime_type_arg_params_for_call(
        ...
        let func_loc = match resolution {
            ...
            // An abstract interface method (`Animal.speak`) has no impl function;
            // its generic params are tracked separately, so treat it like a field.
        ...
        match resolution {
            ...
            MemberResolution::Field { .. }
            | MemberResolution::Variant { .. }
            | MemberResolution::InterfaceRequiredMethod { .. } => callee_generic_params.to_vec(),
        }
    }
