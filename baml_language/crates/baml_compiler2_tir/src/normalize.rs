...
fn ty_has_cycle(
    ...
    match ty {
        ...
        Ty::Class(_, type_args) | Ty::Interface(_, type_args) => type_args
            .iter()
            .any(|t| ty_has_cycle(t, aliases, visited, stack)),
        Ty::Function {
            params,
...
mod tests {
    ...
    fn same_normalized_type_accepts_union_reordering() {
        ...
        let int = int_ty();
        let string = string_ty();
        let lhs = union_of(vec![int.clone(), string.clone()]);
        let rhs = union_of(vec![string, int]);
    ...
    fn same_normalized_type_rejects_narrower_union_member() {
        ...
        let int = int_ty();
        let string = string_ty();
        let union = union_of(vec![int.clone(), string]);
    ...
    fn same_normalized_type_expands_aliases() {
        ...
        aliases.insert(qn("IntOrString"), union_of(vec![int_ty(), string_ty()]));
        let direct = union_of(vec![string_ty(), int_ty()]);
    ...
    fn test_simple_alias() {
        let mut aliases = HashMap::new();
        aliases.insert(qn("MyInt"), int_ty());

        assert!(is_subtype_of(&type_alias("MyInt"), &int_ty(), &aliases));
        assert!(is_subtype_of(&int_ty(), &type_alias("MyInt"), &aliases));
    }
    ...
    fn test_recursive_alias_detection() {
        let mut aliases = HashMap::new();
        aliases.insert(qn("List"), union_of(vec![null_ty(), type_alias("List")]));
    ...
    fn test_never_is_bottom() {
        ...
        assert!(is_subtype_of(&never_ty(), &int_ty(), &aliases));
        assert!(is_subtype_of(&never_ty(), &string_ty(), &aliases));
        assert!(is_subtype_of(
            &never_ty(),
            &Ty::Class(qn("Foo"), vec![]),
            &aliases
        ));
        assert!(is_subtype_of(&never_ty(), &opt_of(int_ty()), &aliases));
    }
    ...
    fn test_int_not_subtype_of_float() {
        ...
        // must be explicit.
        let aliases = HashMap::new();
        assert!(!is_subtype_of(&int_ty(), &float_ty(), &aliases));
        assert!(!is_subtype_of(&float_ty(), &int_ty(), &aliases));
    }
    ...
    fn test_int_not_subtype_of_bigint() {
        ...
        let aliases = HashMap::new();
        assert!(!is_subtype_of(&int_ty(), &bigint_ty(), &aliases));
    }
    ...
    fn test_function_covariant_return() {
        ...
        let returns_literal_string = Ty::Function {
            generic_params: Vec::new(),
            generic_param_bounds: Vec::new(),
            params: vec![required_param(int_ty())],
        ...
        let returns_variant = Ty::Function {
            ...
            generic_param_bounds: Vec::new(),
            params: vec![],
            ret: Box::new(Ty::EnumVariant(qn("Color"), Name::new("Red"))),
            throws: Box::new(never_ty()),
        };
    ...
    fn test_function_contravariant_params() {
        ...
        let accepts_string = Ty::Function {
            generic_params: Vec::new(),
            generic_param_bounds: Vec::new(),
            params: vec![required_param(string_ty())],
            ret: Box::new(string_ty()),
            throws: Box::new(never_ty()),
        };
        ...
        let accepts_enum = Ty::Function {
            generic_params: Vec::new(),
            generic_param_bounds: Vec::new(),
            params: vec![required_param(Ty::Enum(qn("Color")))],
            ret: Box::new(string_ty()),
            throws: Box::new(never_ty()),
        };
        ...
        let accepts_bigint = Ty::Function {
            generic_params: Vec::new(),
            generic_param_bounds: Vec::new(),
            params: vec![required_param(bigint_ty())],
            ret: Box::new(string_ty()),
            throws: Box::new(never_ty()),
        };
    ...
    fn test_function_optional_param_dropping_subtyping() {
        ...
        let with_two_optionals = Ty::Function {
            ...
            generic_param_bounds: Vec::new(),
            params: vec![
                required_param(string_ty()),
                optional_param("max", int_ty()),
                optional_param("filter", string_ty()),
            ],
            ret: Box::new(string_ty()),
            throws: Box::new(never_ty()),
        };
    ...
    fn test_function_optional_and_required_params_are_incomparable() {
        ...
        let optional = Ty::Function {
            generic_params: Vec::new(),
            generic_param_bounds: Vec::new(),
            params: vec![optional_param("value", int_ty())],
            ret: Box::new(string_ty()),
            throws: Box::new(never_ty()),
        };
    ...
    fn test_optional_subtyping() {
        let aliases = HashMap::new();
        // int <: int?
        assert!(is_subtype_of(&int_ty(), &opt_of(int_ty()), &aliases));
        // null <: int?
        assert!(is_subtype_of(&null_ty(), &opt_of(int_ty()), &aliases));
        // string NOT <: int?
        assert!(!is_subtype_of(&string_ty(), &opt_of(int_ty()), &aliases));
    }
    ...
    fn test_evolving_list_subtype_of_list() {
        ...
        assert!(is_subtype_of(
            &Ty::EvolvingList(Box::new(int_ty())),
            &list_of(int_ty()),
            &aliases
        ));
    ...
    fn test_evolving_list_is_invariant() {
        ...
        assert!(!is_subtype_of(
            &Ty::EvolvingList(Box::new(string_ty())),
            &list_of(int_ty()),
            &aliases
        ));
        ...
        assert!(is_subtype_of(
            &Ty::EvolvingList(Box::new(Ty::Literal(
                LiteralValue::Int(1),
                Freshness::Fresh
            ))),
            &list_of(int_ty()),
    ...
    fn test_evolving_list_never_is_bottom() {
        ...
        assert!(is_subtype_of(
            &Ty::EvolvingList(Box::new(never_ty())),
            &list_of(int_ty()),
            &aliases
        ));
    }
    ...
    fn test_evolving_map_subtype_of_map() {
        ...
        assert!(is_subtype_of(
            &Ty::EvolvingMap(Box::new(string_ty()), Box::new(int_ty())),
            &Ty::Map(Box::new(string_ty()), Box::new(int_ty())),
            &aliases
        ));
    }
    ...
    fn test_map_key_covariance_never() {
        ...
        assert!(is_subtype_of(
            &Ty::Map(Box::new(never_ty()), Box::new(never_ty())),
            &Ty::Map(Box::new(string_ty()), Box::new(Ty::BuiltinUnknown)),
            &aliases
        ));
    ...
    fn test_map_invariant_no_int_to_float_value_widening() {
        ...
        assert!(!is_subtype_of(
            &Ty::Map(Box::new(string_ty()), Box::new(int_ty())),
            &Ty::Map(Box::new(string_ty()), Box::new(float_ty())),
            &aliases
        ));
    }
    ...
    fn test_map_invariant_no_int_to_bigint_value_widening() {
        ...
        assert!(!is_subtype_of(
            &Ty::Map(Box::new(string_ty()), Box::new(int_ty())),
            &Ty::Map(Box::new(string_ty()), Box::new(bigint_ty())),
            &aliases
        ));
    }
    ...
    fn test_make_evolving() {
        ...
        assert_eq!(
            list_of(never_ty()).make_evolving(),
            Ty::EvolvingList(Box::new(never_ty()))
        );
        // Map(Never, Never) → EvolvingMap(Never, Never)
        assert_eq!(
            Ty::Map(Box::new(never_ty()), Box::new(never_ty())).make_evolving(),
            Ty::EvolvingMap(Box::new(never_ty()), Box::new(never_ty()))
        );
        // Non-empty List passes through
        assert_eq!(list_of(int_ty()).make_evolving(), list_of(int_ty()));
        // Non-container passes through
        assert_eq!(int_ty().make_evolving(), int_ty());
    }

    #[test]
    fn test_evolving_display() {
        assert_eq!(Ty::EvolvingList(Box::new(never_ty())).to_string(), "_[]");
        assert_eq!(
            Ty::EvolvingList(Box::new(int_ty())).to_string(),
            "int[] (evolving)"
        );
        assert_eq!(
            Ty::EvolvingMap(Box::new(never_ty()), Box::new(never_ty())).to_string(),
            "map<_, _>"
        );
        assert_eq!(
            Ty::EvolvingMap(Box::new(string_ty()), Box::new(int_ty())).to_string(),
    ...
    fn test_union_of_int_literals_subtype_of_int() {
        let aliases = HashMap::new();
        let sub = union_of(vec![
            Ty::Literal(LiteralValue::Int(1), Freshness::Fresh),
            Ty::Literal(LiteralValue::Int(2), Freshness::Fresh),
            Ty::Literal(LiteralValue::Int(3), Freshness::Fresh),
        ]);
        let sup = int_ty();
    ...
    fn test_list_of_literal_union_subtype_of_list_int() {
        let aliases = HashMap::new();
        let sub = list_of(union_of(vec![
            Ty::Literal(LiteralValue::Int(1), Freshness::Fresh),
            Ty::Literal(LiteralValue::Int(2), Freshness::Fresh),
        ]));
        let sup = list_of(int_ty());
        assert!(is_subtype_of(&sub, &sup, &aliases));
    }
    ...
    fn test_union_of_int_and_float_literals_not_subtype_of_float() {
        ...
        let sub = union_of(vec![
            Ty::Literal(LiteralValue::Int(1), Freshness::Fresh),
            Ty::Literal(LiteralValue::Float("2.0".to_string()), Freshness::Fresh),
        ]);
        let sup = float_ty();
        assert!(!is_subtype_of(&sub, &sup, &aliases));
    }
    ...
    fn test_typevar_subtype_of_union_containing_same_typevar() {
        let aliases = HashMap::new();
        let t = Ty::TypeVar(Name::new("T"));
        let union = union_of(vec![t.clone(), string_ty()]);
    ...
    fn test_typevar_not_subtype_of_union_without_same_typevar() {
        let aliases = HashMap::new();
        let t = Ty::TypeVar(Name::new("T"));
        let union = union_of(vec![int_ty(), string_ty()]);
    ...
    fn test_typevar_not_subtype_of_concrete() {
        ...
        assert!(!is_subtype_of(&t, &int_ty(), &aliases));
        // Int <: T should NOT hold
        assert!(!is_subtype_of(&int_ty(), &t, &aliases));
    }
    ...
    fn test_recursive_alias_through_class_type_arg_is_detected() {
        ...
        let mut aliases = HashMap::new();
        aliases.insert(qn("A"), Ty::Class(qn("Box"), vec![type_alias("A")]));
    ...
    fn test_recursive_alias_through_interface_type_arg_is_detected() {
        ...
        let mut aliases = HashMap::new();
        aliases.insert(qn("A"), Ty::Interface(qn("BoxLike"), vec![type_alias("A")]));
