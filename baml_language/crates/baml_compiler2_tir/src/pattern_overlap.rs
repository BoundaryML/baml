...
/// [`get_implements_block`](crate::interfaces::get_implements_block). Consulted only
...
/// (type, interface) pair.
pub(crate) type ImplementsOracle<'a> = &'a dyn Fn(&Ty, &baml_type::Interface) -> bool;

/// Everything [`pattern_overlap`] needs to know about the enclosing scope and program.
pub(crate) struct PatternOverlapEnv<'a> {
...
fn interface_membership_overlap(iface: &Ty, other: &Ty, env: &PatternOverlapEnv<'_>) -> Overlap {
    ...
    let realized = |t: &Ty| baml_type::RealizedTy::try_from(t).is_ok();
...
fn pattern_bounds_refute(
    ...
    for name in env.vars {
        ...
        if baml_type::RealizedTy::try_from(witness).is_err() {
            continue;
        }
        ...
        for bound in bounds {
            let bound = bound.map_tys(|t| crate::generics::substitute_ty(t, &witnesses));
            if bound
                .tys()
...
mod tests {
    ...
    fn constraint(name: &str) -> baml_type::Interface {
        baml_type::Interface::new(TypeName::local(Name::new(name)), vec![], vec![])
    }
