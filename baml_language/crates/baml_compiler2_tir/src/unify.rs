...
use baml_type::{FunctionParamTy, ParamTy, Ty, TypeName, normalize::TypeContext as _};
use rustc_hash::FxHashMap;

use crate::type_context::AliasEquivCtx;
...
pub(crate) const MAX_UNIFY_DEPTH: usize = 256;
...
pub(crate) fn contains_bound_typevar(ty: &Ty, generic_params: &[ParamTy]) -> bool {
    match ty {
...
pub(crate) fn var_under_union(param: &ParamTy, ty: &Ty) -> bool {
...
pub(crate) type EnumVariants<'a> = &'a dyn Fn(&TypeName) -> Option<Vec<Name>>;
...
pub(crate) fn nf(ty: &Ty, enum_variants: EnumVariants) -> Ty {
    match ty {
...
pub(crate) fn expand_alias_head(ty: &Ty, aliases: &std::collections::HashMap<TypeName, Ty>) -> Ty {
...
pub(crate) fn is_literal_subtype(member: &Ty, candidate: &Ty) -> bool {
...
pub(crate) fn chase_var(ty: &Ty, vars: &[ParamTy], bindings: &TypeBindings) -> Ty {
    let mut current = ty.clone();
