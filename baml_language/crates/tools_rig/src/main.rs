...
fn main() {
    ...
    for language in &languages {
        for fixture_name in fixtures {
            let crate_name = format!("{language}_{fixture_name}");
    ...
    if check_mode && has_diff {
        eprintln!("\nRun `cargo run -p tools_rig` to regenerate.");
        std::process::exit(1);
    } else if !check_mode {
...
fn generate_from_template(template_dir: &Path, output_dir: &Path, fixture_name: &str) {
    ...
    for file in ["Cargo.toml", "build.rs", "src/lib.rs"] {
        let template_path = template_dir.join(format!("{file}.template"));
        ...
        let header = format!("Generated by tools_rig from crate_templates/{language}");
        let comment_prefix = if file.ends_with(".toml") { "#" } else { "//" };
        let with_header = format!("{comment_prefix} {header}\n{rendered}");
...
fn check_differences(template_dir: &Path, output_dir: &Path, fixture_name: &str) -> Option<String> {
    ...
    for file in ["Cargo.toml", "build.rs", "src/lib.rs"] {
        ...
        let expected_header = format!("Generated by tools_rig from crate_templates/{language}");
        let comment_prefix = if file.ends_with(".toml") { "#" } else { "//" };
        ...
        if first_line != expected_line {
            return Some(format!(
                "{file} (missing or invalid generation header, expected: '{expected_line}')"
            ));
        }

        // Add header to template content to match what was generated
        let template_with_header = format!("{expected_line}\n{template_content}");
        ...
        if template_formatted != actual_formatted {
            ...
            return Some(format!(
                "{file} ({template_lines} lines vs {actual_lines}, {template_bytes} bytes vs {actual_bytes})"
            ));
        }

        // For build.rs, validate that embedded scripts have generation headers
        if file == "build.rs" {
            let expected_sh_header =
                format!("# Generated by tools_rig from crate_templates/{language}");
            let expected_ps1_header = expected_sh_header.clone();

            if !actual_content.contains(&expected_sh_header) {
                return Some(format!(
                    "build.rs (missing generation header in test.sh script, expected: '{expected_sh_header}')"
                ));
            }

            if !actual_content.contains(&expected_ps1_header) {
                return Some(format!(
                    "build.rs (missing generation header in test.ps1 script, expected: '{expected_ps1_header}')"
                ));
            }
        }
    }
