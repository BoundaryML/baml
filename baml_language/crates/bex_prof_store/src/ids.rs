...
#[cfg(target_arch = "wasm32")]
static WORKERD_UUID_STATE: AtomicU64 = AtomicU64::new(0);
...
#[cfg(any(target_arch = "wasm32", test))]
fn non_cryptographic_uuid_v4(state: u64) -> uuid::Uuid {
    let high = splitmix64(state);
    let low = splitmix64(state ^ 0x4241_4D4C_2D43_4657);
...
/// Select the non-cryptographic `UUIDv4` source used by the workerd package.
///
/// The workerd package calls this once, before generated bytecode is staged,
/// because workerd forbids both Web Crypto and `Math.random()` during module
/// evaluation. Other WebAssembly hosts never call this entry point and keep
/// using [`uuid::Uuid::new_v4`], exactly like native targets.
#[cfg(target_arch = "wasm32")]
pub fn configure_workerd_uuid_seed(seed: u64) -> Result<(), &'static str> {
    if PROCESS_EUID.get().is_some() {
        return Err(
            "the workerd UUID source must be configured before the process identity is minted",
        );
    }
    let seeded = splitmix64(seed ^ 0x4241_4D4C_2D57_524B) | 1;
    WORKERD_UUID_STATE
        .compare_exchange(0, seeded, Ordering::Relaxed, Ordering::Relaxed)
        .map(|_| ())
        .map_err(|_| "the workerd UUID source is already configured")
}

fn random_uuid_v4() -> uuid::Uuid {
    #[cfg(target_arch = "wasm32")]
    {
        let state = WORKERD_UUID_STATE.load(Ordering::Relaxed);
        if state != 0 {
            let next = WORKERD_UUID_STATE.fetch_add(0x9E37_79B9_7F4A_7C15, Ordering::Relaxed);
            return non_cryptographic_uuid_v4(next);
        }
    }
    uuid::Uuid::new_v4()
}
...
mod tests {
    ...
    #[test]
    fn non_cryptographic_uuid_has_v4_variant_and_is_deterministic() {
        let first = non_cryptographic_uuid_v4(123);
        let repeated = non_cryptographic_uuid_v4(123);
        let next = non_cryptographic_uuid_v4(124);
