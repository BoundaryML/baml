...
#[test]
fn test_evaluate_baml_null_alias() {
    let types = PredefinedTypes::default(JinjaContext::Prompt);
    assert_eq!(assert_evaluates_to!("null", &types), Type::None);
}
...
fn test_enum() {
    ...
    assert_eq!(
        ...
        vec![
            "Comparing enum VALUE_A to string variable - enum-string comparisons will soon be deprecated. Please see https://github.com/BoundaryML/baml/issues/2339."
        ]
    );
}
...
fn test_output_format() {
    ...
    assert_eq!(
        ...
        vec![
            "Function 'baml::OutputFormat' expects argument 'always_hoist_enums' to be of type (none | bool), but got literal[1]"
        ]
    );
    ...
    assert_eq!(
        ...
        vec![
            "Function 'baml::OutputFormat' expects argument 'hoisted_class_prefix' to be of type (none | string), but got literal[1]"
        ]
    );
...
fn sum_filter() {
    ...
    assert_eq!(
        ...
        vec![r#"'[hi,1]' is a list[(literal[1] | literal["hi"])], expected (int|float)[]"#]
    );
}
