...
use super::{TypeError, expr::evaluate_type, types::PredefinedTypes};
...
pub(crate) fn get_variable_types(stmt: &Stmt, state: &mut PredefinedTypes) -> Vec<TypeError> {
...
/// Γ: { a: Number | null }
/// (!!!a) -> []
pub(super) fn predicate_implications<'a>(
    expr: &'a ast::Expr<'a>,
...
pub(super) fn truthy(ty: &Type) -> Option<Type> {
    match ty {
        Type::Unknown => None,
        Type::Undefined => None,
        Type::None => None,
