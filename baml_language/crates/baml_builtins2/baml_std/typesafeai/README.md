...
The adapter's plan and decoding are written in BAML reflection, with no TypeSafe-specific native operations. Enum and class construction use `unreflect` with `baml.json.from_string<T>`. Literal values, including enum-variant literals, come from `reflect.literal.Type.value()`. Skip flags come from reflection: `reflect.Meta.skip` on class fields and enum variants. Skipped class fields produce no questions and are filled with null; their declared types must accept null (including `unknown`), otherwise validation fails before HTTP. `ai.ModelTurn.parsed_output` carries the reconstructed typed result through the runner, avoiding an additional heuristic SAP parse. Its wrapper distinguishes a decoded null from an absent decoded result, and the runner checks the value against its output type.
