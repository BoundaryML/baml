...
pub enum PpirTy {
    ...
    /// Type constructors.
    Optional { inner: Box<PpirTy>, attrs: PpirTypeAttrs },
    List { inner: Box<PpirTy>, attrs: PpirTypeAttrs },
    Map { key: Box<PpirTy>, value: Box<PpirTy>, attrs: PpirTypeAttrs },
    Union { variants: Vec<PpirTy>, attrs: PpirTypeAttrs },

    /// Literal types.
    StringLiteral { value: std::string::String, attrs: PpirTypeAttrs },
    IntLiteral { value: i64, attrs: PpirTypeAttrs },
    BoolLiteral { value: bool, attrs: PpirTypeAttrs },

    /// Media types.
    Media { kind: baml_base::MediaKind, attrs: PpirTypeAttrs },
...
impl PpirTy {
    ...
    pub fn clone_without_attrs(&self) -> Self {
        ...
        match self {
            Self::Named { name, .. } => Self::Named { name: name.clone(), attrs: d },
            ...
            Self::Optional { inner, .. } => Self::Optional { inner: inner.clone(), attrs: d },
            Self::List { inner, .. } => Self::List { inner: inner.clone(), attrs: d },
            Self::Map { key, value, .. } => Self::Map {
                key: key.clone(),
                value: value.clone(),
                attrs: d,
            },
            Self::Union { variants, .. } => Self::Union { variants: variants.clone(), attrs: d },
            Self::StringLiteral { value, .. } => Self::StringLiteral { value: value.clone(), attrs: d },
            Self::IntLiteral { value, .. } => Self::IntLiteral { value: *value, attrs: d },
            Self::BoolLiteral { value, .. } => Self::BoolLiteral { value: *value, attrs: d },
            Self::Media { kind, .. } => Self::Media { kind: *kind, attrs: d },
    ...
    pub fn named(name: impl Into<Name>) -> Self {
        PpirTy::Named { name: name.into(), attrs: PpirTypeAttrs::default() }
    }

    /// Create a list type.
    pub fn list(inner: PpirTy) -> Self {
        PpirTy::List { inner: Box::new(inner), attrs: PpirTypeAttrs::default() }
    }

    /// Create an optional type.
    pub fn optional(inner: PpirTy) -> Self {
        PpirTy::Optional { inner: Box::new(inner), attrs: PpirTypeAttrs::default() }
    }

    /// Create a union type.
    pub fn union(types: Vec<PpirTy>) -> Self {
        PpirTy::Union { variants: types, attrs: PpirTypeAttrs::default() }
    }
    ...
    pub fn from_type_name(name: &str) -> Self {
        ...
        match name {
            ...
            "image" => PpirTy::Media { kind: baml_base::MediaKind::Image, attrs: d },
            "audio" => PpirTy::Media { kind: baml_base::MediaKind::Audio, attrs: d },
            "video" => PpirTy::Media { kind: baml_base::MediaKind::Video, attrs: d },
            "pdf" => PpirTy::Media { kind: baml_base::MediaKind::Pdf, attrs: d },
            _ => PpirTy::Named { name: SmolStr::new(name), attrs: d },
        }
    }
    ...
    fn from_union_member(parts: &baml_compiler_syntax::ast::UnionMemberParts) -> Self {
        ...
        PpirTy::Unknown { attrs: PpirTypeAttrs::default() }
    }
...
/// processes these into `PpirExpandedField`s.
#[derive(Debug, Clone)]
pub struct PpirField {
    ...
    /// Parsing of `<arg>` is deferred to Phase 3 (PPIR → HIR).
    pub starts_as: Option<SyntaxNode>,
    /// `@stream.not_null` → desugared to `starts_as = "never"`.
    pub not_null: bool,
    /// True when `@@stream.done` is on the enclosing class.
    /// Distributes `starts_as=never` at low priority (field-level `@stream.starts_as` overrides).
    /// Distinct from type-level `@stream.done` which only affects expansion, not starts_as.
    pub class_stream_done: bool,
