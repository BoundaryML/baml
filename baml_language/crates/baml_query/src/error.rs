//! Typed query errors (TASK/baml-query-scope.md §5.7).
//!
...
//! retryability — never secrets, raw customer bodies, or private
//! physical layout.
...
pub enum QueryErrorCode {
    ...
    /// have. Decision-frozen spelling: `E_BACKEND_CAPABILITY`.
    BackendCapability,
...
impl QueryError {
    ...
    /// Planning-time backend rejection: named function, required backend,
    /// current backend — before any data read.
    #[must_use]
...
/// A did-you-mean remedy against a set of known names (house style from
/// `baml describe`): the closest name by Jaro-Winkler above 0.75, if any.
#[must_use]
pub fn did_you_mean<'a>(unknown: &str, known: impl Iterator<Item = &'a str>) -> Option<String> {
    let mut best: Option<(f64, &str)> = None;
    for candidate in known {
        let score = strsim::jaro_winkler(unknown, candidate);
        if score > 0.75 && best.is_none_or(|(s, _)| score > s) {
