...
impl LoweringContext<'_> {
    ...
    fn enclosing_generic_params(&self) -> Vec<ParamTy> {
        ...
        let mut params = baml_compiler2_hir_ty::lower::function_generic_frame(self.db, fl);
    ...
    fn lower_explicit_type_args(
        ...
        for (idx, type_arg) in ast_type_args.iter().enumerate() {
            ...
            let AstTypeArg::Static(type_arg) = type_arg else {
                unreachable!("all TypeArg variants handled above")
            };
...
impl LoweringContext<'_> {
    fn lower_stmt(&mut self, stmt_id: AstStmtId) {
        ...
        match stmt {
            ...
            // Slice 7 turns this into `BindType`; Phase A still evaluates the
            // hidden operand exactly once so its effects are preserved.
            AstStmt::TypeBinding { value, .. } => {
                let ty = self.expr_ty(value);
                let temp = self.builder.temp(ty);
                self.lower_expr(value, Place::local(temp));
            }
