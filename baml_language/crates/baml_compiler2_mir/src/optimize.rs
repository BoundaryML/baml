...
fn collect_place_index_locals(body: &MirFunctionBody) -> HashSet<Local> {
    ...
    fn scan_rvalue(rv: &crate::Rvalue, set: &mut HashSet<Local>) {
        match rv {
            ...
            crate::Rvalue::MakeInstantiatedFunction { .. } => {
                // No local operands — only type-arg templates.
            }
...
fn apply_subst_to_rvalue(rv: &mut crate::Rvalue, subst: &HashMap<Local, Operand>) {
    match rv {
        ...
        crate::Rvalue::MakeInstantiatedFunction { .. } => {
            // No local operands to substitute.
        }
        crate::Rvalue::LoadType(_) => {
