...
fn render_type_value_source(vm: &BexVm, type_value: &TypeValue) -> String {
    ...
    for ptr in type_value.defs().enums.values() {
    ...
    for ptr in type_value.defs().classes.values() {
    ...
    let root_is_declared = matches!(
        &type_value.ty,
        baml_type::RealizedTy::Class(name, _, _) if type_value.defs().classes.contains_key(name)
    ) || matches!(
        &type_value.ty,
        baml_type::RealizedTy::Enum(name, _) if type_value.defs().enums.contains_key(name)
    );
    if !root_is_declared {
