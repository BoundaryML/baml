...
/// Encode an optional error-span codepoint offset as a BAML value.
fn offset_value(chars: usize) -> Value {
    Value::try_int(offset_int(chars)).unwrap_or(Value::NULL)
}

/// Build a capture with a shared substring and precomputed codepoint offsets.
...
fn match_values(
    ...
    let spans: Vec<usize> = matches
        .iter()
        .flat_map(|raw| &raw.groups)
        .flatten()
    ...
    matches
        .iter()
        .map(|raw| {
            let count = raw.groups.iter().flatten().count() * 2;
            let (offsets, rest) = remaining.split_at(count);
            remaining = rest;
            match_value(vm, hay, raw, names, offsets)
        })
        .collect()
}

/// Materialize one match using the already-converted codepoint offsets.
fn match_value(
    vm: &mut BexVm,
    hay: &BexStr,
    raw: &RawMatch,
...
/// Materialize a classified construction error and translate its byte span to codepoints.
...
/// Raise a fresh BAML regex error, preserving fatal errors during value construction.
...
impl BamlClassRegexRegex for PackageBamlImpl {
    ...
    fn match_(
        ...
        let found = prog
            ...
            .map_err(|abort| aborted(&prog, abort))?;
        Ok(match_values(vm, haystack, found.as_slice(), prog.names()).pop())
    }
    ...
    fn match_all(
        ...
        let found = prog
            ...
            .map_err(|abort| aborted(&prog, abort))?;
        Ok(match_values(vm, haystack, &found, prog.names()))
    }
    ...
    /// Return shared substrings for fields and participating delimiter captures.
    fn split(
        vm: &mut BexVm,
        regex: &Value,
        haystack: &BexStr,
    ...
    /// Expand a regex replacement template for only the first match.
    ...
    /// Expand a regex replacement template for every non-overlapping match.
...
impl BamlNamespaceRegex for PackageBamlImpl {
    /// Search from a codepoint offset without materializing a suffix.
    #[expect(clippy::cast_possible_wrap)]
    fn _index_of_from(haystack: &BexStr, search: &BexStr, start: i64) -> Option<i64> {
        let start = usize::try_from(start).ok()?;
        haystack
            .char_index_of_from(search.as_str(), start)
            .map(|i| i as i64)
    }

    /// Compile the requested dialect or throw its classified construction error.
    fn _compile(
        vm: &mut BexVm,
        pattern: &BexStr,
        backtracking: bool,
    ...
    /// Quote literal text for insertion into either regex dialect.
