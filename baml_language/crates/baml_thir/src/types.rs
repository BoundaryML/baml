...
impl fmt::Display for Ty {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Ty::Pdf => write!(f, "pdf"),
            Ty::Class(id) => write!(f, "{}", id.name),
            Ty::Enum(id) => write!(f, "{}", id.name),
...
mod tests {
    ...
    fn test_subtype_same() {
        ...
        assert!(Ty::String.is_subtype_of(&Ty::String));
    }

    #[test]
    fn test_subtype_numeric_widening() {
        assert!(Ty::Int.is_subtype_of(&Ty::Float));
        assert!(!Ty::Float.is_subtype_of(&Ty::Int));
    }

    #[test]
    fn test_subtype_optional() {
        let opt_int = Ty::Optional(Box::new(Ty::Int));
        assert!(Ty::Int.is_subtype_of(&opt_int));
        assert!(Ty::Null.is_subtype_of(&opt_int));
        assert!(!Ty::String.is_subtype_of(&opt_int));
    }

    #[test]
    fn test_subtype_union() {
        let union = Ty::Union(vec![Ty::Int, Ty::String]);
        assert!(Ty::Int.is_subtype_of(&union));
        assert!(Ty::String.is_subtype_of(&union));
        assert!(!Ty::Bool.is_subtype_of(&union));
    }

    #[test]
    fn test_subtype_list_covariance() {
        let list_int = Ty::List(Box::new(Ty::Int));
        let list_float = Ty::List(Box::new(Ty::Float));
