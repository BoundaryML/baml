...
//!
//! These functions convert `ppir::PpirTy`, `ppir::Class`, and
...
/// Convert a `ppir::PpirTy` to an `hir::TypeRef`.
pub(crate) fn convert_ppir_ty(ppir_ty: &baml_compiler_ppir::PpirTy) -> TypeRef {
    use baml_compiler_ppir::PpirTy as PT;
    match ppir_ty {
        ...
        PT::Optional { inner, .. } => TypeRef::Optional(Box::new(convert_ppir_ty(inner))),
        PT::List { inner, .. } => TypeRef::List(Box::new(convert_ppir_ty(inner))),
        PT::Map { key, value, .. } => TypeRef::Map {
            key: Box::new(convert_ppir_ty(key)),
            value: Box::new(convert_ppir_ty(value)),
        },
        PT::Union { variants, .. } => TypeRef::Union(variants.iter().map(convert_ppir_ty).collect()),
        PT::StringLiteral { value, .. } => TypeRef::StringLiteral(value.clone()),
        PT::IntLiteral { value, .. } => TypeRef::IntLiteral(*value),
        PT::BoolLiteral { value, .. } => TypeRef::BoolLiteral(*value),
...
pub(crate) fn convert_ppir_class(ppir_class: &baml_compiler_ppir::Class) -> Class {
    let fields = ppir_class
        ...
        .map(|pf| Field {
            name: pf.name.clone(),
            type_ref: convert_ppir_ty(&pf.type_ref),
...
pub(crate) fn convert_ppir_type_alias(ppir_alias: &baml_compiler_ppir::TypeAlias) -> TypeAlias {
    TypeAlias {
        ...
        type_ref: convert_ppir_ty(&ppir_alias.type_ref),
    }
}
...
pub(crate) fn convert_ppir_normalized(
    ...
    NormalizedStreamAnnotations {
        stream_type: convert_ppir_ty(&ppir_field.stream_type),
        in_progress_never: ppir_field.in_progress_never,
        starts_as: ppir_field.starts_as.clone(),
        typeof_s: ppir_field.typeof_s.as_ref().map(convert_ppir_ty),
    }
}
