...
//!   [`collect_compiler2_diagnostics`], [`collect_compiler2_diagnostics_narrowed`],
//!   [`collect_package_level_diagnostics`]) and the parallel drivers
//!   ([`check_files_parallel`], [`prime_file_indexes_parallel`]) over
//!   [`ProjectDatabase`], plus its `check()` / `check_file()` methods.
//!
//! ## Per-file pipeline
//!
...
//!    construction.
//! 3. **TIR2 scope diagnostics** — inference findings per body owner,
//!    resolved to source ranges via the body's source map. Calls
//!    `infer_body` (Salsa-cached per owner) internally.
//! 4. **TIR2 structural diagnostics** — type errors in signatures, class
//!    field annotations, type alias bodies, interface impls (all
//!    Salsa-cached per item).
//!
//! ## Example
//!
//! ```ignore
...
///
/// Files checked are [`baml_compiler2_hir::compiler2_all_files`]: every
/// source root's files — workspace sources, source-bearing dependencies, and
/// the compiler2 stdlib under `<builtin>/...` (packages `baml`, `env`, etc.).
///
...
pub fn check_files_parallel(db: &ProjectDatabase, files: &[SourceFile]) -> Vec<Vec<Diagnostic>> {
    ...
    rayon::scope(move |s| {
        for ((chunk_index, chunk), db) in chunks.into_iter().zip(handles) {
            let tx = tx.clone();
            s.spawn(move |_| {
                let out: Vec<Vec<Diagnostic>> =
                    chunk.iter().map(|file| check_file(&db, *file)).collect();
...
pub fn collect_compiler2_diagnostics_narrowed(
    ...
    // one `check_file` call, appended contiguously).
    if checked.len() > 8 {
        collect_file_diagnostics_parallel(db, &checked, &mut fresh);
    } else {
        for file in &checked {
            fresh.extend(check_file(db, *file));
        }
    }
...
fn rich_source_aware_tir_type_error_message(
    ...
    let ty = |ty: &Ty| display_ty_for_file(db, file, ty);
    match error {
...
// ── File-contextual type rendering ───────────────────────────────────────────

/// Context for file-relative type rendering: knows the file's current package
/// and namespace so qualified names collapse to the shortest unambiguous form
/// (bare when in scope, `root.path` when not, the dependency package prefix
...
/// to their bare (or `root.`-qualified) spelling, cross-package names keep
...
mod tests {
    ...
    use super::*;
    use crate::testing::setup_test_db;

    /// A single-file workspace plus that file.
    fn single_file(source: &str) -> (ProjectDatabase, SourceFile) {
...
"#,
        ...
        let messages = check_file(&db, file)
            .into_iter()
            .map(|diag| diag.message)
            .collect::<Vec<_>>();
...
"#,
    ...
    fn test_check_parse_error() {
        let (db, _file) = single_file("class Foo {");
