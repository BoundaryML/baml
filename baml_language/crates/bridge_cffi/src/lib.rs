...
mod tests {
    use bridge_ctypes::{
        ...
        baml_bridge::cffi::{
            BamlHandle, BamlHandleType, BamlTy, BamlTyArg, BamlTyAssociatedTypeProjection,
            CallFunctionArgs, InboundMapEntry, InboundValue, baml_ty::Ty, inbound_map_entry::Key,
            inbound_value::Value,
        },
    };
    use prost::Message as _;
    ...
    #[test]
    fn decode_function_call_args_rejects_invalid_protobuf() {
        assert!(matches!(
            decode_function_call_args(&[0x80]),
            Err(BridgeError::Ctypes(
                bridge_ctypes::CtypesError::ProtobufDecode(_)
            ))
        ));
    }

    #[test]
    fn decode_function_call_args_rejects_zero_call_id() {
        assert!(matches!(
            decode_function_call_args(&CallFunctionArgs::default().encode_to_vec()),
            Err(BridgeError::InvalidCallId)
        ));
    }

    #[test]
    fn decode_function_call_args_preserves_type_args_and_handles() {
        let key = HANDLE_TABLE.insert(CffiHandleTableEntry::FunctionRef { global_index: 17 });
        let encoded = CallFunctionArgs {
            kwargs: vec![InboundMapEntry {
                key: Some(Key::StringKey("callback".to_string())),
            ...
            type_args: vec![BamlTyArg {
                ...
                type_value: Some(bridge_ctypes::runtime_ty_to_proto_ty(
                    &bex_project::RuntimeTy::int(),
                )),
            }],
        }
        .encode_to_vec();
        ...
        let (kwargs, context) =
            decode_function_call_args(&encoded).expect("call arguments should decode");

        assert_eq!(context.host_call_id, bex_project::CallId(42));
        assert_eq!(
            context.type_args.get("T"),
            Some(&bex_project::RuntimeTy::int())
        );
        assert!(matches!(
            kwargs.0.get("callback"),
            Some(bex_project::BexExternalValue::FunctionRef { global_index: 17 })
        ));
        assert!(HANDLE_TABLE.resolve(key).is_none());
    }

    #[test]
    fn decode_function_call_args_validates_type_args_before_draining_handles() {
        let key = HANDLE_TABLE.insert(CffiHandleTableEntry::FunctionRef { global_index: 17 });
        let encoded = CallFunctionArgs {
            kwargs: vec![InboundMapEntry {
                key: Some(Key::StringKey("callback".to_string())),
            ...
            type_args: vec![BamlTyArg {
                type_var: "T".to_string(),
                type_value: Some(BamlTy {
                    ty: Some(Ty::AssociatedTypeProjection(Box::new(
        ...
        assert!(decode_function_call_args(&encoded).is_err());
