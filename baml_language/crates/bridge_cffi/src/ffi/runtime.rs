...
/// Create an independent dynamic BAML runtime.
///
/// # Arguments
...
///
/// # Returns
/// Owned opaque handle on success, null on failure. Release with destroy_baml_runtime.
...
pub extern "C" fn create_baml_runtime(
    ...
    ffi_safe_ptr(|| -> Result<*const libc::c_void, String> {
        if root_path.is_null() || src_files_json.is_null() {
            return Err("null source runtime input".into());
        }
        // Parse root_path
        ...
        let runtime = initialize_runtime(root_path_str, src_files)
            .map_err(|e| format!("Failed to initialize runtime: {e}"))?;

        let key = crate::runtime_key(&runtime).map_err(|e| e.to_string())?;
        Ok(Box::into_raw(Box::new(key)).cast())
    })
}
...
/// Release a dynamic source runtime handle.
///
/// # Safety
/// The pointer must be null or a live handle returned by create_baml_runtime.
/// Each handle may be destroyed exactly once.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn destroy_baml_runtime(runtime: *const libc::c_void) {
    if !runtime.is_null() {
        let key = unsafe { Box::from_raw(runtime.cast_mut().cast::<u64>()) };
        let _ = crate::unregister_runtime(*key);
    }
}
...
/// Register generated bytecode. Empty status means success; caller frees every status.
/// Register an immutable generated program under its content identity.
///
/// # Safety
/// `bytecode` must point to `length` readable bytes for this call. If non-null,
/// `metadata` must point to a valid NUL-terminated UTF-8 string.
pub unsafe extern "C" fn register_program_ffi(
    key: u64,
    bytecode: *const u8,
    length: usize,
    ...
    runtime_status(|| {
        let bytes = unsafe { runtime_bytes(bytecode, length) }?;
        let metadata = if metadata.is_null() {
            None
        } else {
            Some(
                unsafe { CStr::from_ptr(metadata) }
                    .to_str()
                    .map_err(|e| e.to_string())?,
            )
        };
        crate::register_runtime_from_bytecode(key, bytes, metadata)
            .map(|_| ())
            .map_err(|e| e.to_string())
    })
}

/// Create an independent dynamic registration. Write the uint64 key only on success.
/// Create an independently owned dynamic runtime.
///
/// # Safety
/// `bytecode` must point to `length` readable bytes for this call, and `out_key`
/// must be aligned and writable for one `u64`.
pub unsafe extern "C" fn create_runtime_ffi(
    bytecode: *const u8,
    length: usize,
    out_key: *mut u64,
) -> Buffer {
    runtime_status(|| {
        if out_key.is_null() {
            return Err("runtime key output is null".into());
        }
        let bytes = unsafe { runtime_bytes(bytecode, length) }?;
        let runtime =
            crate::initialize_runtime_from_bytecode(bytes, None).map_err(|e| e.to_string())?;
        let key = crate::runtime_key(&runtime).map_err(|e| e.to_string())?;
        unsafe {
            out_key.write(key);
        }
...
/// Remove a dynamic registration; already dispatched calls retain ownership.
pub extern "C" fn unregister_runtime_ffi(key: u64) -> Buffer {
    runtime_status(|| {
        crate::unregister_runtime(key)
            .map(|_| ())
            .map_err(|e| e.to_string())
    })
}

unsafe fn runtime_bytes<'a>(pointer: *const u8, length: usize) -> Result<&'a [u8], String> {
    if length > isize::MAX as usize || (pointer.is_null() && length != 0) {
        return Err("invalid bytecode pointer or length".into());
    }
    Ok(if length == 0 {
        &[]
    } else {
...
fn runtime_status(operation: impl FnOnce() -> Result<(), String>) -> Buffer {
    match std::panic::catch_unwind(AssertUnwindSafe(operation)) {
        Ok(Ok(())) => Buffer::from(Vec::new()),
        Ok(Err(error)) => Buffer::from(error.into_bytes()),
        Err(_) => Buffer::from(b"panic in BAML runtime registration".to_vec()),
    }
}

/// Compute the canonical generated uint64 identity without constructing a runtime.
/// Compute the stable generated identity of a program artifact.
///
/// # Safety
/// `bytecode` must point to `length` readable bytes for this call, and `out_key`
/// must be aligned and writable for one `u64`.
pub unsafe extern "C" fn program_key_ffi(
    bytecode: *const u8,
    length: usize,
    out_key: *mut u64,
) -> Buffer {
    runtime_status(|| {
        ...
        let bytes = unsafe { runtime_bytes(bytecode, length) }?;
        let key = baml_program_identity::program_key(bytes)?;
        unsafe {
            out_key.write(key);
        }
        Ok(())
