...
fn render_minijinja(
    ...
    if !rendered.contains(MAGIC_CHAT_ROLE_DELIMITER) && !rendered.contains(MAGIC_MEDIA_DELIMITER) {
        return Ok(PromptAst::without_span(PromptAstNode::Str(rendered)));
    }
        ...
        if chunk.starts_with(":baml-start-baml:") && chunk.ends_with(":baml-end-baml:") {
            ...
            for part in chunk.split(MAGIC_MEDIA_DELIMITER) {
                    ...
                    && part.ends_with(":baml-end-media:")
                {
                    ...
                    match serde_json::from_str::<BamlMedia>(media_data) {
                        Ok(m) => Some(PromptAst::without_span(PromptAstNode::Media(
                            baml_media_to_media_value(m),
                        ))),
                        Err(_) => {
                    ...
                    Some(PromptAst::without_span(PromptAstNode::Str(
                        part.trim().to_string(),
                    )))
                } else {
                    None
            ...
            if !parts.is_empty() {
                let content = if parts.len() == 1 {
                    ...
                    PromptAst::without_span(PromptAstNode::Vec(parts))
                };
                ...
                chat_messages.push(PromptAst::without_span(PromptAstNode::Message {
    ...
    for msg in &mut chat_messages {
        if let PromptAstNode::Message { role, .. } = &mut msg.node {
            if let Some(remap) = remap_role.get(role) {
                *role = remap.clone();
            }
        }
    }

    Ok(PromptAst::without_span(PromptAstNode::Vec(chat_messages)))
}

fn baml_media_to_media_value(media: BamlMedia) -> MediaValue {
    let kind = match media.media_type {
        BamlMediaType::Image => MediaKind::Image,
        BamlMediaType::Audio => MediaKind::Audio,
        BamlMediaType::Video => MediaKind::Video,
        BamlMediaType::Pdf => MediaKind::Pdf,
    };

    let (content, mime_type) = match media.content {
...
mod tests {
    ...
    fn test_render_prompt_completion() {
        ...
        match result.unwrap().node {
            PromptAstNode::Str(text) => assert_eq!(text, "Process: Hello"),
