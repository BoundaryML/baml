...
// `build_symbol_pool` → `to_source_code`. This crate started life
// generated by `tools_rig` but is now hand-written and modeled on
// `python_example_09a`.
use std::{
    env, fs,
    path::{Path, PathBuf},
...
fn main() {
    ...
    // 1. Discover + load .baml files into the project DB. The `empty`
    //    fixture legitimately has no `.baml` files; we don't assert
    //    non-emptiness here.
...
name = "baml-test-full-type-coverage"
version = "0.1.0"
requires-python = ">=3.8"
dependencies = []
